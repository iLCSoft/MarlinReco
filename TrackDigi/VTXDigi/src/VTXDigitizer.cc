...
VTXDigitizer::VTXDigitizer() : Processor("VTXDigitizer") {
  ...
  registerProcessorParameter("TanLorentz", "Tangent of Lorentz Angle", _tanLorentzAngle, (double)0.8);

  registerProcessorParameter("CutOnDeltaRays", "Cut on delta-ray energy (MeV)", _cutOnDeltaRays, (double)0.030);

  registerProcessorParameter("Diffusion", "Diffusion coefficient (in mm) for layer thickness", _diffusionCoefficient,
                             (double)0.002);

  registerProcessorParameter("PixelSizeX", "Pixel Size X", _pixelSizeX, (double)0.025);

  registerProcessorParameter("PixelSizeY", "Pixel Size Y", _pixelSizeY, (double)0.025);
  ...
  registerProcessorParameter("SegmentLength", "Segment Length", _segmentLength, double(0.005));

  registerProcessorParameter("WidthOfCluster", "Width of cluster", _widthOfCluster, double(3.0));
  ...
  registerProcessorParameter("ElectronicNoise", "electronic noise in electrons", _electronicNoise, 100.);

  registerProcessorParameter("StoreFiredPixels", "Store fired pixels", _produceFullPattern, int(0));

  registerProcessorParameter("UseMCPMomentum", "Use Particle Momentum", _useMCPMomentum, int(1));

  registerProcessorParameter("EnergyLoss", "Energy Loss keV/mm", _energyLoss, double(280.0));

  registerProcessorParameter("RemoveDRayPixels", "Remove D-Ray Pixels", _removeDrays, int(1));
...
void VTXDigitizer::end() {
  ...
  std::cout << "VTXDigitizer::end()  " << name() << " processed " << _nEvt << " events in " << _nRun << " runs "
            << std::endl;
...
void VTXDigitizer::ProduceIonisationPoints(SimTrackerHit* hit) {
  ...
  // std::cout << "HERE WE ARE " << trackLength << " " << _segmentLength << " "<<_layerThickness[_currentLayer]<<"
  // "<<tanx<<" "<<tany<<std::endl;

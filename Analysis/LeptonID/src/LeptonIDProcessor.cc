...
LeptonIDProcessor::LeptonIDProcessor() : Processor("LeptonIDProcessor"), _mvareader("!Color:!Silent") {
  ...
  _description = "The LeptonIDProcessor is used to identify electrons and muons using TMVA classifiers. It also "
                 "produces the TTrees needed to train them";

  // register steering parameters: name, description, class-variable, default value
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "PandoraPFOsName",
                          "Name of the collection holding the PID info usually PandoraPFOs", _PandoraPFOsColName,
                          std::string("PandoraPFOs"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputPFOsName",
                          "Name of the PFO collection the training or evaluation should be done on", _InputPFOsColName,
                          std::string("DistilledPFOs"));

  registerInputCollection(LCIO::LCRELATION, "RecoMCTruthLinkName", "Name of the RecoMCTruthLink collection",
                          _RecoMCTruthLinkName, std::string("RecoMCTruthLink"));

  registerInputCollection(LCIO::LCRELATION, "MCTruthRecoLinkName", "Name of the MCTruthRecoLink collection",
                          _MCTruthRecoLinkName, std::string("MCTruthRecoLink"));

  registerInputCollection(LCIO::CLUSTER, "PandoraClustersName", "Name of the PandoraClusters collection",
                          _PandoraClustersColName, std::string("PandoraClusters"));

  registerProcessorParameter("MCTruthRecoCweightCut", "Cut used on MCTruthRecoCluster weight", _MCTruthRecoCweightCut,
                             0.5F);

  registerProcessorParameter("MCTruthRecoTweightCut", "Cut used on MCTruthRecoTrack weight", _MCTruthRecoTweightCut,
                             0.5F);

  registerProcessorParameter("RecoMCTruthCweightCut", "Cut used on RecoMCTruthCluster weight", _RecoMCTruthCweightCut,
                             0.8F);

  registerProcessorParameter("RecoMCTruthTweightCut", "Cut used on RecoMCTruthTrack weight", _RecoMCTruthTweightCut,
                             0.5F);

  registerProcessorParameter("BuildTree", "Decides if the Processor builds a TTree that can be used for MVA training",
                             _buildTree, true);

  registerProcessorParameter("EvalMVA", "Decides if the Processor does the MVA evaluation (requires weight files)",
                             _evalMVA, false);

  registerProcessorParameter("weightfile", "path to the BDTG weightfile", _weightfile,
                             std::string("weights/PID_multi_jet_dEdx_800t_cm50r80_BDTG.weights.xml"));

  registerProcessorParameter("mvaname", "Name given to the MVA method during training, e.g. BDTG.", _mvaname,
                             std::string("BDTG"));

  registerProcessorParameter("pidname", "Name used to store the PID results in the ReconstructedParticle collection.",
                             _pidname, std::string("LeptonID"));

  registerProcessorParameter("dEdxname",
                             "Name used to read the dEdx distance from the PandoraPFO collection parameters.",
                             _dEdxname, std::string("dEdxPIDv2"));

  std::vector<std::string> usedVars = {"e_over_p",
                                       "ecal_share",
                                       "seenEcalDep",
                                       "seenYokeDep",
                                       "shape0",
  ...
  registerProcessorParameter("UsedVariables", "List of variables used during training", _usedVariables, usedVars);
}
...
void LeptonIDProcessor::processRunHeader(LCRunHeader* run) { (void)run; }
...
void LeptonIDProcessor::processEvent(LCEvent* evt) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (_buildTree) {
      MCParticle* mcp = getMCParticle(p, *RecoMCTruthNavigator, *MCTruthRecoNavigator, _RecoMCTruthCweightCut,
                                      _RecoMCTruthTweightCut, _MCTruthRecoCweightCut, _MCTruthRecoTweightCut);
      if (mcp == nullptr) {
        continue;
      }
      _truePDG = mcp->getPDG();

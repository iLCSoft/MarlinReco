...
struct RandomEngines {
  RandomEngines() {
    ...
    for (int i = 0; i < 30; i++)
      engines.push_back(std::mt19937(rd()));
  }
...
RVec<bool> selectHits_OLD(const RVec<float>& dToLine, const RVec<int>& layer_hit, bool only_closest = true,
                          int n_layers = 10, float cyl_cut = 9999.) {
  ...
  if (only_closest) {
    ...
    for (int layer = 0; layer < n_layers; ++layer) {
      if (layer2hit[layer].empty())
        continue;
      int idx = (*std::min_element(layer2hit[layer].begin(), layer2hit[layer].end(), [](const auto& l, const auto& r) {
...
float getClosestDtoLine(const RVec<float>& dToLine) { return *std::min_element(dToLine.begin(), dToLine.end()); }

...
double PIDVariable_base::BetheBloch(const PIDParticles::PIDParticle_base* hypothesis, const float p) {
  ...
  return (0.5 * hypothesis->GetBBpars()[0] * TMath::Log(hypothesis->GetBBpars()[1] * TMath::Power(bg, 2.0) * tmax) -
          hypothesis->GetBBpars()[3] * b * b - hypothesis->GetBBpars()[4] * bg / 2.0) /
         (b * b);
}
...
PID_CaloTotal::PID_CaloTotal() : PIDVariable_base("CaloTotal", "(ECAL+HCAL)/p", "") {}
...
PID_CaloEFrac::PID_CaloEFrac() : PIDVariable_base("CaloEFrac", "ECAL/(ECAL+HCAL)", "") {}
...
PID_CaloMuSys::PID_CaloMuSys() : PIDVariable_base("CaloMuSys", "E_{#mu system}", "GeV") {}
...
PID_CluShapeChi2::PID_CluShapeChi2() : PIDVariable_base("CluShapeChi2", "Cluster shape #chi^{2}", "") {}
...
PID_CluShapeLDiscr::PID_CluShapeLDiscr() : PIDVariable_base("DiscrepancyL", "d_{Shower max} / d_{EM shower max}", "") {}
...
PID_CluShapeTDiscr::PID_CluShapeTDiscr() : PIDVariable_base("DiscrepancyT", "Absorption length", "R_{m}") {}
...
PID_CluShapeXl20::PID_CluShapeXl20() : PIDVariable_base("Xl20", "xl20", "?") {}
...
PID_dEdxChi2::PID_dEdxChi2(const PID_dEdxChi2& ref)
    : PIDVariable_base(ref.Name(), ref.Description(), ref.Unit()), _hypothesis(ref._hypothesis),
      _dEdx_MIP(ref._dEdx_MIP) {}
...
PID_dEdxChi2::~PID_dEdxChi2() { delete _hypothesis; }

int PID_dEdxChi2::Update(const EVENT::ClusterVec /*cluvec*/, const EVENT::TrackVec trax, const TVector3 p3) {
  int result = 0;
  float ExpdEdx = BetheBloch(_hypothesis, p3.Mag());
  float dEdx;
  if (trax.size() > 0) {
    dEdx = trax.at(0)->getdEdx();
  } else {
    dEdx = -_dEdx_MIP;
    result |= MASK_EmptyTracks;
  } // Keep an eye on get_dEdxChi2()...
...
PID_dEdxLogChi2::PID_dEdxLogChi2(const PIDParticles::PIDParticle_base* hypothesis, const float dEdx_MIP)
    : PIDVariable_base(Form("dEdx_LogChi2_%s", hypothesis->Name()), Form("Log(#chi2_{dE/dx %s})", hypothesis->Name()),
                       ""),
      _hypothesis(hypothesis),
      _dEdx_MIP(dEdx_MIP) { /*std::cout << _name << "; " << _description << "; " << _unit << std::endl;*/ }

PID_dEdxLogChi2::~PID_dEdxLogChi2() { delete _hypothesis; }

int PID_dEdxLogChi2::Update(const EVENT::ClusterVec /*cluvec*/, const EVENT::TrackVec trax, const TVector3 p3) {
  int result = 0;
  float ExpdEdx = BetheBloch(_hypothesis, p3.Mag());
  float dEdx;
  if (trax.size() > 0) {
...
PIDVariables_base::PIDVariables_base() : _p(0.) {}

PIDVariables_base::PIDVariables_base(EVENT::ReconstructedParticle* /*particle*/) : _p(0.) {}

PIDVariables_base::~PIDVariables_base() { _varVec.clear(); }
...
int PIDVariables_base::Update(const EVENT::ClusterVec cluvec, const EVENT::TrackVec trax, const TVector3 p3) {
...
PIDVariables_LLPID::PIDVariables_LLPID() { Populate(); }
...
PIDVariables_MvaPid::PIDVariables_MvaPid() { Populate(); }

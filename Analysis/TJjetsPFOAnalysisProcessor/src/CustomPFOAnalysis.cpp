...
void TJjetsPFOAnalysisProcessor::ExtractCollections(JetContentPair* jet_content) {
  ...
  try {
    ...
    streamlog_out(DEBUG) << "N leftover mcs: " << preliminary_mcPfoCandidates.size() << " N input: " << mcs.size()
                         << std::endl;
...
void TJjetsPFOAnalysisProcessor::ApplyPfoSelectionRules(EVENT::MCParticle* pMCParticle,
                                                        MCParticleList& mcPfoCandidates) const {
  const float innerRadius(std::sqrt(pMCParticle->getVertex()[0] * pMCParticle->getVertex()[0] +
                                    pMCParticle->getVertex()[1] * pMCParticle->getVertex()[1] +
                                    pMCParticle->getVertex()[2] * pMCParticle->getVertex()[2]));
  const float outerRadius(std::sqrt(pMCParticle->getEndpoint()[0] * pMCParticle->getEndpoint()[0] +
                                    pMCParticle->getEndpoint()[1] * pMCParticle->getEndpoint()[1] +
                                    pMCParticle->getEndpoint()[2] * pMCParticle->getEndpoint()[2]));
  const float momentum(std::sqrt(pMCParticle->getMomentum()[0] * pMCParticle->getMomentum()[0] +
                                 pMCParticle->getMomentum()[1] * pMCParticle->getMomentum()[1] +
                                 pMCParticle->getMomentum()[2] * pMCParticle->getMomentum()[2]));

  if ((mcPfoCandidates.find(pMCParticle) == mcPfoCandidates.end()) && (outerRadius > m_mcPfoSelectionRadius) &&
      (innerRadius <= m_mcPfoSelectionRadius) && (momentum > m_mcPfoSelectionMomentum) &&
      !((pMCParticle->getPDG() == 2212 || pMCParticle->getPDG() == 2112) &&
        (pMCParticle->getEnergy() < m_mcPfoSelectionLowEnergyNPCutOff))) {
  ...
  //
  //     for (EVENT::MCParticleVec::const_iterator iter = daughterVector.begin(), iterEnd = daughterVector.end(); iter
  //     != iterEnd; ++iter)
  //     {
    ...
    for (unsigned int i = 0, nElements = mcs.size(); i < nElements; ++i) {
      ...
      if (!m_lookForQuarksWithMotherZ) {
        if ((absPdgCode >= 1) && (absPdgCode <= 6) &&
            !hasSomeParentsInMCList(pMCParticle,
                                    mcs_list)) // pMCParticle->getParents().empty()) //!!!!!!!!!!!!!!!!!!!!!!!!!!!
          mcQuarkVector.push_back(pMCParticle);
      } else {
        // For MC files generated in the SLIC environment, the primary quarks have parents; the mother should be the
        // Z-boson
        if ((absPdgCode >= 1) && (absPdgCode <= 6)) {
...
void TJjetsPFOAnalysisProcessor::PerformPfoAnalysis() {
  ...
  for (ParticleVector::const_iterator iter = m_pfoVector.begin(), iterEnd = m_pfoVector.end(); iter != iterEnd;
       ++iter) {
    ...
    const float momentum(std::sqrt(pPfo->getMomentum()[0] * pPfo->getMomentum()[0] +
                                   pPfo->getMomentum()[1] * pPfo->getMomentum()[1] +
                                   pPfo->getMomentum()[2] * pPfo->getMomentum()[2]));
    const float cosTheta((momentum > std::numeric_limits<float>::epsilon()) ? pPfo->getMomentum()[2] / momentum
                                                                            : -999.f);
    ...
    if (!pPfo->getTracks().empty()) {
      ...
      for (EVENT::ClusterVec::const_iterator clustIter = clusterVec.begin(), clustIterEnd = clusterVec.end();
           clustIter != clustIterEnd; ++clustIter) {
        const EVENT::CalorimeterHitVec& calorimeterHitVec((*clustIter)->getCalorimeterHits());

        for (EVENT::CalorimeterHitVec::const_iterator hitIter = calorimeterHitVec.begin(),
                                                      hitIterEnd = calorimeterHitVec.end();
      ...
      const float correctionFactor(
          (cellEnergySum < std::numeric_limits<float>::epsilon()) ? 0.f : pPfo->getEnergy() / cellEnergySum);
    ...
    if (m_printing > 0) {
      std::cout << " RECO PFO, pdg: " << pPfo->getType() << ", E: " << pPfo->getEnergy()
                << ", nTracks: " << pPfo->getTracks().size() << ", nClusters: " << pPfo->getClusters().size()
                << ", charge: " << pPfo->getCharge() << std::endl;
    }
  }

  m_pfoMassTotal = std::sqrt(m_pfoEnergyTotal * m_pfoEnergyTotal - momTot[0] * momTot[0] - momTot[1] * momTot[1] -
                             momTot[2] * momTot[2]);

  // Extract quantities relating to pfo targets
  for (MCParticleVector::const_iterator iter = m_pfoTargetVector.begin(), iterEnd = m_pfoTargetVector.end();
       iter != iterEnd; ++iter) {
    ...
    const float momentum(std::sqrt(pMCParticle->getMomentum()[0] * pMCParticle->getMomentum()[0] +
                                   pMCParticle->getMomentum()[1] * pMCParticle->getMomentum()[1] +
                                   pMCParticle->getMomentum()[2] * pMCParticle->getMomentum()[2]));
    const float cosTheta((momentum > std::numeric_limits<float>::epsilon()) ? pMCParticle->getMomentum()[2] / momentum
                                                                            : -999.f);
    ...
    if ((std::abs(pMCParticle->getPDG()) == 12) || (std::abs(pMCParticle->getPDG()) == 14) ||
        (std::abs(pMCParticle->getPDG()) == 16))
    ...
    } else if ((11 == std::abs(pMCParticle->getPDG())) || (13 == std::abs(pMCParticle->getPDG())) ||
               (211 == std::abs(pMCParticle->getPDG()))) // TODO, more options here?
    {
      ++m_nPfoTargetsTracks;
  ...
  if (m_printing > 0) {
    std::cout << " EVENT                : " << m_nEvt << std::endl
              << " NPFOs                : " << m_nPfosTotal << " (" << m_nPfosTracks << " + "
              << m_nPfosPhotons + m_nPfosNeutralHadrons << ")" << std::endl

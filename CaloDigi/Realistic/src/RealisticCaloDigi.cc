...
RealisticCaloDigi::RealisticCaloDigi() : Processor("RealisticCaloDigi") {
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "inputHitCollections", "input simcalhit Collection Names",
  ...
  registerProcessorParameter("outputHitCollections", "output calorimeterhit Collection Names", _outputCollections,
                             outputCollections);

  // the collections of relations between sim and digitised hits
  std::vector<std::string> outputRelCollections;
  registerProcessorParameter("outputRelationCollections", "output hit relatiob Collection Names", _outputRelCollections,
                             outputRelCollections);

  // energy threshold
  registerProcessorParameter("threshold", "Threshold for Hit", _threshold_value, (float)0.5);

  registerProcessorParameter(
      "thresholdUnit",
      "Unit for threshold. Can be \"GeV\", \"MIP\" or \"px\". MIP and px need properly set calibration constants",
      _threshold_unit, std::string("MIP"));

  // timing
  registerProcessorParameter("timingCut", "Use hit times", _time_apply, (int)0);

  registerProcessorParameter("timingCorrectForPropagation", "Correct hit times for propagation: radial distance/c",
                             _time_correctForPropagation, (int)0);

  registerProcessorParameter("timingWindowMin", "Time Window minimum time in ns", _time_windowMin, (float)-10.0);

  registerProcessorParameter("timingWindowMax", "Time Window maximum time in ns", _time_windowMax, (float)+100.0);

  registerProcessorParameter("integrationMethod",
                             "Energy integration and time calculation method. Options: Standard, ROC",
                             _integration_method, std::string("Standard"));

  registerProcessorParameter("fastShaper", "Fast shaper value. Unit in ns", _fast_shaper, (float)0.f);

  registerProcessorParameter("slowShaper", "Slow shaper value. Unit in ns", _slow_shaper, (float)0.f);

  registerProcessorParameter("timingResolution", "Time resolution to apply (gaussian smearing). Unit in ns",
                             _time_resol, (float)0);

  // additional digi effects

  registerProcessorParameter("calibration_mip", "average G4 deposited energy by MIP for calibration", _calib_mip,
                             (float)1.0e-4);

  registerProcessorParameter("miscalibration_uncorrel",
                             "uncorrelated random gaussian miscalibration (as a fraction: 1.0 = 100%) ",
                             _misCalib_uncorrel, (float)0.0);

  registerProcessorParameter("miscalibration_uncorrel_memorise",
                             "store oncorrelated miscalbrations in memory? (i.e. use same miscalibrations in each "
                             "event. WARNING: can take a lot of memory if used...) ",
                             _misCalib_uncorrel_keep, (bool)false);

  registerProcessorParameter("miscalibration_correl",
                             "correlated random gaussian miscalibration (as a fraction: 1.0 = 100%) ", _misCalib_correl,
                             (float)0.0);

  registerProcessorParameter("deadCell_fraction", "random dead cell fraction (as a fraction: 0->1) ",
                             _deadCell_fraction, (float)0.);

  registerProcessorParameter("deadCell_memorise",
                             "store dead cells in memory? (i.e. use same dead cells in each event. WARNING: can take a "
                             "lot of memory if used...) ",
                             _deadCell_keep, (bool)false);

  // simple model of electronics properties
  registerProcessorParameter("elec_noise_mip", "typical electronics noise (in MIP units)", _elec_noiseMip, float(0));

  registerProcessorParameter("elec_range_mip", "maximum of dynamic range of electronics (in MIPs)", _elec_rangeMip,
                             float(2500));
...
void RealisticCaloDigi::init() {
  ...
  if (_threshold_unit.compare("MIP") == 0) {
    ...
    streamlog_out(ERROR) << "could not identify threshold unit. Please use \"GeV\", \"MIP\" or \"px\"! Aborting."
                         << std::endl;
...
void RealisticCaloDigi::processEvent(LCEvent* evt) {
  ...
  if (_misCalib_correl > 0)
    _event_correl_miscalib = CLHEP::RandGauss::shoot(1.0, _misCalib_correl);
...
float RealisticCaloDigi::EnergyDigi(float energy, int id0, int id1) {
  ...
  if (_elec_noiseMip > 0)
    e_out += CLHEP::RandGauss::shoot(0, _elec_noiseMip * oneMipInMyUnits);
  ...
  if (_deadCell_fraction > 0) {
    if (_deadCell_keep == true) { // memorise dead cells from event to event
      ...
      if (CLHEP::RandFlat::shoot(0.0, 1.0) < _deadCell_fraction)
        e_out = 0;
    }
  }
  return e_out;
...
RealisticCaloDigi::integr_res_opt RealisticCaloDigi::ROCIntegration(const SimCalorimeterHit* hit) const {
  ...
  std::sort(mcconts.begin(), mcconts.end(), [](auto lhs, auto rhs) { return (lhs.time < rhs.time); });

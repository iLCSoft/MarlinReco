...
float getParameterFromPID(EVENT::ReconstructedParticle* pfo, UTIL::PIDHandler& pidHandler, std::string algorithmName,
...
float getTrackWeight(float encodedWeight) { return float(int(encodedWeight) % 10000) / 1000.f; }

float getClusterWeight(float encodedWeight) { return float(int(encodedWeight) / 10000) / 1000.f; }

EVENT::ReconstructedParticle* getRelatedReconstructedParticle(EVENT::MCParticle* mc,
                                                              const UTIL::LCRelationNavigator& mc2pfo,
                                                              const UTIL::LCRelationNavigator& pfo2mc) {
  ...
  if (objects.empty())
    return nullptr; // has NO linked recosntructed particle
  ...
  if (*maxTrackWeightIt == 0.f)
    maxIdx = std::distance(clusterWeights.begin(), maxClusterWeightIt);
...
EVENT::MCParticle* getMC(EVENT::ReconstructedParticle* pfo, const UTIL::LCRelationNavigator& pfo2mc) {
  ...
  if (objects.empty())
    return nullptr; // has NO linked MCParticles
...
float getECALBarelRMin() {
  float cm2mm = 10.;
  auto ecalBarrelData = MarlinUtil::getLayeredCalorimeterData(
      (DetType::CALORIMETER | DetType::ELECTROMAGNETIC | DetType::BARREL), (DetType::AUXILIARY | DetType::FORWARD));
...
float getECALEndcapZMin() {
  float cm2mm = 10.;
  auto ecalEndcapData = MarlinUtil::getLayeredCalorimeterData(
      (DetType::CALORIMETER | DetType::ELECTROMAGNETIC | DetType::ENDCAP), (DetType::AUXILIARY | DetType::FORWARD));
...
dd4hep::rec::Vector3D getPhotonAtCalorimeter(EVENT::MCParticle* mc) {
  ...
  // zMin - minimal longitudinal distance to the endcap ECAL surface to deduce normal vector n and point at the surface
  // p0.
  ...
  if (intersectionBarrel.r() <= intersectionEndcap.r())
    return intersectionBarrel;
...
EVENT::SimTrackerHit* getSimTrackerHit(EVENT::TrackerHit* hit, const UTIL::LCRelationNavigator& navToSimTrackerHits) {
  ...
  if (navToSimTrackerHits.getRelatedToObjects(hit).empty())
    return nullptr;
  ...
  EVENT::SimTrackerHit* simHit =
      static_cast<EVENT::SimTrackerHit*>(navToSimTrackerHits.getRelatedToObjects(hit)[max_i]);
  return simHit;
}
...
float getZ0True(EVENT::MCParticle* mc) {
  ...
  if (std::abs(n1 - xCircles) < std::abs(n2 - xCircles))
    nCircles = n1;
  else
    nCircles = n2;
...
EVENT::MCParticle* getFirstStableParent(EVENT::MCParticle* mc) {
  ...
  if (isNotShortResonance || isMCInternal)
    return parent;
  return getFirstStableParent(parent);
}

bool isBottomPDG(unsigned int pdg) {
  std::vector<unsigned int> bottomPDGs = {
      511,    521,    10511,  10521,   513,     523,   10513, 10523, 20513,  20523,  515,    525,    531,
      10531,  533,    10533,  20533,   535,     541,   10541, 543,   10543,  20543,  545,    551,    10551,
      100551, 110551, 200551, 210551,  553,     10553, 20553, 30553, 100553, 110553, 120553, 130553, 200553,
      210553, 220553, 300553, 9000553, 9010553, 555,   10555, 20555, 100555, 110555, 120555, 200555, 557,
      100557, 5122,   5112,   5212,    5222,    5114,  5214,  5224,  5132,   5232,   5312,   5322,   5314,
      5324,   5332,   5334,   5142,    5242,    5412,  5422,  5414,  5424,   5342,   5432,   5434,   5442,
      5444,   5512,   5522,   5514,    5524,    5532,  5534,  5542,  5544,   5554};
  return std::find(bottomPDGs.begin(), bottomPDGs.end(), pdg) != bottomPDGs.end();
}

bool isCharmPDG(unsigned int pdg) {
  std::vector<unsigned int> charmPDGs = {
      411,     421,   10411,  10421, 413,  423,   10413,  10423, 20413, 20423, 415,    425,   431,     10431,
      433,     10433, 20433,  435,   441,  10441, 100441, 443,   10443, 20443, 100443, 30443, 9000443, 9010443,
      9020443, 445,   100445, 4122,  4222, 4212,  4112,   4224,  4214,  4114,  4232,   4132,  4322,    4312,
...
bool isStrangePDG(unsigned int pdg) {
  std::vector<unsigned int> strangePDGs = {
      3122,    3222,    3212, 3112, 3224,    3214,    3114,  3322,  3312,    3324,   3314,    3334,
      130,     310,     311,  321,  9000311, 9000321, 10311, 10321, 100311,  100321, 9010311, 9010321,
      9020311, 9020321, 313,  323,  10313,   10323,   20313, 20323, 100313,  100323, 9000313, 9000323,
      30313,   30323,   315,  325,  9000315, 9000325, 10315, 10325, 20315,   20325,  9010315, 9010325,
      9020315, 9020325, 317,  327,  9010317, 9010327, 319,   329,   9000319, 9000329};
  return std::find(strangePDGs.begin(), strangePDGs.end(), pdg) != strangePDGs.end();
}

bool isMCInternalPDG(unsigned int pdg) {
  return (80 < pdg && pdg < 101) || (900 < pdg && pdg < 931) || (997 < pdg && pdg < 999) ||
         (1900 < pdg && pdg < 1931) || (2900 < pdg && pdg < 2931) || (3900 < pdg && pdg < 3931);
}
...
unsigned int getQuarkTypeDecay(EVENT::MCParticle* mc) {
  ...
  if (parent->getParents().size() != 1)
    return 0; // probably overlay
  while (true) {
    parent = parent->getParents()[0];
    if (isMCInternalPDG(std::abs(parent->getPDG())))
      break;
    pdgs.push_back(std::abs(parent->getPDG()));
    if (parent->getParents().size() != 1)
      return 0; // probably overlay
  ...
  if (std::find_if(pdgs.begin(), pdgs.end(), [](unsigned int pdg) { return isBottomPDG(pdg); }) != pdgs.end())
    return 5;
  else if (std::find_if(pdgs.begin(), pdgs.end(), [](unsigned int pdg) { return isCharmPDG(pdg); }) != pdgs.end())
    return 4;
  return 1;
}
...
std::vector<VertexData> getReconstructableTrueVertices(EVENT::LCEvent* evt) {
  ...
  for (int i = 0; i < mcs->getNumberOfElements(); ++i) {
    ...
    if (pfo == nullptr || pfo->getTracks().size() == 0)
      continue;
  ...
  for (auto& vtxData : verticies) {
    ...
    for (auto mc : vtxData.mcs)
      streamlog_out(DEBUG2) << mc << "    ";
    streamlog_out(DEBUG2) << std::endl;
  }

  // Merge all vertices within 3 um and combine their MC particles. And recalculate vertex position as a weighted
  // average
  ...
  verticies.erase(
      std::remove_if(verticies.begin(), verticies.end(), [](const VertexData& vtx) { return vtx.mcs.size() < 2; }),
      verticies.end());
  ...
  auto compDistanceToTrue = [&](const VertexData& a, const VertexData& b) {
    return (a.pos - primVertexPosTrue).r() < (b.pos - primVertexPosTrue).r();
  };

  if (verticies.empty())
    return verticies; // no TRUE primary vertex may hapen. This happens once in ~200k events.
  (*std::min_element(verticies.begin(), verticies.end(), compDistanceToTrue)).isPrimary = true;

  for (auto& vtx : verticies) {
    if (vtx.mcs.size() != 2)
      continue;
    if (vtx.mcs[0]->getCharge() == vtx.mcs[1]->getCharge())
      continue;
...
void mergeCloseVerticies(std::vector<VertexData>& verticies, double mergeDistance = 0.003) {
  for (auto iter1 = verticies.begin(); iter1 != verticies.end();) {
    ...
    for (auto iter2 = std::next(iter1); iter2 != verticies.end();) {
      // streamlog_out(DEBUG1)<<"Beginning merge loop Iter1: "<<std::distance(vtx2mcs.begin(),
      // iter1)<<"/"<<vtx2mcs.size()<<endl; streamlog_out(DEBUG1)<<"Beginning merge loop Iter2:
      // "<<std::distance(vtx2mcs.begin(), iter2)<<"/"<<vtx2mcs.size()<<endl;
      Vector3D pos1 = (*iter1).pos;
      Vector3D pos2 = (*iter2).pos;
      double distance = (pos1 - pos2).r();
      // Assuming vertex detector resolution 3um, merge vertices within 3um and recalculate vertex position as a
      // weighted average. NOTE: I merge them in arbitrary order. Practially it doesn't matter, but this may cause a
      ...
      if (distance < mergeDistance) {
        ...
        // removing two vertices that are being merged. erase-remove idiom:
        // https://stackoverflow.com/questions/347441/how-can-you-erase-elements-from-a-vector-while-iterating
        verticies.erase(std::remove_if(verticies.begin(), verticies.end(),
                                       [&](const VertexData& vtx) { return vtx.mcs == mcs1 || vtx.mcs == mcs2; }),
                        verticies.end());
...
unsigned int getQuarksToPythia(EVENT::LCEvent* evt) {
  ...
  for (int i = 0; i < mcs->getNumberOfElements(); ++i) {
    EVENT::MCParticle* mc = static_cast<EVENT::MCParticle*>(mcs->getElementAt(i));
    if (mc == nullptr || mc->getPDG() != 94)
      continue;
    auto parents = mc->getParents();
    for (auto parent : parents) {
      unsigned int quarkPDG = std::abs(parent->getPDG());
      if (not(0 < quarkPDG && quarkPDG < 10) && not(quarkPDG == 21)) {
        streamlog_out(DEBUG8) << "Ignoring parents of pdg=94 (pythia input) in the event " << evt->getEventNumber()
                              << " with abs(pdg) : " << quarkPDG << std::endl;
        continue;
      }
      if (quarkPDG == 21)
        pdgs.push_back(9);
  ...
  for (size_t i = 0; i < pdgs.size(); i++)
    encodedPDGs += pdgs[i] * int(std::pow(10, i));
  return encodedPDGs;
}
...
unsigned int getHiggsDaughters(EVENT::LCEvent* evt) {
  ...
  unsigned int encodedPDGs = 0;
  for (size_t i = 0; i < pdgs.size(); i++)
    encodedPDGs += pdgs[i] * int(std::pow(10, 2 * i));
  return encodedPDGs;
}
...
EVENT::LCObject* getMatchingElement(EVENT::LCCollection* col1, EVENT::LCObject* requestedObject,
                                    EVENT::LCCollection* col2) {
  // Return LCObject from col2 that matches the index of object in col1.
  for (int i = 0; i < col1->getNumberOfElements(); ++i) {
    if (requestedObject == col1->getElementAt(i) && i < col2->getNumberOfElements())
      return col2->getElementAt(i);
  }
  // IMPROVE: print collection names. I hope they will be stored in the corresponding in the future. Now they are
  // accesible only through LCEvent
  streamlog_out(WARNING) << "Could not find a matching object in the collection " << col2
                         << " for the requested object " << requestedObject << " in the collection " << col1
                         << std::endl;
  return nullptr;
}
...
EVENT::LCCollection* getMCParticleCollection(EVENT::LCEvent* evt) {
  ...
  if (std::find(names->begin(), names->end(), "MCParticle") != names->end())
    return evt->getCollection("MCParticle");
  else if (std::find(names->begin(), names->end(), "MCParticlesSkimmed") != names->end())
    return evt->getCollection("MCParticlesSkimmed");
  else
    return nullptr;
}

...
std::vector<std::string> InputAlgorithm_dEdx_RCD::init(const std::vector<float>& inparF,
                                                       const std::vector<std::string>& inparS) {
  print_init(inparF, inparS);

  if (inparF.size() < 25) {
    sloE << _algoName << ": InputAlgorithm dEdx_RCD received too few (<25) parameters for reference curves!"
         << std::endl;
  ...
  for (int i = 0; i < 5; ++i)
    _RC_parameters.push_back(std::vector<double>(it + i * 5, it + i * 5 + 5));
...
std::vector<std::pair<float, float>> InputAlgorithm_dEdx_RCD::extractObservables(ReconstructedParticleImpl* pfo,
                                                                                 LCCollection*, int PDG) {
  std::vector<std::pair<float, float>> obsValues;
  for (int i = 0; i < 5; ++i)
    obsValues.push_back(std::pair<float, float>{-1000, -1});
...
double InputAlgorithm_dEdx_RCD::BB_curve(double mass, double mom, std::vector<double>& pars) {
  ...
  return 1.35e-7 * (.5 * pars[0] * log(pars[1] * pow(bg, 2.) * tmax) - pars[3] * bb * bb - pars[4] * bg / 2.) /
         (bb * bb);
}

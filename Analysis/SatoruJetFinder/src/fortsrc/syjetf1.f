...
*  Particle Re-association Routine (Mode=1)
*  Iteratively, core is re-defined when a particle is added to the jet
*
...
*--
     ...
     +    (PART(6)*PJSTMP(6,J))))/(9.*(PART(4)+PJSTMP(4,J))**2)  ! Geneva
            EXAM(6) = EJADE   ! it makes too bad cross talk
...
*        NEW ASSIGN AND NEW JETS
         JETASS(II) = IEXAMN
         DO K=1,4
            PJSTMP(K,IEXAMN) =  PJSTMP(K,IEXAMN) + PART(K)
         ENDDO
         PJSTMP(6,IEXAMN) = PJSTMP(1,IEXAMN)**2+
     +                      PJSTMP(2,IEXAMN)**2+PJSTMP(3,IEXAMN)**2
         PJSTMP(5,IEXAMN) = PJSTMP(4,IEXAMN)**2-PJSTMP(6,IEXAMN)
         PJSTMP(6,IEXAMN) = SQRT(MAX(0.,PJSTMP(6,IEXAMN)))
         PJSTMP(5,IEXAMN) = SQRT(MAX(0.,PJSTMP(5,IEXAMN)))
      ENDDO

...
EMShowerFinder::EMShowerFinder() : Processor("EMShowerFinder") {
  ...
  registerProcessorParameter("colNameECAL", "ECAL Collection Name", _colNameECAL, std::string("ECAL"));

  registerProcessorParameter("collectionNameOfEMShowerCandidates", "Name of the collection of EM shower candidates",
                             _collectionNameOfEMShowerCandidates, std::string("EMShowerCandidates"));

  registerProcessorParameter("Cleaning", "To do the cleaning on hits or not ", _ToClean, std::string("YES"));
  registerProcessorParameter(
      "TopologicalCut", "At which number of neighbors to put the threshold, condition is < so you need to put N+1 ",
      _CleanCut, (int)5);

  registerProcessorParameter("NumberOfLevels", "Number of levels for central loop ", _N, (int)10);
  ...
  registerProcessorParameter("Levels", "Levels for central loop in MIP ", _miipstep, miipstep);

  registerProcessorParameter("MinHit0", "Minimal Number of hits for ground level cluster ", _MinHit0, (int)4);

  registerProcessorParameter("MinHitSplit", "Minimal Number of hits for i-th level cluster ", _MinHitSplit, (int)2);

  registerProcessorParameter("Rcut", "Fluctuation suprresion cut", _Rcut, (double)0.4);
  registerProcessorParameter("Distcut", "Square of distance cut for merging ", _Distcut, (double)35.0);
  registerProcessorParameter("Coscut", "Cosine of the angle for merging ", _Coscut, (double)0.95);

  registerProcessorParameter("energyDeviationCut",
                             "cut on energy deviation of em shower candidates from estimated energy ( abs( (Ecluster - "
                             "Eestimated)/Eestimated ) < cut )",
                             _energyDeviationCut, (double)0.25);

  registerProcessorParameter("probabilityDensityCut", "cut on the probability density to assign hits to shower cores",
                             _probabilityDensityCut, (double)1E-3);

  registerProcessorParameter("DebugLevel",
                             "limits the amount of information written to std out (0 - none, 9 - maximal information)",
                             _debugLevel, int(0));

  registerProcessorParameter("DrawOnCED", "draw objects on CED", _drawOnCED, int(0));
}
...
void EMShowerFinder::processRunHeader(LCRunHeader* /*run*/) { ++_nRun; }
...
void EMShowerFinder::processEvent(LCEvent* evt) {
  ...
  try {
    ...
    if (colt != 0) {
      ...
      for (unsigned int i = 0; i < prs2.size(); i++) {
        if (prs2[i].active == true) {
          ...
          for (unsigned int j = 0; j < prs2[i].cl->hits.size(); j++) {
            ...
            // debug
            if (_drawOnCED)
              ced_hit(prs2[i].cl->hits[j]->chit->getPosition()[0], prs2[i].cl->hits[j]->chit->getPosition()[1],
                      prs2[i].cl->hits[j]->chit->getPosition()[2], 0 | 4 << CED_LAYER_SHIFT, 2, 0xff47e6);

            double vCenterToHit[3];
            for (unsigned int k = 0; k < 3; k++)
              vCenterToHit[k] = centerPosition[k] - prs2[i].cl->hits[j]->chit->getPosition()[k];

            double projectionAlongMainPrincipalAxis = 0.0;
            for (unsigned int k = 0; k < 3; k++)
              projectionAlongMainPrincipalAxis += directionOfCluster[k] * vCenterToHit[k];
            ...
            if (projectionAlongMainPrincipalAxis < minProjectionAlongMainPrincipalAxis) {
              ...
              for (unsigned int k = 0; k < 3; k++)
                startPositionMinProjection[k] =
                    centerPosition[k] - 2.0 * minProjectionAlongMainPrincipalAxis * directionOfCluster[k];
              // for(unsigned int k=0;k<3;k++) startPositionMinProjection[k] =
              // prs2[i].cl->hits[j]->chit->getPosition()[k];
            }
            ...
            if (projectionAlongMainPrincipalAxis > maxProjectionAlongMainPrincipalAxis) {
              ...
              for (unsigned int k = 0; k < 3; k++)
                startPositionMaxProjection[k] =
                    centerPosition[k] - 2.0 * maxProjectionAlongMainPrincipalAxis * directionOfCluster[k];
              // for(unsigned int k=0;k<3;k++) startPositionMaxProjection[k] =
              // prs2[i].cl->hits[j]->chit->getPosition()[k];
            }

            double rStartPositionMinProjection =
                sqrt(pow(startPositionMinProjection[0], 2) + pow(startPositionMinProjection[1], 2) +
                     pow(startPositionMinProjection[2], 2));
            double rStartPositionMaxProjection =
                sqrt(pow(startPositionMaxProjection[0], 2) + pow(startPositionMaxProjection[1], 2) +
                     pow(startPositionMaxProjection[2], 2));
          ...
          if (_debugLevel > 5) {
            std::cout << "EM Shower Core found by the KIT package: " << std::endl
                      << "CoreEnergy: " << coreEnergy << "  " << " estimated 'em' particle energy: " << photonE
                      << std::endl
                      << "centerPos: " << "(" << centerPosition[0] << ", " << centerPosition[1] << ", "
                      << centerPosition[2] << ")"
                      << "  " << "startPos: " << "(" << startPosition[0] << ", " << startPosition[1] << ", "
                      << startPosition[2] << ")" << std::endl
                      << std::endl;
          }
            ...
            for (unsigned int j = 0; j < nelem; ++j) {
              ...
              if (!isECALHitAlreadyAssigned) {
                ...
                if (probabilityAndDistance[0] > 0.0) {
                  ...
                  ECALHitsWithAttributes.at(indexOfECALHitAlreadyAssigned)
                      .probabilitiesForThisECALHit.push_back(probabilityAndDistance[0]);
                  ECALHitsWithAttributes.at(indexOfECALHitAlreadyAssigned)
                      .distancesToCoresForThisECALHit.push_back(probabilityAndDistance[1]);
      ...
      if (_debugLevel > 5) {
        ...
        for (unsigned int i = 0; i < ECALHitsWithAttributes.size(); ++i) {
          std::cout << "ECAL Hit: " << ECALHitsWithAttributes.at(i).ECALHit << " ("
                    << ECALHitsWithAttributes.at(i).ECALHit->getPosition()[0] << ", "
                    << ECALHitsWithAttributes.at(i).ECALHit->getPosition()[1] << ", "
          ...
          for (unsigned int j = 0; j < ECALHitsWithAttributes.at(i).relatedCores.size(); ++j) {
            ...
            if (j < ECALHitsWithAttributes.at(i).relatedCores.size() - 1)
              std::cout << ",";
          }
          std::cout << ")" << "  " << std::endl;
          std::cout << "Probs: " << "(";
          for (unsigned int j = 0; j < ECALHitsWithAttributes.at(i).probabilitiesForThisECALHit.size(); ++j) {
            ...
            if (j < ECALHitsWithAttributes.at(i).probabilitiesForThisECALHit.size() - 1)
              std::cout << ",";
          }
          std::cout << ")" << "  " << std::endl;
          std::cout << "Dists: " << "(";
          for (unsigned int j = 0; j < ECALHitsWithAttributes.at(i).distancesToCoresForThisECALHit.size(); ++j) {
            ...
            if (j < ECALHitsWithAttributes.at(i).distancesToCoresForThisECALHit.size() - 1)
              std::cout << ",";
          }
          std::cout << ")" << "  " << std::endl;
          ...
          for (unsigned int j = 0; j < ECALHitsWithAttributes.at(i).estimatedEnergyPerCore.size(); ++j) {
            ...
            if (j < ECALHitsWithAttributes.at(i).estimatedEnergyPerCore.size() - 1)
              std::cout << ",";
          }
      ...
      for (unsigned int i = 0; i < ECALHitsWithAttributes.size(); ++i) {
        ...
        if (ECALHitsWithAttributes.at(i).relatedCores.size() == 1) {
          ...
          estimatedEnergyOfCoreToAddHitTo =
              ECALHitsWithAttributes.at(i).estimatedEnergyPerCore.at(indexOfMaxProbability);
        }
      ...
      for (std::map<PROTSEED2*, ClusterImpl*>::iterator i = mapCoreToCluster.begin(); i != mapCoreToCluster.end();
           ++i) {
        ...
        if (_debugLevel > 5) {
          std::cout << std::endl
                    ...
                    << "energy of cluster: " << energy << "  " << "estimated energy: " << estimatedEnergyOfCluster
                    << "  " << "deviation: " << energyDeviation << std::endl
                    << "Calorimeter Hist in this Cluster:" << std::endl;
          for (unsigned int j = 0; j < cluster->getCalorimeterHits().size(); ++j) {

            std::cout << "Position: " << "(" << cluster->getCalorimeterHits().at(j)->getPosition()[0] << ","
                      << cluster->getCalorimeterHits().at(j)->getPosition()[1] << ","
                      << cluster->getCalorimeterHits().at(j)->getPosition()[2] << ")" << "  "
        ...
        if ((energyDeviation < _energyDeviationCut) && hitWithinCutRangeFound) {
          ...
          // flag hits
          for (unsigned int j = 0; j < cluster->getCalorimeterHits().size(); ++j)
            cluster->getCalorimeterHits().at(j)->ext<isPartOfEMShowerCandidate>() = 1;
          ...
          // debug
          if (_debugLevel > 5) {
            std::cout << "EM Shower Candidate FOUND : " << "cluster " << cluster
                      << " with energy E = " << cluster->getEnergy() << "  "
                      << "n of hits = " << cluster->getCalorimeterHits().size() << std::endl;
          }
    ...
    std::cout << "no valid ECAL collection in event " << _nEvt << std::endl;
  }

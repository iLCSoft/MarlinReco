...
BohdanAna::BohdanAna() : marlin::Processor("BohdanAna"), EventDisplayer(this) {
  ...
  registerProcessorParameter("produce_csv_output", "Produce csv output file for Konrad or not", _produce_csv_output,
                             false);

  registerProcessorParameter("dst_mode", "Write only DST level output", _dst_mode, false);

  // Rerunning LCFIPlus vertexing on statistics of DST files is very slow and is a bottleneck.
  registerProcessorParameter("produce_refit_output", "Store info about refitted tracks and vertices",
                             _produce_refit_output, true);
}
...
void BohdanAna::init() {
  ...
  if (fileNames.empty())
    streamlog_out(ERROR) << " No slcio files provided!" << std::endl;
  for (auto& name : fileNames) {
    if (_dst_mode && name.find("rec") != std::string::npos) {
      streamlog_out(WARNING) << " Found \"rec\" in a filename while using DST mode! use DST files with full statistics!"
                             << std::endl;
      break;
    } else if (not _dst_mode && name.find("dst") != std::string::npos) {
      streamlog_out(ERROR) << " Found \"dst\" in a filename while using REC mode! Use DST mode or a REC file. Exiting "
                              "to prevent a crash."
                           << std::endl;
      exit(1);
    }
  ...
  if (_produce_csv_output) {
    if (_dst_mode)
      streamlog_out(ERROR) << " I cannot produce Konrad CSV file with hit information while running in DST mode!"
                           << std::endl;
...
void BohdanAna::fillMCTrueInfo(EVENT::MCParticle* mc, EVENT::ReconstructedParticle* pfo) {
  ...
  if (not mc->getParents().empty()) {
    ...
    if (firstStableParent != nullptr)
      _firstStableParentPDG = firstStableParent->getPDG();
  }
...
void BohdanAna::fillTrueVertexInfo(EVENT::MCParticle* mc, const std::vector<VertexData>& trueVertices) {
  for (auto& vtx : trueVertices) {
    ...
    if (it == vtx.mcs.end())
      continue;
    if (vtx.isPrimary)
      _isInTruePrimaryVertex = true;
    else
      _isInTrueSecondaryVertex = true;
    ...
    for (auto* vtxMc : vtx.mcs) {
      fourMomentum +=
          PxPyPzEVector(vtxMc->getMomentum()[0], vtxMc->getMomentum()[1], vtxMc->getMomentum()[2], vtxMc->getEnergy());
    }
...
void BohdanAna::fillRecoVertexInfo(EVENT::LCEvent* evt, EVENT::MCParticle* mc,
  ...
  for (int i = 0; i < primVtxCol->getNumberOfElements(); ++i) {
    ...
    auto it = std::find_if(vtxPfos.begin(), vtxPfos.end(),
                           [&](EVENT::ReconstructedParticle* pfo) { return getMC(pfo, pfo2mc) == mc; });
    if (it == vtxPfos.end())
      continue;
  ...
  // NOTE: Ideally MC particle must be only in secondary or primary vertex. However, due to the bug cleaning up tracks
  // from primary vertex it can appear in both. In this case I write the properties of the secondary vertex as the most
  // interesting one, but still mark recoInPrimary as true.
  ...
  if (matchedVertex != nullptr) {
    ...
    for (auto* pfo : prongs) {
      fourMomentum +=
          PxPyPzEVector(pfo->getMomentum()[0], pfo->getMomentum()[1], pfo->getMomentum()[2], pfo->getEnergy());
    }
    _invMassOfRecoVertex = fourMomentum.M();
    auto compEnergy = [](ReconstructedParticle* a, ReconstructedParticle* b) {
  ...
  if (matchedRefittedVertex != nullptr) {
    ...
    _minEnergyOfRecoRefitVertex =
        (*std::min_element(refittedProngs.begin(), refittedProngs.end(), compEnergy))->getEnergy();
...
void BohdanAna::fillTrackStates(EVENT::ReconstructedParticle* pfo, EVENT::ReconstructedParticle* refittedPfo) {
  ...
  if (refittedPfo == nullptr || refittedPfo->getTracks().size() != 1 ||
      refittedPfo->getTracks()[0]->getOmega() == 0.f || refittedPfo->getTracks()[0]->getTanLambda() == 0.f)
    return;
  ...
  _refittedRecoIpMom = Vector3D(refittedMomIP[0], refittedMomIP[1],
                                refittedMomIP[2]); // should be identical to refittedPfo->getMomentum();
...
void BohdanAna::fillTrackLengthInfo(EVENT::MCParticle* mc, EVENT::ReconstructedParticle* pfo,
                                    const UTIL::LCRelationNavigator& navToSimTrackerHits) {
  if (pfo == nullptr || pfo->getTracks().size() != 1)
    return;
  auto track = pfo->getTracks()[0];
  ...
  for (auto& hitState : trackHitStates) {
    ...
    if (hitState.simHit != nullptr && hitState.simHit->getMCParticle() != mc)
      _cleanTrack = false;
  }

  std::tie(_trackLength_IKF_phiLambda, _harmonicMom_IKF_phiLambda) =
      getTrackLengthIKF(trackStates, _bField, TrackLengthOption::phiLambda);
  std::tie(_trackLength_IKF_phiZed, _harmonicMom_IKF_phiZed) =
      getTrackLengthIKF(trackStates, _bField, TrackLengthOption::phiZed);
  std::tie(_trackLength_IKF_zedLambda, _harmonicMom_IKF_zedLambda) =
      getTrackLengthIKF(trackStates, _bField, TrackLengthOption::zedLambda);

  auto it = std::find_if(trackHitStates.begin(), trackHitStates.end(),
                         [](const HitState& hitState) { return isSETHit(hitState.hit); });
  bool foundSETHit = it != trackHitStates.end();
  if (foundSETHit) {
    int idx = it - trackHitStates.begin();
    std::vector<IMPL::TrackStateImpl> trackStatesToSET =
        std::vector<IMPL::TrackStateImpl>(trackStates.begin(), trackStates.begin() + idx + 1);
    std::tie(_trackLengthToSET_IKF_phiLambda, _harmonicMomToSET_IKF_phiLambda) =
        getTrackLengthIKF(trackStatesToSET, _bField, TrackLengthOption::phiLambda);
    std::tie(_trackLengthToSET_IKF_phiZed, _harmonicMomToSET_IKF_phiZed) =
        getTrackLengthIKF(trackStatesToSET, _bField, TrackLengthOption::phiZed);
    std::tie(_trackLengthToSET_IKF_zedLambda, _harmonicMomToSET_IKF_zedLambda) =
        getTrackLengthIKF(trackStatesToSET, _bField, TrackLengthOption::zedLambda);
  }
}

void BohdanAna::fillTOFInfo(EVENT::MCParticle* mc, EVENT::ReconstructedParticle* pfo,
...
void BohdanAna::processEvent(EVENT::LCEvent* evt) {
  ...
  auto primVertexIt =
      std::find_if(trueVertices.begin(), trueVertices.end(), [](const VertexData& vtx) { return vtx.isPrimary; });
  if (primVertexIt != trueVertices.end())
    _primVertexTrue = (*primVertexIt).pos;
  ...
  for (int i = 0; i < mcs->getNumberOfElements(); ++i) {
    ...
    if (_produce_refit_output)
      refittedPFO = static_cast<ReconstructedParticle*>(getMatchingElement(pfos, pfo, updatedPfos));
    ...
    if (not _dst_mode) {
      ...
      if (_produce_csv_output && _tofClosest[0] > 6.)
        fillCsvForKonrad(pfo, _pdg, _tofClosest[0], _trackLength_IKF_zedLambda);
    }

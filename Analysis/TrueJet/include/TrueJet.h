...
protected:
  ...
  /**  ouput collection names. The corresponding collections are filled from the internal data structues at the end of
   * processEvent */
...
private:
  ...
  void first_parton(int this_partic, int this_jet, int& first_partic, int& last_94_parent, int& nfsr, int& info,
                    int& info2);
  int flavour(int k2);
  void isr();
  void photon();
  ...
  int fafp_last[26]{};  // fafp = fermion-antifermion pair. _last is the last created = the first if there was no gluon
                        // splitting or from a W from eg a top decay
  ...
  int dijet_begining[26]{}; // the initial di-jet of the jet. the fafp:s of all jets in this group is the initial fafp
                            // ie. ie the fermion anti-fermion pair the boson directly decayed to. A colour singlet.

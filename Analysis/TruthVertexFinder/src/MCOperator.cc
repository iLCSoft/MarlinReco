...
MCParticle* MCOperator::GetConsistentDaughter(MCParticle* parent, MCParticle* service, PDGTYPE type) {
  ...
  for (unsigned int i = 0; i < angle.size(); i++) {
    ...
    if (angle[i] < minAngle) // &&
                             ...
                             //(daughters[i]->getCharge()*parent->getCharge() > -0.0001 && ( type == BOTTOM_MESONS ||
                             // type == CHARMED_MESONS || type == STRANGE_MESONS )))
    {
      minAngle = angle[i];
      winner = i;
    }
  ...
  if (winner > -1) {
    ...
    if (daughters[winner]->getCharge() * parent->getCharge() < -0.0001 && type != BOTTOM_HADRONS &&
        type != BOTTOM_BARYONS) {
...
MCParticle* MCOperator::FindYoungestChild(MCParticle* parent, PDGTYPE type) {
  ...
  if (result) {
    if (count > 1 && abs(parent->getPDG()) != 92 && (type == CHARMED_MESONS || type == CHARMED_HADRONS)) {
      streamlog_out(MESSAGE) << "FATAL: Daughter multiplicity found for meson type " << type << " and parent "
                             << parent->getPDG() << "!\n";
      return NULL;
    }
    return result;
...
vector<MCParticle*> MCOperator::SelectStableCloseDaughters(MCParticle* parent, int excludePDG, bool selectReco,
                                                           vector<MCParticle*>* misReconstructed) {
  vector<MCParticle*> result;
  if (!parent || CheckParticle(parent, NONTRACKABLE_PARTICLES) ||
  ...
  for (unsigned int i = 0; i < daughters.size(); i++) {
    ...
    if (CheckParticle(daughter, TRACKABLE_PARTICLES)) {
      ...
      if (MathOperator::getModule(daughter->getMomentum()) < 0.1) {
        // streamlog_out(MESSAGE)<<"CAUTION: Low momentum particle of " <<
        // MathOperator::getModule(daughter->getMomentum()) << " GeV!\n";
      }
...
bool MCOperator::CheckCompatibility(const vector<MCParticle*>& daughters, MCParticle* parent, int plusCharge) {
  ...
  if (charge + plusCharge != (int)(parent->getCharge())) {
    streamlog_out(MESSAGE) << "CAUTION: Charge is not compatible: charges " << charge << " and " << parent->getCharge()
                           << ", " << plusCharge << "!\n";
    return false;
  }
...
vector<MCParticle*> MCOperator::CheckDaughterVisibility(const vector<MCParticle*>& daughters) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    streamlog_out(MESSAGE) << "\tPDG: " << daughter->getPDG() << " p: " << p << " pt: " << pt << " teta: " << eta
                           << " offset: " << offset << '\n';

...
int PDF::InitHistogram(std::string HistName, int Dim, std::string* VarName, double* StartVal, double* BinWidth,
                       int* NoOfBins) {
  int ret = 0;
  ...
  for (int i = 0; i < nCats; i++) {
    ...
    if (Dim > VO->GetNoOfVariables()) {
      std::cout << " Dimension of histogram can be at most the # of variables in VObject " << VO->GetNoOfVariables()
                << std::endl;
      return -1;
    }
...
int PDF::FillHistograms(std::string CatName) {
  ...
  if (ret == -2)
    std::cout << " Error when Filling the histogram: No matching histogram found!" << std::endl;
  return ret;
}
...
int PDF::ReadPDF(std::string Filename) {
  ...
  // FIXED:SJA:removed variable array:    std::string CatNames[nCats];                               //  names of the
  // categories
  std::string* CatNames = new std::string[nCats];
  ...
  // FIXED:SJA:removed variable array:  std::string VarNames[NoOfVar];                             // array with names
  // of variables
  ...
  // FIXED:SJA:removed variable array:    int dims[NoOfHists];                                       // dimensions of
  // the histograms
  int* dims = new int[NoOfHists];
  ...
  // FIXED:SJA:removed variable array:    std::string HistNames[NoOfHists];                          // Names of the
  // histograms

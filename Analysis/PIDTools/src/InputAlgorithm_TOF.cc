...
InputAlgorithm_TOF::InputAlgorithm_TOF() : InputAlgorithm("TOF") { _description = "Returns TOF beta of PFO"; }

std::vector<std::string> InputAlgorithm_TOF::init(const std::vector<float>& inparF,
...
std::vector<std::pair<float, float>> InputAlgorithm_TOF::extractObservables(ReconstructedParticleImpl* pfo,
  ...
  if (int(TOFParams.size()) > TOFParaID_length) {
    ...
    float TOFmass2 =
        (isnan(TOFbeta_ch) || isinf(TOFbeta_ch)) ? 0 : momabs * momabs / TOFbeta_ch / TOFbeta_ch - momabs * momabs;

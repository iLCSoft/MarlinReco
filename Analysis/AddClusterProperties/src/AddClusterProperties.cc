...
AddClusterProperties::AddClusterProperties() : Processor("AddClusterProperties") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "PFOCollectionName", "Name of the input PFO collection",
                          _PFOName, std::string("PandoraPFOs"));
...
void AddClusterProperties::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void AddClusterProperties::processEvent(LCEvent* evt) {
  ...
  try {
    ...
    for (unsigned kkk = 0; kkk < shape_keys.size(); kkk++) {
      if (shape_keys[kkk] == "npoints") {
        npoints_index = kkk;
      }
      if (shape_keys[kkk] == "sum_wgt") {
        sum_wgt_index = kkk;
      }
      if (shape_keys[kkk] == "sum_wgt^2") {
        sum_wgt2_index = kkk;
      }
      if (shape_keys[kkk] == "sum_wgt^4") {
        sum_wgt4_index = kkk;
      }
    }
    ...
    for (unsigned kkk = 0; kkk < subDetectorNames.size(); kkk++) {
      if (subDetectorNames[kkk] == "ecal") {
        ecal_index = kkk;
      }
      if (subDetectorNames[kkk] == "hcal") {
        hcal_index = kkk;
      }
      if (subDetectorNames[kkk] == "yoke") {
        yoke_index = kkk;
      }
      if (subDetectorNames[kkk] == "lcal") {
        lcal_index = kkk;
      }
      if (subDetectorNames[kkk] == "lhcal") {
        lhcal_index = kkk;
      }
      if (subDetectorNames[kkk] == "bcal") {
        bcal_index = kkk;
      }
    }
  ...
  if (clucol != NULL) {
    ...
    for (int i = 0; i < nclu; i++) {
      ...
      if (np == 0) {
        streamlog_out(ERROR) << " Cluster with no hits. Input probably isnt a RECO-file, so just stop here ! "
                             << std::endl;
      ...
      for (int iii = 0; iii < 2; iii++) {
        for (int jjj = 0; jjj < 2; jjj++) {
          for (int kkk = 0; kkk < 3; kkk++) {
            evpe[iii][jjj][kkk] = evpev[kkk + jjj * 3 + iii * 3 * 2];
          }
        }
      };
      ...
      streamlog_out(DEBUG3) << "i PositionError : " << std::endl;
      streamlog_out(DEBUG3) << "i ";
      for (int iii = 0; iii < 6; iii++) {
        streamlog_out(DEBUG3) << PositionError[iii] << " ";
      };
      ...
      Position[1] = cog[1];
      Position[2] = cog[2];
      streamlog_out(DEBUG3) << "i Position : " << std::endl;
      streamlog_out(DEBUG3) << "i ";
      for (int iii = 0; iii < 3; iii++) {
        streamlog_out(DEBUG3) << Position[iii] << " ";
      };
      streamlog_out(DEBUG3) << std::endl;

      float theta = evp[0][2];
      float phi = evp[1][2];
      streamlog_out(DEBUG3) << "i theta/phi : " << std::endl;
      streamlog_out(DEBUG3) << "i ";
      ...
      streamlog_out(DEBUG3) << "i DirectionError : " << std::endl;
      streamlog_out(DEBUG3) << "i ";
      for (int iii = 0; iii < 3; iii++) {
        streamlog_out(DEBUG3) << DirectionError[iii] << " ";
      };
      streamlog_out(DEBUG3) << std::endl;
      ...
      if (Eerror == 0.0) {
        ...
        streamlog_out(DEBUG3) << " energies : " << E << " " << Eem << " " << Ehad << " " << Eerror << " "
                              << Eerror / sqrt(E) << std::endl;
      }

      for (int kkk = npoints_index; kkk <= sum_wgt4_index; kkk++) {
        if (shape_keys[kkk] != shape_keys[kkk]) {
          streamlog_out(WARNING) << " shape_keys " << kkk << " is NaN " << std::endl;
        }
      }
      for (int kkk = 0; kkk < 2; kkk++) {
        if (Position[kkk] != Position[kkk]) {
          streamlog_out(WARNING) << " Position " << kkk << " is NaN " << std::endl;
        }
      }
      for (int kkk = 0; kkk < 6; kkk++) {
        if (PositionError[kkk] != PositionError[kkk]) {
          streamlog_out(WARNING) << " PositionError " << kkk << " is NaN " << std::endl;
        }
      }
      for (int kkk = 0; kkk < 3; kkk++) {
        if (DirectionError[kkk] != DirectionError[kkk]) {
          streamlog_out(WARNING) << " DirectionError " << kkk << " is NaN " << std::endl;
        }
      }
      if (theta != theta) {
        streamlog_out(WARNING) << " theta is Nan " << std::endl;
      }
      if (phi != phi) {
        streamlog_out(WARNING) << " phi is Nan " << std::endl;
      }
      ...
      if (streamlog_level(DEBUG2)) {
        ...
        debuging(clucol, clu, cog, &cov[0][0], eval, eval_err, &evp[0][0], &evpe[0][0][0], &evc[0][0], np, sum_wgt,
                 sum_wgtsqr, sum_wgt4);
      }
    }
  }
  ...
  if (pfocol != NULL) {
    ...
    for (int i = 0; i < npfo; i++) {
      ...
      if (part->getCharge() == 0 && nclu == 1 && ntrk == 0) {
        ...
        double mom[4];
        for (int kkk = 0; kkk < 3; kkk++) {
          mom[kkk] = E * cogv[kkk] / dist;
        }
        mom[3] = E;
        streamlog_out(DEBUG3) << "i clu-mom: ";
        for (int kkk = 0; kkk < 4; kkk++) {
          streamlog_out(DEBUG3) << mom[kkk] << " ";
        }
        streamlog_out(DEBUG3) << std::endl;
        streamlog_out(DEBUG3) << "i pfo-mom: ";
        for (int kkk = 0; kkk < 4; kkk++) {
          streamlog_out(DEBUG3) << mompfo[kkk] << " ";
        }
        streamlog_out(DEBUG3) << std::endl;
        ...
        for (int kkk = 0; kkk < 4; kkk++) {
          for (int iii = 0; iii < 4; iii++) {
            streamlog_out(DEBUG3) << covmat[kkk][iii] << " ";
          }
          streamlog_out(DEBUG3) << std::endl;
        }
        ...
        for (int kkk = 0; kkk < 4; kkk++) {
          for (int iii = 0; iii < 4; iii++) {
            streamlog_out(DEBUG3) << dp_drE[kkk][iii] << " ";
          }
          streamlog_out(DEBUG3) << std::endl;
        }
        // propagate
        ...
        for (int kkk = 0; kkk < 4; kkk++) {
          for (int iii = 0; iii < 4; iii++) {
            streamlog_out(DEBUG3) << "i " << p_cov[kkk][iii] << " ";
          }
          streamlog_out(DEBUG3) << std::endl;
        }
        nnn = 0;
        ...
        for (int iii = 0; iii < 4; iii++) {
          for (int jjj = 0; jjj <= iii; jjj++) {
            ...
            if (p_cov_v[nnn] != p_cov_v[nnn]) {
              streamlog_out(WARNING) << " p_cov_v " << nnn << " is NaN " << p_cov_v[nnn] << std::endl;
            }
            nnn++;
          }
        }
...
void AddClusterProperties::debuging(LCCollection* clucol, ClusterImpl* clu, double* cog, double* cov, double* eval,
                                    double* eval_err, double* evp, double* evpe, double* evc, int np, double sum_wgt,
                                    double sum_wgtsqr, double sum_wgt4) {

  streamlog_out(DEBUG2) << " input: cog                    " << cog[0] << " " << cog[1] << " " << cog[2] << std::endl;
  streamlog_out(DEBUG2) << " input: covariance mat         " << cov[0 * 3 + 0] << " " << cov[0 * 3 + 1] << " "
                        << cov[0 * 3 + 2] << std::endl;
  streamlog_out(DEBUG2) << " input: covariance mat         " << cov[1 * 3 + 0] << " " << cov[1 * 3 + 1] << " "
                        << cov[1 * 3 + 2] << std::endl;
  streamlog_out(DEBUG2) << " input: covariance mat         " << cov[2 * 3 + 0] << " " << cov[2 * 3 + 1] << " "
                        << cov[2 * 3 + 2] << std::endl;
  streamlog_out(DEBUG2) << " input: eigenvals              " << eval[0] << " " << eval[1] << " " << eval[2]
                        << std::endl;
  streamlog_out(DEBUG2) << " input: V(eigenvals)           " << eval_err[0] << " " << eval_err[1] << " " << eval_err[2]
                        << std::endl;
  ...
  streamlog_out(DEBUG2) << " input: cov(eigen vec) 1 (p)   " << evpe[0 * 2 * 3 + 0 * 3 + 0] << " "
                        << evpe[1 * 2 * 3 + 1 * 3 + 0] << " " << evpe[1 * 2 * 3 + 0 * 3 + 0] << std::endl;
  streamlog_out(DEBUG2) << " input: cov(eigen vec) 2 (p)   " << evpe[0 * 2 * 3 + 0 * 3 + 1] << " "
                        << evpe[1 * 2 * 3 + 1 * 3 + 1] << " " << evpe[1 * 2 * 3 + 0 * 3 + 1] << std::endl;
  streamlog_out(DEBUG2) << " input: cov(eigen vec) 3 (p)   " << evpe[0 * 2 * 3 + 0 * 3 + 2] << " "
                        << evpe[1 * 2 * 3 + 1 * 3 + 2] << " " << evpe[1 * 2 * 3 + 0 * 3 + 2] << std::endl;
  streamlog_out(DEBUG2) << " input: eigen vec 1 (c)        " << evc[0 * 3 + 0] << " " << evc[1 * 3 + 0] << " "
                        << evc[1 * 3 + 0] << std::endl;
  streamlog_out(DEBUG2) << " input: eigen vec 2 (c)        " << evc[0 * 3 + 1] << " " << evc[1 * 3 + 1] << " "
                        << evc[1 * 3 + 1] << std::endl;
  streamlog_out(DEBUG2) << " input: eigen vec 3 (c)        " << evc[0 * 3 + 2] << " " << evc[1 * 3 + 2] << " "
                        << evc[1 * 3 + 2] << std::endl;
  streamlog_out(DEBUG2) << " input: E, np, sum, sum_sq, sum_4 " << clu->getEnergy() << " " << np << " " << sum_wgt
                        << " " << sum_wgtsqr << " " << sum_wgt4 << std::endl;

  // check reading back: get values now stored in clu
  const float* cogv = clu->getPosition();
  vector<double> cog_fr_clu;
  for (int iii = 0; iii < 3; iii++) {
    cog_fr_clu.push_back(cogv[iii]);
  }
  FloatVec PositionError = clu->getPositionError();
  StringVec shape_keys;
  shape_keys = clucol->getParameters().getStringVals(std::string("ClusterShapeParameters"), shape_keys);
  ...
  for (unsigned kkk = 0; kkk < shape_keys.size(); kkk++) {
    if (shape_keys[kkk] == "npoints") {
      npnt = int(wgts_sumv[kkk]);
    }
    if (shape_keys[kkk] == "sum_wgt") {
      wgt_sum = wgts_sumv[kkk];
    }
    if (shape_keys[kkk] == "sum_wgt^2") {
      wgt_sq_sum = wgts_sumv[kkk];
    }
    if (shape_keys[kkk] == "sum_wgt^4") {
      wgt_4_sum = wgts_sumv[kkk];
    }
  }
  ...
  for (int jjj = 0; jjj < 3; jjj++) {
    for (int iii = 0; iii < 3; iii++) {
      cov_fr_clu.push_back(seen_covmat[jjj][iii]);
    }
  }
  ...
  for (int iii = 0; iii < 3; iii++) {
    thphcov.push_back(DirectionError[iii]);
  }
  // a new WeightedPoints3D object, ctored with the cluster-shape params
  WeightedPoints3D wgtp_readback =
      WeightedPoints3D(cog_fr_clu, cov_fr_clu, thphcov, npnt, wgt_sum, wgt_sq_sum, wgt_4_sum);
  ...
  for (int iii = 0; iii < 3; iii++) {
    for (int jjj = 0; jjj < 3; jjj++) {
      evc_readback[iii][jjj] = evcv[jjj + iii * 3];
    }
  };
  ...
  for (int iii = 0; iii < 2; iii++) {
    for (int jjj = 0; jjj < 2; jjj++) {
      for (int kkk = 0; kkk < 3; kkk++) {
        evpe_readback[iii][jjj][kkk] = evpev[kkk + jjj * 3 + iii * 3 * 2];
      }
    }
  };
  streamlog_out(DEBUG2) << " read back: cog                    " << cog_readback[0] << " " << cog_readback[1] << " "
                        << cog_readback[2] << std::endl;
  streamlog_out(DEBUG2) << " read back: covariance mat         " << cov_readback[0][0] << " " << cov_readback[0][1]
                        << " " << cov_readback[0][2] << std::endl;
  streamlog_out(DEBUG2) << " read back: covariance mat         " << cov_readback[1][0] << " " << cov_readback[1][1]
                        << " " << cov_readback[1][2] << std::endl;
  streamlog_out(DEBUG2) << " read back: covariance mat         " << cov_readback[2][0] << " " << cov_readback[2][1]
                        << " " << cov_readback[2][2] << std::endl;
  streamlog_out(DEBUG2) << " read back: eigenvals              " << eval_readback[0] << " " << eval_readback[1] << " "
                        << eval_readback[2] << std::endl;
  streamlog_out(DEBUG2) << " read back: V(eigenvals)           " << eval_err_readback[0] << " " << eval_err_readback[1]
                        << " " << eval_err_readback[2] << std::endl;
  streamlog_out(DEBUG2) << " read back: eigen vec 1 (p)        " << evp_readback[0][0] << " " << evp_readback[1][0]
                        << std::endl;
  streamlog_out(DEBUG2) << " read back: eigen vec 2 (p)        " << evp_readback[0][1] << " " << evp_readback[1][1]
                        << std::endl;
  streamlog_out(DEBUG2) << " read back: eigen vec 3 (p)        " << evp_readback[0][2] << " " << evp_readback[1][2]
                        << std::endl;
  streamlog_out(DEBUG2) << " read back: cov(eigen vec) 1 (p)   " << evpe_readback[0][0][0] << " "
                        << evpe_readback[1][1][0] << " " << evpe_readback[1][0][0] << std::endl;
  streamlog_out(DEBUG2) << " read back: cov(eigen vec) 2 (p)   " << evpe_readback[0][0][1] << " "
                        << evpe_readback[1][1][1] << " " << evpe_readback[1][0][1] << std::endl;
  streamlog_out(DEBUG2) << " read back: cov(eigen vec) 3 (p)   " << evpe_readback[0][0][2] << " "
                        << evpe_readback[1][1][2] << " " << evpe_readback[1][0][2] << std::endl;
  streamlog_out(DEBUG2) << " read back: eigen vec 1 (c)        " << evc_readback[0][0] << " " << evc_readback[1][0]
                        << " " << evc_readback[1][0] << std::endl;
  streamlog_out(DEBUG2) << " read back: eigen vec 2 (c)        " << evc_readback[0][1] << " " << evc_readback[1][1]
                        << " " << evc_readback[1][1] << std::endl;
  streamlog_out(DEBUG2) << " read back: eigen vec 3 (c)        " << evc_readback[0][2] << " " << evc_readback[1][2]
                        << " " << evc_readback[1][2] << std::endl;
  streamlog_out(DEBUG2) << " read back: E, np, sum, sum_sq, sum_4 " << clu->getEnergy() << " " << np_readback << " "
                        << sum_wgt_readback << " " << sum_wgtsqr_readback << " " << sum_wgt4_readback << std::endl;
}

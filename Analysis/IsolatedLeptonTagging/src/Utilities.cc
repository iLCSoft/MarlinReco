...
Int_t getOriginalPDG(MCParticle* mcPart, Bool_t iHiggs) {
  ...
  while (nParents > 0) {
    ...
    if (!iHiggs) {
      if (abs(pdg) == 24 || abs(pdg) == 23 || abs(pdg) == 25)
        break;
    } else {
      if (abs(pdg) == 25)
        break;
...
Int_t getOriginalSerial(ReconstructedParticle* recPart, LCCollection* colMCTL, LCCollection* colMCP,
                        Bool_t /*iHiggs*/) {
...
Int_t getLeptonID(ReconstructedParticle* recPart) {
  ...
  else if (TMath::Abs(charge) > 0.5 && totalCalEnergy / momentumMagnitude < fMuonCut1 && yokeEnergy > fMuonCut3) {
    iLeptonType = 13;
  }
...
Bool_t getSplitTag(ReconstructedParticle* motherPart, ReconstructedParticle* recPart) {
  ...
  if (ti.size() == 1)
    sigpMom = trackMom * sqrt(ti[0]->getCovMatrix()[5]) / TMath::Abs(ti[0]->getOmega());
  if (ci.size() > 0) {
  ...
  if (dr < 30.0 && chiMom > 4.0 && TMath::Abs(chiNew) < chiMom)
    isSplit = true;
  if (dr < 40.0 && chiMom > 5.0 && TMath::Abs(chiNew) < chiMom)
    isSplit = true;
  if (dr < 50.0 && chiMom > 7.0 && TMath::Abs(chiNew) < chiMom)
    isSplit = true;
  // sanity check
  if (TMath::Abs(chiMom) < 2.0 && chiNew * chiNew > chiMom * chiMom + 5.0)
    isSplit = false;
  ...
  if (costheta < 0.99995 && eg / trackMom < 0.03)
    isSplit = true;
...
void getConeEnergy(ReconstructedParticle* recPart, LCCollection* colPFO, Double_t cosCone, Bool_t woFSR,
                   Double_t coneEnergy[3], Double_t pFSR[4]) {
  // get the cone energy of the particle
  //  woFSR = kTRUE;
...
void getConeEnergy(ReconstructedParticle* recPart, LCCollection* colPFO, Double_t cosCone, Bool_t woFSR,
                   Double_t coneEnergy[3], Double_t pFSR[4], Double_t cosCone2, Double_t pCone2[4],
                   Int_t& nConePhoton) {
  // get the cone energy of the particle
...
void getConeEnergy(ReconstructedParticle* recPart, LCCollection* colPFO, Double_t cosCone, Double_t coneEnergy[3],
                   Double_t cosCone2, Double_t pCone2[4]) {
  // get the cone energy of the particle
  // add another larger cone
  TLorentzVector lortzCon = TLorentzVector(0, 0, 0, 0);
  Int_t nPFOs = colPFO->getNumberOfElements();
...
Double_t getInvariantMass(ReconstructedParticle* recPart1, ReconstructedParticle* recPart2,
  ...
  Double_t invariantMass = 0.;
  Double_t invariantMass2 =
      (energy1 + energy2 + energy3) * (energy1 + energy2 + energy3) - (momentum1 + momentum2 + momentum3).Mag2();
  if (invariantMass2 > 0.) {
    invariantMass = sqrt(invariantMass2);
  } else {
    invariantMass = -sqrt(TMath::Abs(invariantMass2));
...
Int_t isSelectedByFastJet(ReconstructedParticle* pfo, LCCollection* colFastJet, Double_t& ratioEPartEJet,
  ...
  for (Int_t i = 0; i < nJets; i++) {
    ...
    for (std::vector<lcio::ReconstructedParticle*>::const_iterator iPart = partVec.begin(); iPart != partVec.end();
         ++iPart) {
      if ((*iPart) == pfo) {
        iFastJet = 1;
        iJet = i;
  ...
  if (iJet >= 0) {
    ReconstructedParticle* theJet =
        dynamic_cast<ReconstructedParticle*>(colFastJet->getElementAt(iJet)); // the jet where the pfo belongs
    ...
    Double_t mJet =
        eJet * eJet > pJet.Mag2() ? TMath::Sqrt(eJet * eJet - pJet.Mag2()) : TMath::Sqrt(-eJet * eJet + pJet.Mag2());
...
void doPhotonRecovery(ReconstructedParticle* electron, LCCollection* colPFO, ReconstructedParticleImpl* recoElectron,
                      Double_t fCosFSRCut) {
...
void doPhotonRecovery(ReconstructedParticle* electron, LCCollection* colPFO, ReconstructedParticleImpl* recoElectron,
                      Double_t fCosFSRCut, Int_t lepType, std::vector<lcio::ReconstructedParticle*>& photons) {
  // recover the BS and FSR photons
  TLorentzVector lortzElectron = TLorentzVector(electron->getMomentum(), electron->getEnergy());
  ...
  for (Int_t i = 0; i < nPFOs; i++) {
    ...
    if (recPart == electron)
      continue;
    if (isFoundInVector(recPart, photons))
      continue;
    Bool_t isFSR = getFSRTag(electron, recPart, fCosFSRCut);
    if (!isFSR)
      continue;
...
Int_t calculateEnergyComponents(LCCollection* colMC, LCCollection* colMCTL, ReconstructedParticle* jet,
...
Int_t calculateEnergyComponents(LCCollection* colMC, LCCollection* colMCTL, ReconstructedParticle* jet,
                                Double_t energyComponents[4], Int_t iColor1, Int_t iColor2, Int_t iColor3) {
  // calculate energies from different color singlets in a jet
  // [0]: from first Higgs; [1]: from second Higgs; [2]: from other color-singlet (mostly Z); [3]: from overlay
  // for qqH
...
Double_t getLikelihood(TString fname, TString hist, Double_t mass) {
  ...
  for (Int_t j = 0; j < nbin; j++) {
    ...
    Double_t density =
        1.0 * nj / ntot / TMath::Sqrt(2 * 3.1416) / hj * TMath::Exp(-(mass - tj) * (mass - tj) / 2 / hj / hj);
...
void listMCParticles(LCCollection* colMC) {
  ...
  std::cout << setw(6) << "Index" << setw(7) << "PDG" << setw(7) << "Mother" << setw(10) << "Charge" << setw(10)
            << "Mass" << setw(11) << "Energy" << setw(18) << "NumberOfDaughters" << setw(16) << "NumberOfParents"
  ...
  for (Int_t i = 0; i < nMCP; i++) {
    ...
    std::cout << setw(6) << i << setw(7) << pdg << setw(7) << motherpdg << setw(10) << setprecision(3) << charge
              << setw(10) << mass << setw(11) << energy << setw(18) << ndaughters << setw(16) << nparents << setw(10)
              << originalPDG << setw(8) << ioverlay << setw(8) << icreatedinsim << std::endl;
  }
}
...
void dumpJetParticles(ReconstructedParticle* jet, LCCollection* /*colMC*/, LCCollection* colMCTL) {
  ...
  cerr << "----Particles of the Jet: (E,Px,Py,Pz) = (" << lortz.E() << "," << lortz.Px() << "," << lortz.Py() << ","
       << lortz.Pz() << ")------" << endl;

  Int_t np = jet->getParticles().size();
  cerr << setw(6) << "Index" << setw(7) << "PDG" << setw(7) << "Mother" << setw(10) << "Charge" << setw(10) << "Mass"
       << setw(11) << "Energy" << setw(18) << "NumberOfDaughters" << setw(16) << "NumberOfParents" << setw(10)
       << "Original" << setw(8) << "Overlay" << setw(8) << "FromSim" << endl;
  for (Int_t i = 0; i < np; i++) {
    ReconstructedParticle* pfo = dynamic_cast<ReconstructedParticle*>(jet->getParticles()[i]);
    LCObjectVec vecMCTL = navMCTL->getRelatedToObjects(pfo);
    if (vecMCTL.size() > 0) {
	...
	if (ndaughters > 0) {
      ...
      cerr << setw(6) << i << setw(7) << pdg << setw(7) << motherpdg << setw(10) << setprecision(3) << charge
           << setw(10) << mass << setw(11) << energy << setw(18) << ndaughters << setw(16) << nparents << setw(10)
           << originalPDG << setw(8) << ioverlay << setw(8) << icreatedinsim << endl;
    }
  }
}
...
// *******************************************************
// *******************************************************
Int_t getVertexComponents(LCCollection* /*colMC*/, LCCollection* colMCTL, ReconstructedParticle* vertex,
                          Double_t energyComponents[2], Int_t nparticles[2]) {
...
std::vector<Int_t> getHiggsDecayModes(LCCollection* colMC) {
  ...
  for (Int_t i = 0; i < nMCP; i++) {
    ...
    if (pdg == 25 && abs(daughterpdg) == 5)
      nHbb++;
    if (pdg == 25 && abs(daughterpdg) == 24)
      nHWW++;
    if (pdg == 25 && abs(daughterpdg) == 21)
      nHgg++;
    if (pdg == 25 && abs(daughterpdg) == 23)
      nHZZ++;
    if (pdg == 25 && abs(daughterpdg) == 4)
      nHcc++;
    if (pdg == 25 && abs(daughterpdg) == 13)
      nHmumu++;
    if (pdg == 25 && abs(daughterpdg) == 15)
      nHtautau++;
    if (pdg == 25 && abs(daughterpdg) == 22)
      nHgamgam++;
    if (abs(pdg) == 24 && abs(motherpdg) == 25 && abs(daughterpdg) > 10 && abs(daughterpdg) < 20)
      nWlv++;
    if (abs(pdg) == 24 && abs(motherpdg) == 25 && abs(daughterpdg) < 10 && abs(daughterpdg) > 0)
      nWqq++;
    if (abs(pdg) == 23 && abs(motherpdg) == 25 &&
        (abs(daughterpdg) == 12 || abs(daughterpdg) == 14 || abs(daughterpdg) == 16))
      nZvv++;
    if (abs(pdg) == 23 && abs(motherpdg) == 25 && abs(daughterpdg) < 10 && abs(daughterpdg) > 0)
      nZqq++;
    if (abs(pdg) == 23 && abs(motherpdg) == 25 && abs(daughterpdg) == 11)
      nZee++;
    if (abs(pdg) == 23 && abs(motherpdg) == 25 && abs(daughterpdg) == 13)
      nZmm++;
  }
...
std::vector<Int_t> getNumberOfOverlayEvents(Double_t fEcm, LCCollection* colMC) {
  ...
  for (Int_t i = 0; i < nMCP; i++) {
    ...
    if (iovl == 1 && ibeam == 1 && pdg == 22)
      nBgam += 1;
    if (iovl == 1 && ibeam == 1 && pdg == 11)
      nBelectron += 1;
    if (iovl == 1 && ibeam == 1 && pdg == -11)
      nBpositron += 1;
  }
...
void mcDebug(LCCollection* colMC) {
  ...
  for (Int_t i = 0; i < nMCP; i++) {
    ...
    Int_t ioverlay = mcPart->isOverlay() ? 1 : 0;
    Int_t icreatedinsim = mcPart->isCreatedInSimulation() ? 1 : 0;
    cerr << setw(6) << i << setw(7) << pdg << setw(7) << motherpdg << setw(10) << setprecision(3) << charge << setw(10)
         << mass << setw(11) << energy << setw(18) << ndaughters << setw(16) << nparents << setw(10) << originalPDG
         << setw(8) << ioverlay << setw(8) << icreatedinsim << endl;
  }
}

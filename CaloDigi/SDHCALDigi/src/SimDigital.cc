...
SimDigital::SimDigital() : Processor("SimDigital"), chargeSpreaderParameters() {
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "inputHitCollections", "Sim Calorimeter Hit Collections",
                           _inputCollections, hcalCollections);

  std::vector<std::string> outputHcalCollections = {"HCALBarrelDigi", "HCALEndcapDigi", "HCALOtherDigi"};
  registerProcessorParameter("outputHitCollections", "output hit collection names", _outputCollections,
  ...
  registerProcessorParameter("HCALThreshold", "Threshold for HCAL Hits in pC", _thresholdHcal, hcalThresholds);

  registerProcessorParameter(
      "HCALCellSize", "Cell size (mm) of HCAL, if it is equal or less than zero then the value is taken from dd4hep",
      _cellSize, 0.0f);

  registerProcessorParameter("EffMapOption", "Step efficiency correction method : should be Uniform", efficiencyOption,
                             std::string("Uniform"));
  ...
  registerProcessorParameter("EffMapConstantValue",
                             ...
                             _constEffMapValue, 0.97f);

  registerProcessorParameter("EffMapFile", "Efficiency map file", effMapFile, std::string(""));

  // charge spreader parameters
  registerProcessorParameter("SpreaderMapFile", "Charge spreader map file", spreaderMapFile, std::string(""));

  registerProcessorParameter("functionRange",
                             "maximal distance (in mm) at which a step can induce charge using the 2D function defined "
                             "with functionFormula or when using ChargeSplitterOption==Erf",
                             chargeSpreaderParameters.range, 30.0f);
  ...
  registerProcessorParameter("RPC_PadSeparation",
                             ...
                             chargeSpreaderParameters.padSeparation, 0.0f);

  std::vector<float> erfWidth = {2};
  registerProcessorParameter("erfWidth", "Width values for the different Erf functions",
                             chargeSpreaderParameters.erfWidth, erfWidth);

  std::vector<float> erfWeigth = {1};
  registerProcessorParameter("erfWeigth", "Weigth for the different Erf functions", chargeSpreaderParameters.erfWeigth,
                             erfWeigth);

  registerProcessorParameter("ChargeSplitterd", "d parameter for exact splitter", chargeSpreaderParameters.d, 1.0f);

  registerProcessorParameter("ChargeSplitterOption", "Define the charge splitter method. Possible option : Erf , Exact",
                             chargeSpreaderOption, std::string("Erf"));

  registerProcessorParameter("CellIDEncodingStringType", "The type of the encoding, LCGEO or PROTO", _encodingType,
                             std::string("LCGEO"));
  ...
  registerProcessorParameter("PolyaOption", "Uniform polya or different polya per Asic", polyaOption,
                             std::string("Uniform"));

  registerProcessorParameter("PolyaMapFile", "Polya map file", polyaMapFile, std::string(""));

  registerProcessorParameter("PolyaRandomSeed", "The seed of the polya function", _polyaRandomSeed, 1);

  registerProcessorParameter("PolyaAverageCharge",
                             "Parameter for the Polya distribution used to simulate the induced charge distribution : "
                             "mean of the distribution",
                             polyaQbar, 1.6f);

  registerProcessorParameter("PolyaWidthParameter",
                             "Parameter for the Polya distribution used to simulate the induced charge distribution : "
                             "related to the distribution width ",
                             polyaTheta, 16.3f);

  registerProcessorParameter("GasGapWidth", "Width of the RPC gas gap", _gasGapWidth, 1.2f);

  registerProcessorParameter("LinkSteps", "Parameter for angle correction", _linkSteps, false);

  registerProcessorParameter("AngleCorrectionPower", "Parameter for angle correction", _angleCorrPow, 0.4f);

  registerProcessorParameter("TimeCut", "Time cut", timeCut, std::numeric_limits<double>::max());

  registerProcessorParameter("StepLengthCut", "Step length cut", stepLengthCut, -1.0);

  registerProcessorParameter("StepCellCenterMaxDistanceLayerDirection",
                             "Maximum distance (mm) between the Geant4 step position and the cell center, in the RPC "
                             "width direction, to keep a step for digitization",
                             _absZstepFilter, 0.0005f);
  ...
  registerProcessorParameter("StepsMinDistanceRPCplaneDirection",
                             ...
                             _minXYdistanceBetweenStep, 0.5f);

  registerProcessorParameter("KeepAtLeastOneStep",
                             "if true, ensure that each hit will keep at least one step for digitisation independatly "
                             "of filtering conditions (StepCellCenterMaxDistanceLayerDirection)",
...
void SimDigital::init() {
  ...
  // book tuples
  _debugTupleStepFilter = AIDAProcessor::tupleFactory(this)->create(
      "SimDigitalStepDebug", "SimDigital_StepDebug",
  ...
  _tupleStepFilter = AIDAProcessor::tupleFactory(this)->create("SimDigitalStepStat", "SimDigital_StepStat",
  ...
  _tupleCollection = AIDAProcessor::tupleFactory(this)->create("CollectionStat", "Collection_statistics",
  ...
  _histoCellCharge =
      AIDAProcessor::histogramFactory(this)->createHistogram1D("CellCharge", "CellCharge", 10000, 0, 100);
...
SimDigital::cellIDHitMap SimDigital::createPotentialOutputHits(LCCollection* col, SimDigitalGeomCellId* aGeomCellId) {
  ...
  for (int j = 0; j < numElements; ++j) {
    ...
    auto sortStepWithCharge = [](const StepAndCharge& s1, const StepAndCharge& s2) -> bool {
      return s1.charge > s2.charge;
    };
    ...
    for (const StepAndCharge& itstep : steps)
      chargeSpreader->addCharge(itstep.charge, static_cast<float>(itstep.step.x()), static_cast<float>(itstep.step.y()),
                                aGeomCellId);

...
public:
  ...
  float GetOffsetError(EVENT::ReconstructedParticle* particle, double* trackPosition, const EVENT::Vertex* ip,
                       double offset);

  double* GetStartPoint(const EVENT::ReconstructedParticle* particle);
  float GetDprime(const EVENT::ReconstructedParticle* particle1, const EVENT::ReconstructedParticle* particle2,
...
private:
  ...
  float getError(GConfig& conf, const std::vector<float> pcovMatrix, const std::vector<float> ccovMatrix,
                 const std::vector<float> ipcovMatrix);

...
NNClusterProcessor::NNClusterProcessor() : Processor("NNClusterProcessor") {
  ...
  registerInputCollections(LCIO::CALORIMETERHIT, "HitCollections", "Name of the input collections", _colNames,
                           colDefault);

  registerOutputCollection(LCIO::CLUSTER, "OutputCollection", "Name of the output collections", _outputColName,
                           std::string("NNClusters"));

  registerProcessorParameter("DistanceCut", "Cut for distance between hits in mm", _distCut, (float)40.0);

  registerProcessorParameter("EnergyCut", "Cut for hit energy in GeV", _eCut, (float)0.0);
}
...
void NNClusterProcessor::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }

void NNClusterProcessor::processEvent(LCEvent* evt) {

  std::cout << " ---- NNClusterProcessor::processEvent() - evt: " << evt->getRunNumber() << " , "
  ...
  std::cout << "  passing " << h.size() << " of " << nHit << "  hits to clustering (E_cut: " << _eCut << ") "
  ...
  std::cout << " ---- NNClusterProcessor::processEvent() - time: " << double(end - start) / double(CLOCKS_PER_SEC)
            << std::endl;
}

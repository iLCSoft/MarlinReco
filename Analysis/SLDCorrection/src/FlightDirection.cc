...
void getTrueFlightDirection(const MCP& SLDLepton, TVector3& trueFlightDirection, DoubleVector& trueStartVertex,
                            DoubleVector& trueSLDVertex) {
  const MCP motherHadron = SLDLepton->getParents()[0];
  trueFlightDirection = TVector3(motherHadron->getMomentumAtEndpoint()[0], motherHadron->getMomentumAtEndpoint()[1],
                                 motherHadron->getMomentumAtEndpoint()[2]);
  trueFlightDirection.SetMag(1.0);
  streamlog_out(DEBUG1) << "	True Flight Direction ( nx , ny , nz ):	(	" << trueFlightDirection.X()
                        << "	,	" << trueFlightDirection.Y() << "	,	" << trueFlightDirection.Z()
                        << "	)" << std::endl;
  ...
  streamlog_out(DEBUG1) << "	True Start Vertex ( x , y , z ):	(	" << trueStartVertex[0]
                        << "	,	" << trueStartVertex[1] << "	,	" << trueStartVertex[2] << "	)"
                        << std::endl;
  trueSLDVertex.clear();
...
int getRecoFlightDirection(const RecoParticle& linkedRecoLepton, TVector3& recoFlightDirection,
                           double& hadronFlightLength, const Vtx& primaryVertex, const Vtx& startVertex, Vtx& SLDVertex,
                           RecoParticle& SLDVertexRP, const RecoParticle& assignedJet,
                           const VertexVector& verticesInJet, const PFOVector& PFOswithAloneTracks,
                           float& helicesDistance, int vertexingScenario, TVector3& daughterHadronFlightDirection,
                           double& daughterHadronFlightDistance, DoubleVector& sldVertexPosition, TVector3& PCAatLepton,
                           TVector3& PCAatOtherParticle, double& lepton3DImpactParameter,
                           double& OtherParticle3DImpactParameter) {
  int SLDStatus = -999;
  bool recoLeptonIsInVertex = false;
  TVector3 startVertexPosition(startVertex->getPosition()[0], startVertex->getPosition()[1],
                               startVertex->getPosition()[2]);
  Vtx recoLeptonVertex = getParticleVertex(linkedRecoLepton, verticesInJet, recoLeptonIsInVertex);
  ...
  if (recoLeptonIsInVertex) {
    ...
    streamlog_out(DEBUG1) << "	(" << SLDStatus
                          << ") Lepton from semi-leptonic decay found in a BuildUp Vertex, BuildUp Vertex is used as "
                             "vertex of semi-leptonic decay!"
                          << std::endl;
    SLDVertex = recoLeptonVertex;
    SLDVertexRP = recoLeptonVertex->getAssociatedParticle();
    recoFlightDirection = TVector3(recoLeptonVertex->getPosition()[0] - startVertex->getPosition()[0],
                                   recoLeptonVertex->getPosition()[1] - startVertex->getPosition()[1],
                                   recoLeptonVertex->getPosition()[2] - startVertex->getPosition()[2]);
    hadronFlightLength = recoFlightDirection.Mag();
    recoFlightDirection.SetMag(1.0);
    RecoParticle recoLeptonVertexRP = recoLeptonVertex->getAssociatedParticle();
    for (unsigned int i_par = 0; i_par < recoLeptonVertexRP->getParticles().size(); ++i_par) {
      if (recoLeptonVertexRP->getParticles()[i_par] != linkedRecoLepton)
        daughterHadronFlightDirection += TVector3((recoLeptonVertexRP->getParticles()[i_par])->getMomentum());
    }
    ...
    OtherParticle3DImpactParameter =
        get3DImpactParameter(daughterHadronFlightDirection, sldVertexPosition, primaryVertex, PCAatLineInIP);
    streamlog_out(DEBUG1) << "	3D-Impact parameter of Lepton at Interaction Point = " << lepton3DImpactParameter
                          << std::endl;
    streamlog_out(DEBUG1) << "	3D-Impact parameter of direction of secondary Vertex at Interaction Point = "
                          << OtherParticle3DImpactParameter << std::endl;
    daughterHadronFlightDirection.SetMag(1.0);
  } else if (verticesInJet.size() != 0) {
    streamlog_out(DEBUG1)
        << "	Lepton from semi-leptonic decay not found in a BuildUp Vertex, Investigating BuildUp vertices in jet"
        << std::endl;
    Vtx thirdVertex = NULL;
    // float minDistanceToPrimaryVertex = 1000000.0;
    TVector3 jetAxis(assignedJet->getMomentum());
    jetAxis.SetMag(1.0);
    ...
    for (unsigned int i_vtx = 0; i_vtx < verticesInJet.size(); ++i_vtx) {
      ...
      Trk leptonTrack = linkedRecoLepton->getTracks()[0];
      TVector3 momentumOfLine((thirdVertex->getAssociatedParticle())->getMomentum());
      TVector3 primaryVertexPosition(primaryVertex->getPosition()[0], primaryVertex->getPosition()[1],
                                     primaryVertex->getPosition()[2]);
      TVector3 tertiaryVertexPosition(thirdVertex->getPosition()[0], thirdVertex->getPosition()[1],
                                      thirdVertex->getPosition()[2]);
      DoubleVector pointOnLine{};
      pointOnLine.clear();
      ...
      helicesDistance =
          intersectTrackLine(leptonTrack, primaryVertex, momentumOfLine, pointOnLine, PCAatLepton, PCAatOtherParticle);
      TVector3 testRecoFlightDirection = PCAatLepton - startVertexPosition;
      testRecoFlightDirection.SetMag(1.0);
      if (testRecoFlightDirection.Dot(jetAxis) > cosAngleFlightDirectionwJetAxis) {
        ...
        hadronFlightLength = recoFlightDirection.Mag();
        daughterHadronFlightDirection =
            TVector3(thirdVertex->getPosition()[0] - PCAatLepton.X(), thirdVertex->getPosition()[1] - PCAatLepton.Y(),
                     thirdVertex->getPosition()[2] - PCAatLepton.Z());
        daughterHadronFlightDistance = daughterHadronFlightDirection.Mag();
        daughterHadronFlightDirection.SetMag(1.0);
        ...
        TVector3 PCAatLineInIP(0.0, 0.0, 0.0);
        OtherParticle3DImpactParameter =
            get3DImpactParameter(momentumOfLine, pointOnLine, primaryVertex, PCAatLineInIP);

        streamlog_out(DEBUG1) << "	(" << SLDStatus << ") There is One BuildUp Vertex in jet" << std::endl;
        streamlog_out(DEBUG1) << "		Intersection point of Lepton and other BuildUp Vertices in jet is used "
                                 "as vertex of semi-leptonic decay!"
                              << std::endl;
        SLDStatus = 5;
        streamlog_out(DEBUG1) << "	3D-Impact parameter of Lepton at Interaction Point = "
                              << lepton3DImpactParameter << std::endl;
        streamlog_out(DEBUG1) << "	3D-Impact parameter of direction of secondary Vertex at Interaction Point = "
                              << OtherParticle3DImpactParameter << std::endl;
      }
      // float distanceToPrimaryVertex = std::sqrt( pow( testVertex->getPosition()[ 0 ] - startVertex->getPosition()[ 0
      // ] , 2 ) + pow( testVertex->getPosition()[ 1 ] - startVertex->getPosition()[ 1 ] , 2 ) + pow(
      // testVertex->getPosition()[ 2 ] - startVertex->getPosition()[ 2 ] , 2 ) ); if ( distanceToPrimaryVertex <
      // minDistanceToPrimaryVertex )
      //{
    ...
    DoubleVector chargedParticlePosition(3, 0.0);
    TVector3 chargedParticleMomentum(0.0, 0.0, 0.0);
    RecoParticle leadingChargedParticle = getLeadingChargedParticle(assignedJet);
    ...
    DoubleVector neutralParticlePosition(3, 0.0);
    TVector3 neutralParticleMomentum(0.0, 0.0, 0.0);
    RecoParticle leadingNeutralParticle = getLeadingNeutralParticle(assignedJet);
    ...
    if (vertexingScenario == 2) {
      if (leadingChargedParticle != NULL) {
        ...
        if (leadingChargedParticle->getTracks().size() == 1) {
          helicesDistance =
              intersectTrackTrack(linkedRecoLepton->getTracks()[0], leadingChargedParticle->getTracks()[0], PCAatLepton,
                                  PCAatOtherParticle);
          TVector3 PCAatTrackInIP(0.0, 0.0, 0.0);
          lepton3DImpactParameter =
              get3DImpactParameter(linkedRecoLepton->getTracks()[0], primaryVertex, PCAatTrackInIP);
          TVector3 PCAatLineInIP(0.0, 0.0, 0.0);
          OtherParticle3DImpactParameter =
              get3DImpactParameter(leadingChargedParticle->getTracks()[0], primaryVertex, PCAatLineInIP);
          streamlog_out(DEBUG1) << "	3D-Impact parameter of Lepton at Interaction Point = "
                                << lepton3DImpactParameter << std::endl;
          streamlog_out(DEBUG1) << "	3D-Impact parameter of direction of secondary Vertex at Interaction Point = "
                                << OtherParticle3DImpactParameter << std::endl;
        } else {
          helicesDistance = intersectTrackLine(linkedRecoLepton->getTracks()[0], primaryVertex, chargedParticleMomentum,
                                               chargedParticlePosition, PCAatLepton, PCAatOtherParticle);
          TVector3 PCAatTrackInIP(0.0, 0.0, 0.0);
          lepton3DImpactParameter =
              get3DImpactParameter(linkedRecoLepton->getTracks()[0], primaryVertex, PCAatTrackInIP);
          TVector3 PCAatLineInIP(0.0, 0.0, 0.0);
          OtherParticle3DImpactParameter =
              get3DImpactParameter(chargedParticleMomentum, chargedParticlePosition, primaryVertex, PCAatLineInIP);
          streamlog_out(DEBUG1) << "	3D-Impact parameter of Lepton at Interaction Point = "
                                << lepton3DImpactParameter << std::endl;
          streamlog_out(DEBUG1) << "	3D-Impact parameter of direction of secondary Vertex at Interaction Point = "
                                << OtherParticle3DImpactParameter << std::endl;
      ...
      if (leadingNeutralParticle != NULL) {
        SLDStatus = 7;
        helicesDistance = intersectTrackLine(linkedRecoLepton->getTracks()[0], primaryVertex, neutralParticleMomentum,
                                             neutralParticlePosition, PCAatLepton, PCAatOtherParticle);
        recoFlightDirection = PCAatLepton - startVertexPosition;
        hadronFlightLength = recoFlightDirection.Mag();
        sldVertexPosition.push_back(PCAatLepton.X());
        sldVertexPosition.push_back(PCAatLepton.Y());
        ...
        TVector3 PCAatLineInIP(0.0, 0.0, 0.0);
        OtherParticle3DImpactParameter =
            get3DImpactParameter(neutralParticleMomentum, neutralParticlePosition, primaryVertex, PCAatLineInIP);
        streamlog_out(DEBUG1) << "	3D-Impact parameter of Lepton at Interaction Point = "
                              << lepton3DImpactParameter << std::endl;
        streamlog_out(DEBUG1) << "	3D-Impact parameter of direction of secondary Vertex at Interaction Point = "
                              << OtherParticle3DImpactParameter << std::endl;
      ...
      for (unsigned int i_par = 0; i_par < PFOswithAloneTracks.size(); ++i_par) {
        ...
        RecoParticle alonePFO = PFOswithAloneTracks[i_par];
        TVector3 alonePFOMomentum(alonePFO->getMomentum());
        DoubleVector alonePFOPosition{};
        ...
        if (alonePFO->getTracks().size() == 1) {
          temphelicesDistance = intersectTrackTrack(linkedRecoLepton->getTracks()[0], alonePFO->getTracks()[0],
                                                    tempPCAatLepton, tempPCAatOtherParticle);
          TVector3 PCAatTrackInIP(0.0, 0.0, 0.0);
          lepton3DImpactParameter =
              get3DImpactParameter(linkedRecoLepton->getTracks()[0], primaryVertex, PCAatTrackInIP);
          TVector3 PCAatLineInIP(0.0, 0.0, 0.0);
          ...
          temphelicesDistance = intersectTrackLine(linkedRecoLepton->getTracks()[0], primaryVertex, alonePFOMomentum,
                                                   alonePFOPosition, tempPCAatLepton, tempPCAatOtherParticle);
          TVector3 PCAatTrackInIP(0.0, 0.0, 0.0);
          lepton3DImpactParameter =
              get3DImpactParameter(linkedRecoLepton->getTracks()[0], primaryVertex, PCAatTrackInIP);
          TVector3 PCAatLineInIP(0.0, 0.0, 0.0);
          OtherParticle3DImpactParameter =
              get3DImpactParameter(alonePFOMomentum, alonePFOPosition, primaryVertex, PCAatLineInIP);
          streamlog_out(DEBUG1) << "	3D-Impact parameter of Lepton at Interaction Point = "
                                << lepton3DImpactParameter << std::endl;
          streamlog_out(DEBUG1) << "	3D-Impact parameter of direction of secondary Vertex at Interaction Point = "
                                << OtherParticle3DImpactParameter << std::endl;
...
double intersectTrackLine(const Trk& track, const Vtx& primaryVertex, const TVector3& momentumOfLine,
                          const DoubleVector& pointOnLine, TVector3& PCAatTrack, TVector3& PCAatLine) {
  ...
  streamlog_out(DEBUG1) << "	Center of Track ( Xc , Yc , Zc ) :	(	" << xCenter << "	,	"
                        << yCenter << "	,	" << zCenter << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	StartPoint of Track ( Xc , Yc , Zc ) :	(	" << trackXs << "	,	"
                        << trackYs << "	,	" << trackZs << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	Momentum of Track ( Px , Py , Pz ) :	(	" << trackPx << "	,	"
                        << trackPy << "	,	" << trackPz << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	Track Reference Point (x,y,z) :		(	" << xReference << "	,	"
                        << yReference << "	,	" << zReference << "	)" << std::endl;

  ////////////////////////////////////////////////////////////////////////////////
  //	double X = helixFreeParameter;
  //
  //	double xTrack		= xReference + ( 1.0 / trackOmega - trackD0 ) * std::sin( trackPhi ) - std::sin( X ) /
  // trackOmega; 	double yTrack		= yReference - ( 1.0 / trackOmega - trackD0 ) * std::cos( trackPhi ) +
  // std::cos( X ) / trackOmega; 	double zTrack		= zReference + trackZ0 - ( X - trackPhi ) *
  // trackTanLambda / std::fabs( trackOmega );
  ////////////////////////////////////////////////////////////////////////////////

  // double vMin = -1.0 * ( momentumOfLine.Px() * ( pointOnLine[ 0 ] - primaryVertex->getPosition()[ 0 ] ) +
  // momentumOfLine.Py() * ( pointOnLine[ 1 ] - primaryVertex->getPosition()[ 1 ] ) + momentumOfLine.Pz() * (
  // pointOnLine[ 2 ] - primaryVertex->getPosition()[ 2 ] ) ) / momentumOfLine.Mag2();
  double vMin = -1.0 * ((pointOnLine[0] - primaryVertex->getPosition()[0]) / momentumOfLine.X() +
                        (pointOnLine[1] - primaryVertex->getPosition()[1]) / momentumOfLine.Y() +
                        (pointOnLine[2] - primaryVertex->getPosition()[2]) / momentumOfLine.Z());
  double vMax = 0.0;

  streamlog_out(DEBUG1) << "	DownStreamVertex Position:  (	" << pointOnLine[0] << "	,	"
                        << pointOnLine[1] << "	,	" << pointOnLine[2] << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	DownStreamVertex Momentum:  (	" << momentumOfLine.Px() << "	,	"
                        << momentumOfLine.Py() << "	,	" << momentumOfLine.Pz() << "	)" << std::endl;
  ...
  DDMarlinCED::drawHelix(m_Bfield, trackcharge, trackXs, trackYs, trackZs, trackPx, trackPy, trackPz, 2, 1, 0xff0000,
                         0.0, 1500.0, 2000.0, 0);
  double t = 20.0;
  ...
  {
          double trackx = xReference + ( 1.0 / trackOmega - trackD0 ) * std::sin( trackPhi ) - std::sin( phi ) /
  trackOmega; double tracky = yReference - ( 1.0 / trackOmega - trackD0 ) * std::cos( trackPhi ) + std::cos( phi ) /
  trackOmega; double trackz = zReference + trackZ0 - ( phi - trackPhi ) * trackTanLambda / trackOmega; ced_hit( trackx ,
  tracky , trackz , 2 , 3 , 0xf44336 );
  }
  ...
  */
  ...
  TF2* distance = new TF2("distance",
                          "std::sqrt( std::pow( ( [ 0 ] + [ 1 ] * std::sin( x ) ) - ( [ 7 ] + [ 8 ] * y ) , 2 ) + "
                          "std::pow( ( [ 2 ] + [ 3 ] * std::cos( x ) ) - ( [ 9 ] + [ 10 ] * y ) , 2 ) + std::pow( ( [ "
                          "4 ] + [ 5 ] * ( x - [ 6 ] ) ) - ( [ 11 ] + [ 12 ] * y ) , 2 ) )",
                          xMin, xMax, yMin, yMax);

  distance->SetParameter(0, xReference + (1.0 / trackOmega - trackD0) * std::sin(trackPhi));
  streamlog_out(DEBUG1) << "	parameter [ 0 ] =  " << xReference + (1.0 / trackOmega - trackD0) * std::sin(trackPhi)
                        << std::endl;
  ...
  streamlog_out(DEBUG1) << "	parameter [ 2 ] =  " << yReference - (1.0 / trackOmega - trackD0) * std::cos(trackPhi)
                        << std::endl;
  ...
  streamlog_out(DEBUG1) << "	DS PCA (x,y,z) = 	( " << xLine << "	,	" << yLine << "	,	"
                        << zLine << "	)" << std::endl;
...
double intersectTrackTrack(const Trk& track1, const Trk& track2, TVector3& PCAatTrack1, TVector3& PCAatTrack2) {
  ...
  streamlog_out(DEBUG1) << "	Center of Track1 ( Xc , Yc , Zc ) :	(	" << xCenter1 << "	,	"
                        << yCenter1 << "	,	" << zCenter1 << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	StartPoint of Track1 ( Xc , Yc , Zc ) :	(	" << track1Xs << "	,	"
                        << track1Ys << "	,	" << track1Zs << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	Momentum of Track1 ( Px , Py , Pz ) :	(	" << track1Px << "	,	"
                        << track1Py << "	,	" << track1Pz << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	Track1 Reference Point (x,y,z) :		(	" << xReference1
                        << "	,	" << yReference1 << "	,	" << zReference1 << "	)" << std::endl;
  ...
  streamlog_out(DEBUG1) << "	Center of Track2 ( Xc , Yc , Zc ) :	(	" << xCenter2 << "	,	"
                        << yCenter2 << "	,	" << zCenter2 << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	StartPoint of Track2 ( Xc , Yc , Zc ) :	(	" << track2Xs << "	,	"
                        << track2Ys << "	,	" << track2Zs << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	Momentum of Track2 ( Px , Py , Pz ) :	(	" << track2Px << "	,	"
                        << track2Py << "	,	" << track2Pz << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	Track2 Reference Point (x,y,z) :		(	" << xReference1
                        << "	,	" << yReference1 << "	,	" << zReference1 << "	)" << std::endl;
  streamlog_out(DEBUG1) << "" << std::endl;
  streamlog_out(DEBUG1) << "-----------------------------------------------------------------------------" << std::endl;
  ...
  DDMarlinCED::drawHelix(m_Bfield, track1charge, track1Xs, track1Ys, track1Zs, track1Px, track1Py, track1Pz, 2, 1,
                         0xff0000, 0.0, 1500.0, 2000.0, 0);
  DDMarlinCED::drawHelix(m_Bfield, track2charge, track2Xs, track2Ys, track2Zs, track2Px, track2Py, track2Pz, 2, 1,
                         0x0061ff, 0.0, 1500.0, 2000.0, 0);
  ...
  // dist = std::sqrt( std::pow( ( [ 0 ] + [ 1 ] * std::sin( x ) ) - ( [ 7 ] + [ 8 ] * std::sin( x ) ) , 2 ) + std::pow(
  // ( [ 2 ] + [ 3 ] * std::cos( x ) ) - ( [ 9 ] + [ 10 ] * std::cos( x ) ) , 2 ) + std::pow( ( [ 4 ] + [ 5 ] * x ) - (
  // [ 11 ] + [ 12 ] * y ) , 2 ) )

  TF2* distance = new TF2("distance",
                          "std::sqrt( std::pow( ( [ 0 ] + [ 1 ] * std::sin( x ) ) - ( [ 6 ] + [ 7 ] * std::sin( y ) ) "
                          ", 2 ) + std::pow( ( [ 2 ] + [ 3 ] * std::cos( x ) ) - ( [ 8 ] + [ 9 ] * std::cos( y ) ) , 2 "
                          ") + std::pow( ( [ 4 ] + [ 5 ] * x ) - ( [ 10 ] + [ 11 ] * y ) , 2 ) )",
                          xMin, xMax, yMin, yMax);

  distance->SetParameter(0, xReference1 + (1.0 / track1Omega - track1D0) * std::sin(track1Phi));
  streamlog_out(DEBUG1) << "	parameter [ 0 ] =  "
                        << xReference1 + (1.0 / track1Omega - track1D0) * std::sin(track1Phi) << std::endl;
  ...
  streamlog_out(DEBUG1) << "	parameter [ 2 ] =  "
                        << yReference1 - (1.0 / track1Omega - track1D0) * std::cos(track1Phi) << std::endl;
  ...
  streamlog_out(DEBUG1) << "	parameter [ 4 ] =  "
                        << zReference1 + track1Z0 + track1Phi * track1TanLambda / track1Omega;
  ...
  streamlog_out(DEBUG1) << "	parameter [ 6 ] =  "
                        << xReference2 + (1.0 / track2Omega - track2D0) * std::sin(track2Phi) << std::endl;
  ...
  streamlog_out(DEBUG1) << "	parameter [ 8 ] =  "
                        << yReference1 - (1.0 / track2Omega - track2D0) * std::cos(track2Phi) << std::endl;
  ...
  streamlog_out(DEBUG1) << "	parameter [ 10 ] =  "
                        << zReference2 + track2Z0 + track2Phi * track2TanLambda / track2Omega;
  ...
  double xTrack1PCA =
      xReference1 + (1.0 / track1Omega - track1D0) * std::sin(track1Phi) - std::sin(minPhi1) / track1Omega;
  double yTrack1PCA =
      yReference1 - (1.0 / track1Omega - track1D0) * std::cos(track1Phi) + std::cos(minPhi1) / track1Omega;
  double zTrack1PCA = zReference1 + track1Z0 - (minPhi1 - track1Phi) * track1TanLambda / track1Omega;
  streamlog_out(DEBUG1) << "	Track 1 PCA (x,y,z) = 	( " << xTrack1PCA << "	,	" << yTrack1PCA
                        << "	,	" << zTrack1PCA << "	)" << std::endl;
  ced_hit(xTrack1PCA, yTrack1PCA, zTrack1PCA, 5, 5, 0xff0000);
  PCAatTrack1 = TVector3(xTrack1PCA, yTrack1PCA, zTrack1PCA);

  double xTrack2PCA =
      xReference2 + (1.0 / track2Omega - track2D0) * std::sin(track2Phi) - std::sin(minPhi2) / track2Omega;
  double yTrack2PCA =
      yReference2 - (1.0 / track2Omega - track2D0) * std::cos(track2Phi) + std::cos(minPhi2) / track2Omega;
  double zTrack2PCA = zReference2 + track2Z0 - (minPhi2 - track2Phi) * track2TanLambda / track2Omega;
  streamlog_out(DEBUG1) << "	Track 2 PCA (x,y,z) = 	( " << xTrack2PCA << "	,	" << yTrack2PCA
                        << "	,	" << zTrack2PCA << "	)" << std::endl;
...
double get3DImpactParameter(const Trk& track, const Vtx& primaryVertex, TVector3& PCAatTrack) {
  streamlog_out(DEBUG1) << "<<<<<<<<<<<<<<<<<<<<   Get 3D Impact Parameter of Track   >>>>>>>>>>>>>>>>>>>>"
                        << std::endl;
  streamlog_out(DEBUG1) << "	TRACK:" << std::endl;
  streamlog_out(DEBUG1) << *track << std::endl;
  double trackD0 = track->getD0();
  ...
  trackOmega; double tracky = yReference - ( 1.0 / trackOmega - trackD0 ) * std::cos( trackPhi ) + std::cos( phi ) /
  trackOmega; double trackz = zReference + trackZ0 - ( phi - trackPhi ) * trackTanLambda / trackOmega; ced_hit( trackx ,
  tracky , trackz , 1 , 1 , 0xf44336 );
  }
  */
  TF1* distance =
      new TF1("distance",
              "std::sqrt( std::pow( ( [ 0 ] + [ 1 ] * std::sin( x ) ) - [ 7 ] , 2 ) + std::pow( ( [ 2 ] + [ 3 ] * "
              "std::cos( x ) ) - [ 8 ] , 2 ) + std::pow( ( [ 4 ] + [ 5 ] * ( x - [ 6 ] ) ) - [ 9 ] , 2 ) )",
              xMin, xMax);
  ...
  streamlog_out(DEBUG1) << "	Distance of Track and Primary Vertex (3D Impact Parameter) = " << minDistance
                        << std::endl;
  return minDistance;
}

double get3DImpactParameter(const TVector3& momentumOfLine, const DoubleVector& pointOnLine, const Vtx& primaryVertex,
                            TVector3& PCAatLine) {
  ...
  streamlog_out(DEBUG1) << "	Position of Reference Point on Line:  (	" << pointOnLine[0] << "	,	"
                        << pointOnLine[1] << "	,	" << pointOnLine[2] << "	)" << std::endl;
  streamlog_out(DEBUG1) << "	Direction of Line :  (	" << momentumOfLine.Px() << "	,	" << momentumOfLine.Py()
                        << "	,	" << momentumOfLine.Pz() << "	)" << std::endl;
  ...
  */
  ...
  TF1* distance = new TF1("distance",
                          "std::sqrt( std::pow( ( [ 0 ] + [ 1 ] * x ) - [ 6 ]  , 2 ) + std::pow( ( [ 2 ] + [ 3 ] * x ) "
                          "- [ 7 ]  , 2 ) + std::pow( ( [ 4 ] + [ 5 ] * x ) - [ 8 ]  , 2 ) )",
                          yMin, yMax);
...
void drawMCParticles(const MCP& motherHadron, const MCP& mcParticle) {
  ...
  if (motherHadron == mcParticle) {
    ced_line(motherHadron->getEndpoint()[0], motherHadron->getEndpoint()[1], motherHadron->getEndpoint()[2],
             motherHadron->getVertex()[0], motherHadron->getVertex()[1], motherHadron->getVertex()[2], 1, 3,
             0x7b00ff); // DRAW MOTHER HADRON IN PURPLE
  } else {
    ced_line(mcParticle->getEndpoint()[0], mcParticle->getEndpoint()[1], mcParticle->getEndpoint()[2],
             mcParticle->getVertex()[0], mcParticle->getVertex()[1], mcParticle->getVertex()[2], 1, 1,
             0x00E0FF); // DRAW UNSTABLE PARTICLES IN CYAN
  }
  ...
  for (unsigned int i_daughter = 0; i_daughter < mcParticle->getDaughters().size(); ++i_daughter) {
    const MCP duaughter = mcParticle->getDaughters()[i_daughter];
    if (duaughter->getGeneratorStatus() == 1) {
      if (std::fabs(duaughter->getPDG()) == 12 || std::fabs(duaughter->getPDG()) == 14 ||
          std::fabs(duaughter->getPDG()) == 16) // DRAW Neutrinos in GRAY
      {
        ced_line(duaughter->getEndpoint()[0], duaughter->getEndpoint()[1], duaughter->getEndpoint()[2],
                 duaughter->getVertex()[0], duaughter->getVertex()[1], duaughter->getVertex()[2], 2, 2, 0x949494);
        streamlog_out(DEBUG0) << " True Neutrino: " << std::endl;
        streamlog_out(DEBUG0) << *duaughter << std::endl;
      } else if (std::fabs(duaughter->getPDG()) == 11 || std::fabs(duaughter->getPDG()) == 13 ||
                 std::fabs(duaughter->getPDG()) == 15) // DRAW Leptons
      {
        for (unsigned int i_td = 0; i_td < motherHadron->getDaughters().size(); ++i_td) {
          MCP testDuaughter = motherHadron->getDaughters()[i_td];
          if (testDuaughter == duaughter) // DRAW Leptons from SLD in RED
          {
            DDMarlinCED::drawHelix(m_Bfield, +1.0, duaughter->getVertex()[0], duaughter->getVertex()[1],
                                   duaughter->getVertex()[2], 100.0 * duaughter->getMomentum()[0],
                                   100.0 * duaughter->getMomentum()[1], 100.0 * duaughter->getMomentum()[2], 1, 1,
                                   0xfe1100, 0.0, 2100.0, 3000.0, 0);
      ...
      {
        ced_line(duaughter->getEndpoint()[0], duaughter->getEndpoint()[1], duaughter->getEndpoint()[2],
                 duaughter->getVertex()[0], duaughter->getVertex()[1], duaughter->getVertex()[2], 1, 1, 0x01be4b);
        streamlog_out(DEBUG0) << " True Neutral Particle: " << std::endl;
        streamlog_out(DEBUG0) << *duaughter << std::endl;
      } else // DRAW Charged Particles in BLUE
      {
        DDMarlinCED::drawHelix(m_Bfield, duaughter->getCharge(), duaughter->getVertex()[0], duaughter->getVertex()[1],
                               duaughter->getVertex()[2], duaughter->getMomentum()[0], duaughter->getMomentum()[1],
                               duaughter->getMomentum()[2], 1, 1, 0x5a6ffa, 0.0, 2100.0, 3000.0, 0);
    ...
    } else // DRAW Unstable Particles in GRAY (Neutral) / BLACK (Charged)
    {
      if (std::fabs(duaughter->getCharge()) <= 0.1) {
        ced_line(duaughter->getEndpoint()[0], duaughter->getEndpoint()[1], duaughter->getEndpoint()[2],
                 duaughter->getVertex()[0], duaughter->getVertex()[1], duaughter->getVertex()[2], 1, 1, 0xbcbcbc);
        streamlog_out(DEBUG0) << " Other Unstable Neutral Particles (genStatus != 1 && genStatus != 2 ): " << std::endl;
        streamlog_out(DEBUG0) << *duaughter << std::endl;
      } else {
        DDMarlinCED::drawHelix(m_Bfield, duaughter->getCharge(), duaughter->getVertex()[0], duaughter->getVertex()[1],
                               duaughter->getVertex()[2], duaughter->getMomentum()[0], duaughter->getMomentum()[1],
                               duaughter->getMomentum()[2], 1, 1, 0x000000, 0.0, 2100.0, 3000.0, 0);
...
void drawReconstructedParticle(const RecoParticle& reconstructedParticle, const Vtx& primaryVertex, int colorCharged,
                               int colorNeutral) {
  double m_Bfield = MarlinUtil::getBzAtOrigin();
  int nTracks = (reconstructedParticle->getTracks()).size();
  if (nTracks > 0) {
    for (int i_trk = 0; i_trk < nTracks; ++i_trk) {
      Trk track = reconstructedParticle->getTracks()[i_trk];
      double trackCharge = (track->getOmega() > 0.0 ? 1.0 : -1.0);
      double trackPt = m_Bfield * 3.0e-4 / std::fabs(track->getOmega());
      double trackPx = trackPt * std::cos(track->getPhi());
      double trackPy = trackPt * std::sin(track->getPhi());
      ...
      double trackYs = track->getReferencePoint()[1] + track->getD0() * std::cos(track->getPhi());
      double trackZs = track->getReferencePoint()[2] + track->getZ0();
      DDMarlinCED::drawHelix(m_Bfield, trackCharge, trackXs, trackYs, trackZs, trackPx, trackPy, trackPz, 2, 2,
                             colorCharged, 0.0, 1500.0, 2000.0, 0);
    }
  } else {
    ...
    double endPointX =
        (reconstructedParticle->getClusters()[0]->getPosition()[0] - primaryVertex->getPosition()[0]) * 2.0 / 3.0;
    double endPointY =
        (reconstructedParticle->getClusters()[0]->getPosition()[1] - primaryVertex->getPosition()[1]) * 2.0 / 3.0;
    double endPointZ =
        (reconstructedParticle->getClusters()[0]->getPosition()[2] - primaryVertex->getPosition()[2]) * 2.0 / 3.0;

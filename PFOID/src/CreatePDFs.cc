...
CreatePDFs::CreatePDFs() : Processor("CreatePDFs") {
  ...
  registerProcessorParameter("MCParticleCollection", "Name of the MC Particle collection", _mcCol,
                             std::string("MCParticle"));

  registerProcessorParameter("RecoParticleCollection",
                             "Name of a reconstructed Particle collection containing PFOs like Wolf, PandoraPFA, ...",
                             _recoCol, std::string("RecoParticles"));

  registerProcessorParameter("ChargedPDFfilename", "Name of output file for charged particles", _filename_c,
                             std::string("pdf.txt"));

  registerProcessorParameter("NeutralPDFfilename", "Name of output file for neutral particles", _filename_n,
                             std::string("npdf.txt"));

  std::vector<int> chStart;
  registerProcessorParameter("ChargedRunStart", "Run numbers where a new charged category sample starts ", _chStart,
                             chStart);

  std::vector<int> nStart;
  registerProcessorParameter("NeutralRunStart", "Run numbers where a new neutral category sample starts ", _nStart,
                             nStart);
  ...
  registerProcessorParameter("MCTracksRelCollection", "Name of MC to Tracks Relation collection", _MCTracksRelCol,
...
void CreatePDFs::init() {
  ...
  if (_chStart.size() > 0 && _nStart.size() > 0) {
    if (_chStart[_chStart.size() - 1] >= _nStart[0]) {
      std::cout << " [CreatePDFs : ERR] - Category of charged particle samples must start before Category of neutral "
                   "particle samples!\n";
      exit(-3);
    }
      ...
      if (nClusters > 0) {
        ...
        if (sd != NULL) {
          ...
          if (sd->getParticleCont(ttmax) != 0) {
            std::cout << " Cluster MC : could be " << sd->getParticleCont(ttmax)->getPDG()
                      << " (# parents: " << sd->getParticleCont(ttmax)->getParents().size()
                      << ")  E: " << sd->getParticleCont(ttmax)->getEnergy() << " GeV ";
        ...
        float evi[3];
        for (int kk = 0; kk < 3; kk++)
          evi[kk] = clsp->getEigenValInertia()[kk];
              ...
              if (track == tv[k]) {
                for (unsigned int kk = 0; kk < MyPidCol.size(); kk++)
                  if (mcp->getPDG() == MyPidCol[kk]) {
                    ...
                    std::cout << " Track MC : could be " << mcp->getPDG() << " (# parents: " << mcp->getParents().size()
                              << ")  E: " << mcp->getEnergy() << " GeV ";

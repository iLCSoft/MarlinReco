...
std::vector<std::string> TrainingModel_TMVA_BDT_MC::initTraining(TrainingModelInterface& tmi) {
  ...
  if (tmi.inparF.size() > 0)
    _nEvtMin4Train = tmi.inparF[0];
  else
    _nEvtMin4Train = 10;
...
void TrainingModel_TMVA_BDT_MC::runTraining(TTree* inTree) {
  for (unsigned int i = 0; i < _tmi.momBins.size() - 1; ++i) {
    ...
    std::stringstream o;
    o << _modelName << "/TMVA_BDT_MC_Out_" << i << ".root";
    ...
    for (int pdg : _tmi.signalPDGs) {
      ...
      if (inTree->GetEntries(cutPDG) < _nEvtMin4Train) {
        sloE << _modelName << ": too few entries for BDT training! Only " << inTree->GetEntries(cutPDG)
             << " events in [" << m.str() << "], [PDG = +/- " << abs(pdg) << "]." << std::endl;
        cancel = true;
      }

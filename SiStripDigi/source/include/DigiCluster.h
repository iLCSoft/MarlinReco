...
public:
  ...
  DigiCluster(short int charge, float time, float depE, CLHEP::Hep3Vector position, short int iLayer, short int iLadder,
              short int iSensor, int cellID, EVENT::MCParticle* mcPart, EVENT::SimTrackerHit* simHit)
      : _time(time), _charge(charge), _iLayer(iLayer), _iLadder(iLadder), _iSensor(iSensor), _iCell(cellID) {
    _nCarriers = depE / Eeh;
    _position = position;
    _MCPart = mcPart;
    _simHit = simHit;

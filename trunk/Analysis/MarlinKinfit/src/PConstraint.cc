/*! \file 
 *  \brief Implements class PConstraint
 *
 * \b Changelog:
 *
 * \b CVS Log messages:
 * - $Log: PConstraint.cc,v $
...
PConstraint::PConstraint (double pxfact_, double pyfact_, double pzfact_,
...
PConstraint::~PConstraint () {
  //std::cout << "destroying PConstraint" << std::endl;
}

// calculate current value of constraint function
double PConstraint::getValue() const {
  double totpx = 0;
  double totpy = 0;
  double totpz = 0;
  double totE = 0;
...
//                                      =  1 * d px(i) /d par(i, j)
void PConstraint::getDerivatives(int idim, double der[]) const {
  for (unsigned int i = 0; i < fitobjects.size(); i++) {
...
void PConstraint::addToGlobalDerMatrix (double lambda, int idim, double *M) const {
...
void PConstraint::invalidateCache() const {
  cachevalid = false;
}

void PConstraint::updateCache() const {
  nparams = 0;
  for (unsigned int i = 0; i < fitobjects.size(); i++) {
    for (int ilocal = 0; ilocal < fitobjects[i]->getNPar(); ilocal++) {
      int iglobal = fitobjects[i]->getGlobalParNum (ilocal);
...
bool PConstraint::secondDerivatives (int i, int j, double *derivatives) const {
  return false;
}  
  
bool PConstraint::firstDerivatives (int i, double *derivatives) const {
  derivatives[0] = efact;

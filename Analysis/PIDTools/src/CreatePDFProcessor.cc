...
CreatePDFProcessor::CreatePDFProcessor() : Processor("CreatePDFProcessor") {

  // Processor description
  _description = "Processor for PDF templetes creation";

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputPDFFilename",
                          "Input collection of Reconstrcuted Particle", _filename,
                          std::string("pdf_standard_v01.root"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "RecoParticleCollection",
                          "Input collection of Reconstrcuted Particle", _PfoCollection, std::string("PandoraPFOs"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "MCTruthLinkCollection",
                          "Input collection of Reconstrcuted Particle", _LinkCollection,
  ...
  for (int i = 0; i < 6; i++) {
    for (int j = 0; j < 21; j++) {
      ...
      case 19:
        ...
        labeltxt = "hit mean";
        if (i == 0)
          pidvariable[i][j] = new TH1F(histtxt.c_str(), "", 100, 0.0, 2500.0);
        if (i != 0)
          pidvariable[i][j] = new TH1F(histtxt.c_str(), "", 100, 0.0, 1000.0);
        pidvariable[i][j]->SetMarkerStyle(20);
        pidvariable[i][j]->SetFillColor(0);
        pidvariable[i][j]->SetLineColor(1);
        pidvariable[i][j]->SetLineWidth(2.0);
      ...
      if (i == 0)
        pidvariable[i][j]->SetLineColor(5);
      if (i == 1)
        pidvariable[i][j]->SetLineColor(6);
      if (i == 2)
        pidvariable[i][j]->SetLineColor(2);
      if (i == 3)
        pidvariable[i][j]->SetLineColor(3);
      if (i == 4)
        pidvariable[i][j]->SetLineColor(4);
      if (i == 5)
        pidvariable[i][j]->SetLineColor(7);
    }
  }
...
void CreatePDFProcessor::processEvent(LCEvent* evt) {
  ...
  for (int i = 0; i < npfo; i++) {
    ...
    if (pdfid < 0)
      continue; // do not match to MC particle
    ...
    double ehypo =
        -0.5 * fabs(_myPID->get_dEdxChi2(0, pp, dedxerr, dedx)) + _myPID->get_dEdxFactor(0, pp, dedxerr, dedx);
    double muhypo =
        -0.5 * fabs(_myPID->get_dEdxChi2(1, pp, dedxerr, dedx)) + _myPID->get_dEdxFactor(1, pp, dedxerr, dedx);
    double pihypo =
        -0.5 * fabs(_myPID->get_dEdxChi2(2, pp, dedxerr, dedx)) + _myPID->get_dEdxFactor(2, pp, dedxerr, dedx);
    double khypo =
        -0.5 * fabs(_myPID->get_dEdxChi2(3, pp, dedxerr, dedx)) + _myPID->get_dEdxFactor(3, pp, dedxerr, dedx);
    double phypo =
        -0.5 * fabs(_myPID->get_dEdxChi2(4, pp, dedxerr, dedx)) + _myPID->get_dEdxFactor(4, pp, dedxerr, dedx);

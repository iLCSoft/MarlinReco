...
V0Finder::V0Finder() : Processor("V0Finder") {
  ...
  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "RecoParticleCollection",
                           "Name of output collection of reconstructed particles", _recoPartColName,
                           std::string("V0RecoParticles"));

  registerOutputCollection(LCIO::VERTEX, "VertexCollection", "Name of output collection of neutral vertices",
                           _vertexColName, std::string("V0Vertices"));
  ...
  registerProcessorParameter("CutOnRadius", "Cuts on V0 radius", _rVertCut, float(5.0));
  ...
  registerProcessorParameter("CutOnTrkDistance", "Cut on two track distance", _dVertCut, float(1.5));

  registerProcessorParameter("MinimumTrackHitRatio",
                             "Minimum ratio of inner track hit radius to reconstructed vertex radius",
                             _minTrackHitRatio, float(0.7));

  registerProcessorParameter("MassRangeGamma", "Maximal deviation in mass for photon candidate", _deltaMassGamma,
                             float(0.01));

  registerProcessorParameter("MassRangeK0S", "Maximal deviation in mass for K0S candidate", _deltaMassK0S, float(0.01));

  registerProcessorParameter("MassRangeL0", "Maximal deviation in mass for Lamda0 candidate", _deltaMassL0,
                             float(0.008));

  registerProcessorParameter("RxyCutGamma", "Minimum radius in xy plane for photon candidate", _rxyCutGamma,
                             float(10.0));

  registerProcessorParameter("RxyCutK0S", "Minimum radius in xy plane for K0S candidate", _rxyCutK0S, float(30.0));

  registerProcessorParameter("RxyCutLambda", "Minimum radius in xy plane for Lambda0 candidate", _rxyCutLambda,
                             float(50.0));
}
    ...
    for (int i = 0; i < nelem - 1; ++i) {
      ...
      for (int j = i + 1; j < nelem; ++j) {
        ...
        if (prodCharge < 0) { // two tracks with opposite charges
          ...
          if (r1 / radV0 < _minTrackHitRatio)
            continue;
          if (r2 / radV0 < _minTrackHitRatio)
            continue;

          //	  if (distV0 < _dVertCut && radV0 > _rVertCut ) { // cut on vertex radius and track misdistance
          if (radV0 > _rVertCut) {

            streamlog_out(DEBUG4) << " ***************** found vertex for tracks : "
                                  << dd4hep::rec::Vector3D((const float*)vertex) << " t1 " << lcshort(firstTrack)
                                  << "\n"
                                  << " t2 " << lcshort(secondTrack) << " distV0 " << distV0 << std::endl;
            ...
            if (distV0 < _dVertCut) { // cut on vertex radius and track misdistance
              ...
              float massK0 = sqrt(energyV0 * energyV0 - momentum[0] * momentum[0] - momentum[1] * momentum[1] -
              ...
              float massL0 = sqrt(energyV0 * energyV0 - momentum[0] * momentum[0] - momentum[1] * momentum[1] -
                                  momentum[2] * momentum[2]);
              ...
              energyV0 = energy1 + energy2;
              float massL0bar = sqrt(energyV0 * energyV0 - momentum[0] * momentum[0] - momentum[1] * momentum[1] -
                                     momentum[2] * momentum[2]);

              // Testing photon hypothesis
              energyV0 = pp1 + pp2;
              float massGamma = sqrt(energyV0 * energyV0 - momentum[0] * momentum[0] - momentum[1] * momentum[1] -
                                     momentum[2] * momentum[2]);
              ...
              if (radV0 < _rxyCutGamma)
                deltaGm = 100000.;
              if (radV0 < _rxyCutK0S)
                deltaK0 = 100000.;
              if (radV0 < _rxyCutLambda)
                deltaL0 = 100000.;
              if (radV0 < _rxyCutLambda)
                deltaL0bar = 100000.;
              ...
              streamlog_out(DEBUG) << "  ***** mass condition :  " << massCondition << "  code : " << code << std::endl;

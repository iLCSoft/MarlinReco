...
VTXBgClusters::VTXBgClusters() : Processor("VTXBgClusters") {
  ...
  registerProcessorParameter("Modif", "sensor modification option", _mod, int(0));
...
void VTXBgClusters::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
      ...
      if (accept == 1) {
        ...
        */
        ...
        if (_mod == 0)
          ...
          sma = 4 * _pitch[layerId];

        double SMA =
            ((int)(effpl * 1000 / _pitch[layerId] - 1 / 4) + 4) * _pitch[layerId]; // ellipse major axis - effpl in mm
        ...
        streamlog_out(DEBUG) << " eff. path length : " << effpl << " SMA " << SMA << " sma " << sma
                             << " layerId :  " << layerId << " ladderId :  " << ladderId << std::endl;
        ...
        SimTHit->ext<ClusterParams>() = new VXDClusterParameters(locPos, dSMAladder, dsmaladder, layerId, ladderId);
      }
    }
  }
...
void VTXBgClusters::end() {
  ...
  streamlog_out(DEBUG4) << "VTXBgClusters::end()  " << name() << " processed " << _nEvt << " events in " << _nRun
                        << " runs " << std::endl;
}

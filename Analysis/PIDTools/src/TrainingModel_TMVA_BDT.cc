...
std::vector<std::string> TrainingModel_TMVA_BDT::initTraining(TrainingModelInterface& tmi) {
  ...
  if (tmi.inparS.size() < 4) {
    sloE << "TrainingModel TMVA_BDT received too few (<4) string parameters for factory options!" << std::endl;
  }
  ...
  struct stat meta;
  if (stat(_modelName.c_str(), &meta))
    mkdir(_modelName.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);

  for (unsigned int i = 0; i < _tmi.momBins.size() - 1; ++i) {
    std::stringstream s;
    s << "/TMVA_BDT_" << i << ".weights.xml";
...
void TrainingModel_TMVA_BDT::initInference(TrainingModelInterface& tmi) {
  ...
  if (_tmi.obsNames.size() != _tmi.obsValues.size()) {
    sloE << "Observables names (" << _tmi.obsNames.size() << ") and values (" << _tmi.obsValues.size()
...
void TrainingModel_TMVA_BDT::runTraining(TTree* inTree) {
  for (unsigned int i = 0; i < _tmi.momBins.size() - 1; ++i) {
    std::stringstream m;
    m << "momabs > " << _tmi.momBins[i] << " && momabs < " << _tmi.momBins[i + 1];
    ...
    if (inTree->GetEntries(cutSig) < _nEvtMin4Train) {
      sloE << _modelName << ": too few entries for BDT training! Only " << inTree->GetEntries(cutSig) << " events in ["
           << m.str() << "] for signal PDGs." << std::endl;
      cancel = true;
    }
    if (inTree->GetEntries(cutBkg) < _nEvtMin4Train) {
      sloE << _modelName << ": too few entries for BDT training! Only " << inTree->GetEntries(cutBkg) << " events in ["
           << m.str() << "] for background PDGs." << std::endl;
      cancel = true;
    }
    if (cancel) {
      sloE << "Cancelling training!" << std::endl;
      return;
    }

    TMVA::DataLoader loader(_modelName.c_str());
    for (unsigned int v = 0; v < _tmi.obsNames.size(); ++v)
      loader.AddVariable(_tmi.obsNames[v]);
    std::stringstream o;
    o << _modelName << "/TMVA_BDT_Out_" << i << ".root";
...
const std::vector<float> TrainingModel_TMVA_BDT::runInference(int momBracket) {
  std::stringstream nm;
  nm << "MomBin_" << momBracket;
  float r = _TReader->EvaluateMVA(nm.str());

  std::vector<float> eval_out{};
  for (unsigned i = 0; i < _tmi.signalPDGs.size(); ++i)
    eval_out.push_back(r);
  for (unsigned i = 0; i < _tmi.backgroundPDGs.size(); ++i)
    eval_out.push_back(-r);

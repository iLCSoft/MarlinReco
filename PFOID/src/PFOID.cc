...
PFOID::PFOID() : Processor("PFOID") {
  ...
  registerProcessorParameter("RecoParticleCollection", "Name of the PFO collection", _recoCol,
  ...
  registerProcessorParameter("FilePDFName",
                             "Name of files containing pdfs for charged particles (order of files must be consistent "
                             "with parameter EnergyBoundaries!!)",
                             _filesCharged, filenamesC);
  ...
  registerProcessorParameter("neutralFilePDFName",
                             "Name of files containing pdfs for neutral particles (order of files must be consistent "
                             "with parameter EnergyBoundaries!!)",
                             _filesNeutral, filenamesN);
  ...
  registerProcessorParameter("EnergyBoundaries", "Boundary for energy binning", _energyBoundaries, energies);

  registerProcessorParameter("Debug", "Debugging?", _debug, int(0));
}
...
void PFOID::init() {
  ...
  if (!consistency) {
    ...
    std::cout
        << "Number of pdf files for neutral particles should be equal to the number of pdf files for charged particles"
        << std::endl;
...
void PFOID::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void PFOID::processEvent(LCEvent* evt) {
  ...
  _bField =
      float(gearBField.at(gear::Vector3D(0.0, 0.0, 0.0)).z()); // BField should be constant and point in z direction
  ...
  try {
    ...
    for (int i = 0; i < nRecos; i++) { // over all reco. particles
      ...
      for (int iBin = 0; iBin < _nEnergyBins; ++iBin) {
        if (energy >= _energyBoundaries[iBin] && energy < _energyBoundaries[iBin + 1]) {
          eBin = iBin;
          break;
        }
      }

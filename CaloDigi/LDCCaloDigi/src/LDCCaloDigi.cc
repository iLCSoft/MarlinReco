...
LDCCaloDigi::LDCCaloDigi() : Processor("LDCCaloDigi") {
  ...
  registerOutputCollection(LCIO::CALORIMETERHIT, "ECALOutputCollection", "ECAL Collection of real Hits",
                           _outputEcalCollection, std::string("ECAL"));

  registerOutputCollection(LCIO::CALORIMETERHIT, "HCALOutputCollection", "HCAL Collection of real Hits",
                           _outputHcalCollection, std::string("HCAL"));
  ...
  registerProcessorParameter("HCALThreshold", "Threshold for HCAL Hits in GeV", _thresholdHcal, (float)2.5e-4);
  ...
  registerProcessorParameter("ECAlEndcapCorrectionFactor", "Energy correction for endcap", _ecalEndcapCorrectionFactor,
                             (float)1.025);
      ...
      for (int j(0); j < numElements; ++j) {
        ...
        if (energy > _thresholdEcal) {
          ...
          if (_digitalEcal) {
            ...
            if (fabs(hit->getPosition()[2]) >= _zOfEcalEndcap)
              energy = energy * _ecalEndcapCorrectionFactor;
          ...
          for (unsigned int j = i + 1; j < _calHitsByStaveLayer[is][il].size(); ++j) {
            ...
            if (fabs(zi) < _zOfEcalEndcap && fabs(zj) < _zOfEcalEndcap) {
              ...
              if (xgap || ygap || xygap) {
                ...
                float f = _ecalGapCorrectionFactor; // fudge
                if (xgap)
                  ecor = 1. + f * (dx - _endcapPixelSizeX[il]) / 2. / _endcapPixelSizeX[il];
                if (ygap)
                  ecor = 1. + f * (dy - _endcapPixelSizeY[il]) / 2. / _endcapPixelSizeY[il];
                if (xygap)
                  ecor = 1. + f * (dx - _endcapPixelSizeX[il]) * (dy - _endcapPixelSizeY[il]) / 4. /
                                  _endcapPixelSizeX[il] / _endcapPixelSizeY[il];

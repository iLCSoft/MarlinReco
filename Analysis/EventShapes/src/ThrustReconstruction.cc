...
ThrustReconstruction::ThrustReconstruction() : Processor("ThrustReconstruction") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "inputCollectionName",
                          ...
                          _inputCollectionName, std::string("SelectedReconstructedParticle"));
...
void ThrustReconstruction::processEvent(LCEvent* evt) {
  ...
  if (_inParVec->getTypeName() != LCIO::RECONSTRUCTEDPARTICLE) {
    ...
    errorMsg << "\nProcessor: ThrustReconstruction \n"
             << "Collection is of wrong type (" << _inParVec->getTypeName() << "). Processor requires collection tpye "
  ...
  if (_principleThrustValue >= _max)
    _max = _principleThrustValue;
  if (_principleThrustValue <= _min)
    _min = _principleThrustValue;
}

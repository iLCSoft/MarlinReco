 ...
 * @param neutralFilePDFName names of ASCII files with pdf's of neutral particles. Each file corresponds to certain
 * energy bin. The order of files must be consistent with the energy bins. <br> (default parameter value :
 * "npdf_ILD00.txt" . Only one energy bin, covering entire energy range, is assumed) <br>
 * @param EnergyBoundaries the vector of energy boundaries, corresponding to different pdf files. The number of ranges
 * should be consistent with the number of ASCII pdf files for charged and neutral particles (The number of boundaries
 * equals to the number of ASCII pdf files plus one). <br> (default parameter values : 0.0 10000000.0 . Only one energy
 * bin, covering the whole energy range, is assumed) <br>

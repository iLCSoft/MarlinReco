...
void TJjetsPFOAnalysisProcessor::findTrueJetParticles(LCEvent* event) {
  for (int i_jet = 0; i_jet < njets(); i_jet++) {
    ...
    LCRelationNavigator* relation_TrueJetFinalColourNeutral =
        new LCRelationNavigator(event->getCollection(_finalColourNeutralLink));
    LCRelationNavigator* relation_TrueJetInitialColourNeutral =
        new LCRelationNavigator(event->getCollection(_initialColourNeutralLink));
    LCRelationNavigator* relation_TrueJetFinalElementon =
        new LCRelationNavigator(event->getCollection(_finalElementonLink));
    LCRelationNavigator* relation_TrueJetInitialElementon =
        new LCRelationNavigator(event->getCollection(_initialElementonLink));
    LCRelationNavigator* relation_TrueJetMCParticle =
        new LCRelationNavigator(event->getCollection(_trueJetMCParticleLink));

    LCRelationNavigatorVec tj_mc_relations{relation_TrueJetFinalColourNeutral, relation_TrueJetInitialColourNeutral,

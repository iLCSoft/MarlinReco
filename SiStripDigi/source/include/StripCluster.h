...
public:
  //! Constructor
  StripCluster(short int layerID, short int ladderID, short int sensorID, CLHEP::Hep3Vector position,
               CLHEP::Hep3Vector posSigma, double charge, short int size)
      : _iLayer(layerID), _iLadder(ladderID), _iSensor(sensorID), _position(position), _posSigma(posSigma),
        _charge(charge), _size(size) {
    _stripFront = 0;
    _stripRear = 0;
  }
  //! Constructor with strips ID
  StripCluster(short int layerID, short int ladderID, short int sensorID, CLHEP::Hep3Vector position,
               CLHEP::Hep3Vector posSigma, double charge, short int size, short int stripFront, short int stripRear)
      : _iLayer(layerID), _iLadder(ladderID), _iSensor(sensorID), _position(position), _posSigma(posSigma),
        _charge(charge), _size(size), _stripFront(stripFront), _stripRear(stripRear) {
    ;
  }
  //! Destructor
  ~StripCluster();

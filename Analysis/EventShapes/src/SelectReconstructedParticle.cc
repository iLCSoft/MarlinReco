...
SelectReconstructedParticle::SelectReconstructedParticle() : Processor("SelectReconstructedParticle") {

  // modify processor description
  _description = "SelectReconstructedParticle: Selects particles from all reconstructed particles to be used for the "
                 "thrust finder";

  // register steering parameters:
  // name, description, class-variable, default value

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "inputCollectionName",
                          "Collection of reconstructed particles to chose from", _inputCollectionName,
                          std::string(LCIO::RECONSTRUCTEDPARTICLE));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "outputCollectionName",
                           "Collection of selected reconstructed particles", _outputCollectionName,
                           std::string("SelectedReconstructedParticle"));

  registerProcessorParameter("MinimumMomentum",
                             "Minimum momentum a particle has to have to be used for the thrust calculation",
                             _minimumMomentum, float(0));
}
...
void SelectReconstructedParticle::processEvent(LCEvent* evt) {
  ...
  if (inParVec->getTypeName() != LCIO::RECONSTRUCTEDPARTICLE) {
    errorMsg << "\nProcessor: SelectReconstructedParticle \n"
             << "Collection is of wrong type (" << inParVec->getTypeName() << "). Processor requires collection tpye "
  ...
  for (int i = 0; i < inParVec->getNumberOfElements(); i++) {
    ...
    totparmom = sqrt(pparmom[0] * pparmom[0] + pparmom[1] * pparmom[1] + pparmom[2] * pparmom[2]);
    if (totparmom >= _minimumMomentum && totparmom <= 5000 &&

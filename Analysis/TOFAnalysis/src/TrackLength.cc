...
std::vector<HitState> getTrackStates(EVENT::ReconstructedParticle* pfo, float bField,
  ...
  for (size_t i = 0; i < subTracks.size(); ++i) {
    ...
    int errorFit = MarlinTrk::createFinalisedLCIOTrack(
        marlinTrk.get(), hits, &refittedTrack, MarlinTrk::IMarlinTrack::backward, &preFit, bField, maxChi2PerHit);
    // if fit fails, try also fit forward
    if (errorFit != 0) {
      streamlog_out(DEBUG7) << "Fit backward fails! Trying to fit forward for " << i + 1 << " subTrack in this PFO!"
                            << std::endl;
      marlinTrk.reset(trkSystem->createTrack());
      errorFit = MarlinTrk::createFinalisedLCIOTrack(marlinTrk.get(), hits, &refittedTrack,
                                                     MarlinTrk::IMarlinTrack::forward, &preFit, bField, maxChi2PerHit);
    }
    if (errorFit != 0) {
    ...
    if (std::abs(zLast - zFirst) > 10.) {
      ...
      streamlog_out(DEBUG7) << "subTrack " << i + 1 << " zFirst: " << hitsInFit.front().first->getPosition()[2]
      ...
      streamlog_out(DEBUG7) << "subTrack " << i + 1 << " rhoFirst: " << rhoFirst << " rhoLast: " << rhoLast
                            << " loop forward: " << loopForward << std::endl;
    }
    ...
    if (loopForward) {
      for (int j = 0; j < nHitsInFit; ++j) {
        ...
        streamlog_out(DEBUG6) << "Added state from hit at (" << state.hit->getPosition()[0] << ", "
                              << state.hit->getPosition()[1] << ", " << state.hit->getPosition()[2] << ")" << std::endl;
  ...
  if (pfo->getClusters().size() > 0 && tsCaloBugged != nullptr) {
    ...
    streamlog_out(DEBUG7) << "Added state tsCalo (" << (state.ts).getReferencePoint()[0] << ", "
                          << (state.ts).getReferencePoint()[1] << ", " << (state.ts).getReferencePoint()[2] << ")"
                          << std::endl;
...
float getTrackLengthSHA(Track* track, int location = TrackState::AtCalorimeter,
                        TrackLengthOption option = TrackLengthOption::zedLambda) {
  const TrackState* tsIP = track->getTrackState(TrackState::AtIP);
  const TrackState* tsCalo = getTrackStateAtCalorimeter(track);
  return getHelixLength(tsIP, tsCalo, location, option);
}

std::tuple<float, float> getTrackLengthIKF(const std::vector<IMPL::TrackStateImpl>& trackStates, float bField,
                                           TrackLengthOption option) {
  int nTrackStates = trackStates.size();
  if (nTrackStates <= 1)
    return std::tuple(0.f, 0.f);

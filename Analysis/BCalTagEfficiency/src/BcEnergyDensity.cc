...
Bool_t BcEnergyDensity::GetEnergyDensity(const Int_t& rLayer, const Double_t& rRadius, const Double_t& rPhi,
  ...
  for (int is = 0; is < nsegs; is++) {
    for (int ir = 0; ir < nrings; ir++) {
      if (!(i_cell[ir][is] < 0)) {
        ...
        if (2 * TMath::Pi() - the_phi < mSegmentDeltaPhi.at(the_ring + ir - 1) && is == 2) {
          phi += 2 * TMath::Pi();
        }
...
Int_t BcEnergyDensity::GetCellNumber(const Int_t& rLayer, const Double_t& rRadius, const Double_t& rPhi) const {
  unsigned int cell_cnt = 0; // cell counter

  if (rRadius < mRMin || rRadius > mRMax)
    return -1;
  ...
  Int_t segm_number = (Int_t)TMath::Floor((phi - mPhiMin) / mSegmentDeltaPhi.at(ring_number));
  if (segm_number >= mNumbersOfSegments.at(ring_number))
    return -1;
  ...
  if (cell_cnt > mCellStorage.size())
    return -1;

  return cell_cnt;
}

Bool_t BcEnergyDensity::MakeInterpolation(const Double_t& rX, const vector<Double_t>& rXData,
                                          const vector<ValErrType>& rYData, ValErrType* pResult) const {
  ...
  */
  ...
  for (size_t i = 0; i < rYData.size(); i++)
    y_data.push_back(rYData.at(i).Val);
  ...
  for (size_t i = 0; i < rYData.size(); i++)
    y_data.push_back(rYData.at(i).Lb);
  interp = new Interpolator(rXData, y_data, interp_type);
  // interp.SetData(rXData, y_data);
  if (interp > 0) {
  ...
  for (size_t i = 0; i < rYData.size(); i++)
    y_data.push_back(rYData.at(i).Ub);
  interp = new Interpolator(rXData, y_data, interp_type);
  // interp.SetData(rXData, y_data);
  if (interp > 0) {

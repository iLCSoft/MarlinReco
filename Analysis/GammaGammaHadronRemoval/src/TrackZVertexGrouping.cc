...
TrackZVertexGrouping::TrackZVertexGrouping() : Processor("TrackZVertexGrouping") {
  ...
  registerInputCollection(LCIO::TRACK, "TrackCollection", "Name of the Track input collection", _colNameTracks,
                          std::string("MarlinTrkTracks"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "TrackGroupPFOs",
                           "Name of the ReconstructedParticle output collection", _colNameTrkGroupPFOs,
                           std::string("TrackGroupPFOs"));

  registerOutputCollection(LCIO::VERTEX, "TrackGroupVertices", "Name of the Vertex output collection",
                           _colNameTrkGroupVertices, std::string("TrackGroupVertices"));

  registerProcessorParameter("Z0SignificanceCut", "Cut for merging track groups in Z0 significance", _z0SignificanceCut,
                             float(1.7));
}
...
void TrackZVertexGrouping::processRunHeader(LCRunHeader*) { _nRun++; }

void TrackZVertexGrouping::processEvent(LCEvent* evt) {

  streamlog_out(DEBUG2) << "   process event: " << evt->getEventNumber() << "   in run:  " << evt->getRunNumber()
                        << std::endl;
  ...
  try {
    ...
    streamlog_out(DEBUG6) << " collection " << _colNameTracks << " not found in event - nothing to do ... "
                          << std::endl;
  }

  if (colTrk->getTypeName() != LCIO::TRACK) {

    streamlog_out(ERROR) << " collection " << _colNameTracks << " not of type LCIO::TRACK " << std::endl;
...
void TrackZVertexGrouping::check(LCEvent* /*evt*/) {
  ...
  if (isFirstEvent()) {
    ...
    // _h.resize(5) ;
    // int nBins = 100 ;
    // _h[0] = new TH2F( "hbetaFirstHitsChrg", "beta vs momentum - first hit - charged",    nBins, .1 , 10., nBins, 0.93
    // , 1.03 ) ; _h[1] = new TH2F( "hbetaCloseHitsChrg", "beta vs momentum - closest hits - charged", nBins, .1 , 10.,
    // nBins, 0.93 , 1.03 ) ;

    // _h[2] = new TH2F( "hbetaFirstHitsNeut", "beta vs momentum - first hit - neutral",    nBins, .1 , 10., nBins, 0.93
    // , 1.03 ) ; _h[3] = new TH2F( "hbetaCloseHitsNeut", "beta vs momentum - closest hits - neutral", nBins, .1 , 10.,
    // nBins, 0.93 , 1.03 ) ;

    // _h[4] = new TH2F( "hbetaLastTrkHit",    "beta vs momentum - last tracker hit", nBins, .1 , 10., nBins, 0.93
    // , 1.03 ) ;
  }
}
...
void TrackZVertexGrouping::end() {
  ...
  streamlog_out(MESSAGE) << "TrackZVertexGrouping::end()  " << name() << " processed " << _nEvt << " events in "
                         << _nRun << " runs " << std::endl;
}

...
EvaluateTauFinder::EvaluateTauFinder() : Processor("EvaluateTauFinder") {
  ...
  registerInputCollection(LCIO::MCPARTICLE, "MCCollectionName", "Name of the MCParticle collection", _colNameMC,
                          std::string("MCParticlesSkimmed"));

  registerInputCollection(LCIO::LCRELATION, "RECOMCTRUTHCollectionName", "Name of the MC Truth PFA collection",
                          _colNameMCTruth, std::string("RecoMCTruthLink"));

  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "TauRecCollection", "Collection of Tau Candidates", _incol,
                          std::string("TauRec_PFO"));

  registerInputCollection(LCIO::LCRELATION, "TauLinkCollectionName", "Name of the link between tau and pfos",
...
void EvaluateTauFinder::init() {
  ...
  tautuple =
      new TNtuple("tautuple", "tautuple", "EvID:mcE:mcPhi:mcTheta:mcD0:recE:recPhi:recTheta:recD0:recNQ:recN:minv");
  mcmisstuple = new TNtuple("mcmiss", "mcmiss", "EvID:E:pt:theta:D0:D1:D2");
  taumatchtuple = new TNtuple("taumatch", "taumatch",
...
void EvaluateTauFinder::processRunHeader(LCRunHeader*) { _nRun++; }
...
void EvaluateTauFinder::processEvent(LCEvent* evt) {
  ...
  if (colTau != 0) {
    ...
    for (int k = 0; k < nT; k++) {
      ...
      if (_nEvt < coutUpToEv || _nEvt == coutEv)
        streamlog_out(DEBUG) << tau->getEnergy() << " " << phi << " " << theta << " " << D0 << " " << tauvec.size()
                             << endl;
      ...
      if (relationNavigatorTau) {
        ...
        if (mctau) {
          ...
          taumatchtuple->Fill(_nEvt, mctau->getEnergy(), Evis, pvis, ptvis, mc_phi, mc_theta, mc_D0, tau->getEnergy(),
                              p, pt, phi, theta, D0, LD);
          if (!contaminated)
  ...
  if (colMC != 0) {
    ...
    for (int k = 0; k < nMCP; k++) {
      ...
      if (particle->getGeneratorStatus() == 2 && fabs(particle->getPDG()) == 15 &&
  ...
  for (unsigned int p = 0; p < common; p++)
    tautuple->Fill(_nEvt, mctauvec[p].E, mctauvec[p].phi, mctauvec[p].theta, mctauvec[p].D0, rectauvec[p].E,
                   rectauvec[p].phi, rectauvec[p].theta, rectauvec[p].D0, rectauvec[p].NQ, rectauvec[p].N,
                   rectauvec[p].minv);
  if (mctauvec.size() > rectauvec.size()) {
  ...
  if (mctauvec.size() < rectauvec.size()) {
    for (unsigned int p = common; p < rectauvec.size(); p++)
      tautuple->Fill(_nEvt, 0, 0, 0, 0, rectauvec[p].E, rectauvec[p].phi, rectauvec[p].theta, rectauvec[p].D0,
                     rectauvec[p].NQ, rectauvec[p].N, rectauvec[p].minv);
  }
...
void EvaluateTauFinder::end() {
  ...
  streamlog_out(DEBUG) << "EvaluateTauFinder::end()  " << name() << " processed " << _nEvt << " events in " << _nRun
                       << " runs " << std::endl;

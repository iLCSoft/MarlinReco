...
TaJetClustering::TaJetClustering() : Processor("TaJetClustering") {
  ...
  // collections
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "PFOCollection", "PandoraPFA PFO collection", _pfoCollectionName,
                          std::string("PandoraPFOs"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputTauCollection", "Tau output collection",
                           _tauCollectionName, std::string("TaJets"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "RemainPFOCollection", "Remained PFO collection",
                           _remainCollectionName, std::string("RemainPFOs"));
  ...
  // selection switch
  registerProcessorParameter("NoSelection", "Neither primary nor cone cut at all if true. Only clustering.",
                             _noSelection, 0);
  // primary cuts
  registerProcessorParameter("MinimumTrackEnergy", "Minimum track energy to be accepted as taus", _minimumTrackEnergy,
                             2.0);
  registerProcessorParameter("MinimumJetEnergy", "Minimum jet energy to be accepted as taus", _minimumJetEnergy, 3.0);
  registerProcessorParameter("MinimumTrackEnergyAssoc", "Minimum track energy to be counted", _minimumTrackEnergyAssoc,
                             2.0);
  registerProcessorParameter("MaximumNeutralEnergyIn3Prong",
                             ...
                             _maximumNeutralEnergyInThreeProng, 1.0);
  registerProcessorParameter("AcceptFlexibleLowEnergyTrack",
                             ...
                             _acceptFlexibleLowEnergyTrack, 1);
  ...
  registerProcessorParameter("ConeMaxEnergyFrac", "Energy fraction of cone compared to central", _coneMaxEnergyFrac,
                             0.1);
  ...
  registerProcessorParameter( "IPCutsSigmaThreeProngWithNeutrals",
                              "Minimum number of sigma for three prong tau with neutrals, used with cone cuts (so
  looser)", _ipCutsSigmaThreeProngWithNeutrals,
                              3. );
...
void TaJetClustering::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void TaJetClustering::processEvent(LCEvent* evt) {
  ...
  */
  ...
  for (int i = 0; i < nPart; i++) {
    ...
    if (pCur == NULL)
      continue; // already associated particle skipped.
    if (!pCur->getCharge())
      continue; // neutral particle skipped.
    if (pCur->getEnergy() < _minimumTrackEnergy)
      break;
    ...
    */
    ...
    while (nPartAssoc != nPartAssocPriv) {
      for (int j = 0; j < nPart; j++) {
        ...
        if (pCur2 == NULL)
          continue; // already associated particle skipped.
        if (pCur2->getCharge() && find(jetTracks.begin(), jetTracks.end(), pCur2) != jetTracks.end())
          continue;
        if (!pCur2->getCharge() && find(jetNeutrals.begin(), jetNeutrals.end(), pCur2) != jetNeutrals.end())
          continue;
        ...
        */
        if (cosangle < _tauCosAngle)
          continue; // acceptance angle cut; skipped.
        if (invmass2 > _tauMass * _tauMass)
          continue; // invmass cut; skipped.
    ...
    if (!_noSelection) {
      ...
      //  primary selection
      if (jetEnergy < _minimumJetEnergy) {
        continue;
      }
      ...
      if (_acceptFlexibleLowEnergyTrack) {
        while ((ntracks != 1 && ntracks != 3) || abs(int(jetCharge)) != 1) {
          ...
          if (p->getEnergy() > _minimumTrackEnergyAssoc)
            break;
          ...
          //	cout << "Flexible low energy tracks: track treated as neutral: e = " << p->getEnergy() << ", c = " <<
          // p->getCharge(); cout	 << ", ntracks now " << ntracks << ", jc = " << jetCharge << endl;
        }
      }
      if ((ntracks != 1 && ntracks != 3) || abs(int(jetCharge)) != 1) {
        continue;
      }
    }
    // set RP
    ...
    if (!_noSelection) {
      ...
      if (ntracks == 3 && eneutral > _maximumNeutralEnergyInThreeProng) {
        delete pJet;
        continue;
      }
      ...
      for (int j = 0; j < nPart; j++) {
        ...
        // remove self
        if (find(pJet->getParticles().begin(), pJet->getParticles().end(), p) != pJet->getParticles().end())
          continue;
        ...
        if (cosangle > _coneMinCosAngle && cosangle < _coneMaxCosAngle) {
          econe += p->getEnergy();
          // cout << "particle added to cone; costheta = " << cosangle << ", e = " << p->getEnergy() << ", charge = " <<
          // p->getCharge() << endl;
        }
      }
      // cout << "econe = " << econe << ", jet energy = " << jetEnergy << endl;

      if (econe > _coneMaxEnergyFrac * jetEnergy) {
        delete pJet;
        continue;
      }
    }
    ...
    if( (ntracks == 3 && eneutral > 1. ) || ( econe > _coneMaxEnergyFrac * jetEnergy ) ){
      ...
      for( unsigned int j = 0; j < pJet->getParticles().size(); j++){
        ...
        if( ntracks == 3 && eneutral > 1. &&
           ( ( fabs( d0/d0err ) > _ipCutsSigmaThreeProngWithNeutrals || fabs( z0/z0err ) >
    _ipCutsSigmaThreeProngWithNeutrals ) && ( d0 < _ipMaxAbsolute && z0 < _ipMaxAbsolute ) ) ) ipcutaccepted = true;

        // 3-prong, no neutral
        if( ntracks == 3 && eneutral < 1. &&
           ( ( fabs( d0/d0err ) > _ipCutsSigmaThreeProngNoNeutrals || fabs( z0/z0err ) >
    _ipCutsSigmaThreeProngNoNeutrals ) && ( d0 < _ipMaxAbsolute && z0 < _ipMaxAbsolute ) ) ) ipcutaccepted = true;
      ...
      if( ntracks == 1 && eneutral < 1. && ipcutaccepted == false ){
        ...
        for( unsigned int j = 0; j < pJet->getParticles().size(); j++ ){
          ...
          //cout << "track for lepton ID: e = " << p->getEnergy() << " fracecal = " << fracecal << " calbytrack = " <<
    calbytrack << endl;

          // muon ID
          if( fracecal < _muMaxFracEcal && calbytrack < _muMaxCalByTrack ) leptoncutaccepted = true;
          // electron ID
          if( fracecal > _eMinFracEcal && calbytrack > _eMinCalByTrack && calbytrack < _eMaxCalByTrack )
    leptoncutaccepted = true;
        }
    ...
    */
    ...
    // cout << " e = " << jetEnergy << " : pz = " << jetMomentum.z() << " npart = " << pJet->getParticles().size() <<
    // endl;

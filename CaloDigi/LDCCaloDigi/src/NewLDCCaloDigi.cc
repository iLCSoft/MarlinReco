  ...
  for (unsigned int i(0); i < _ecalCollections.size(); ++i) {
    ...
    try {
      ...
      for (int j(0); j < numElements; ++j) {
        ...
        streamlog_out(DEBUG0) << " Hit energy " << energy << " in cellID : " << idDecoder(hit).valueString()
                              << std::endl;
      ...
      for (int j(0); j < numElements; ++j) {
        ...
        if (energy > _thresholdHcal[0]) {
          ...
          if (_digitalHcal) {
            ...
            if (ilevel > _calibrCoeffHcal.size() - 1) {
              streamlog_out(ERROR) << " Semi-digital level " << ilevel
                                   << " greater than number of HCAL Calibration Constants (" << _calibrCoeffHcal.size()
                                   << ")" << std::endl;
            } else {
          ...
          if (_digitalHcal) {
            ...
            if (fabs(hit->getPosition()[2]) >= _zOfEcalEndcap)
              energy = energy * _hcalEndcapCorrectionFactor;
            calhit->setEnergy(calibr_coeff * energy);
          }
          ...
          for (unsigned int j = i + 1; j < _calHitsByStaveLayer[is][il].size(); ++j) {
            ...
            if (fabs(zi) < _zOfEcalEndcap && fabs(zj) < _zOfEcalEndcap) {
              ...
              if (xgap || ygap || xygap) {
                ...
                streamlog_out(DEBUG0) << "NewLDCCaloDigi found endcap gap, adjusting energy! " << xgap << " " << ygap
                                      << " " << xygap << " , " << il << endl;
                streamlog_out(DEBUG0) << "stave " << is << " layer " << il << endl;
                streamlog_out(DEBUG0) << "  dx, dy " << dx << " " << dy << " , sizes = " << pixsizex << " " << pixsizey
                                      << endl;
                streamlog_out(DEBUG0) << " xmin... " << xmin << " " << xminm << " " << xmax << " ymin... " << ymin
                                      << " " << yminm << " " << ymax << endl;

...
#ifndef NewFTDDigiProcessor_h
#define NewFTDDigiProcessor_h 1
 ...
 * @modified by J. Duarte Campderros, IFCA (Santander)
 *  Smearing done in RPhi instead of X-Y
 * @modified by M Battaglia (CERN & UCSC) 
 *  Adapted for use with CLIC-ILD fwd layout 

 */


class NewFTDDigiProcessor : public Processor {
  
 public:
  
  virtual Processor*  newProcessor() { return new NewFTDDigiProcessor ; }
 ...
 protected:
  ...
  int _nEvt ;
  int _minIdStrip;
  float _pointResoPixel;
  float _pointResoStrip;
  float _pointResoStripPhi;
  float _momCut;
  float _maxPixelZ;
  int _removeDrays;

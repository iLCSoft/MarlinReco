...
CCDDigitizer::CCDDigitizer() : Processor("CCDDigitizer") {
  ...
  registerInputCollection(LCIO::SIMTRACKERHIT, "CollectionName", "Name of the SimTrackerHit collection", _colName,
                          std::string("vxd01_VXD"));

  registerOutputCollection(LCIO::TRACKERHIT, "OutputCollectionName", "Name of the output TrackerHit collection",
                           _outputCollectionName, std::string("VTXTrackerHits"));

  registerOutputCollection(LCIO::LCRELATION, "RelationColName", "Name of the output VTX trackerhit relation collection",
                           _colVTXRelation, std::string("VTXRelation"));

  //  cut on the energy of delta-electrons (in MeV)
  // (default parameter value : 0.03)
  registerProcessorParameter("CutOnDeltaRays", "Cut on delta-ray energy (MeV)", _cutOnDeltaRays, (double)0.030);

  // pixel size along direction perpendicular to beam axis (in mm)
  registerProcessorParameter("PixelSizeX", "Pixel Size X", _pixelSizeX, (double)0.020);

  // pixel size along beam axis (in mm) <br>
  registerProcessorParameter("PixelSizeY", "Pixel Size Y", _pixelSizeY,
                             (double)0.020); // 0.025

  registerProcessorParameter("Debug", "Debug option", _debug, int(0));

  //  number of electrons produced per MeV of deposited energy
  // (default parameter value : 270.3)
  registerProcessorParameter("ElectronsPerKeV", "Electrons per keV", _electronsPerKeV, (double)270.3);
  ...
  registerProcessorParameter("BackgroundHitsPerLayer", "Background Hits per Layer", _bkgdHitsInLayer, bkgdHitsInLayer);
  ...
  registerProcessorParameter("SegmentLength", "Segment Length", _segmentLength, double(0.003));

  // flag to switch on smearing of signal (signal noise)
  registerProcessorParameter("PoissonSmearing", "Apply Poisson smearing of electrons collected on pixels",
                             _PoissonSmearing, 1);

  // flag to switch on gaussian smearing of signal (electronic noise)
  registerProcessorParameter("ElectronicEffects", "Apply Electronic Effects", _electronicEffects, int(1));

  registerProcessorParameter("ElectronicNoise", "electronic noise in electrons", _electronicNoise, double(100));

  registerProcessorParameter("Saturation", "maximum number of electrons, which can be stroed in a pixel", _saturation,
                             double(10000));
  ...
  registerProcessorParameter("EnergyLoss", "Energy Loss keV/mm", _energyLoss, double(280.0));

  // flag to switch on additional background signals
  registerProcessorParameter("GenerateBackground", "Generate Background", _generateBackground, int(0));

  registerProcessorParameter("depletedDepth", "Thickness of depleted zone", depdep, double(0.01));

  registerProcessorParameter("undepletedDepth", "Thickness of undepleted zone", undep,
                             double(0.01)); // 0.03744 layerthickness

  registerProcessorParameter("BField", "BField in Tesla", _bfield, (double)4);

  // Voltage applied on the n layer is used to calculate electric field in depleted zone,this parameter is used only
  // when parameter Electric Field is set to be negative
  registerProcessorParameter("BiasVolt", "BiasVoltage in V", _biasvolt, (double)10);

  // electric field in depleted zone
  //(default parameter value: 10000)
  registerProcessorParameter("ElectricField", "Electric Field in V/cm", _efield,
                             (double)10000); // when using input biasvoltage, set efield negative

  registerProcessorParameter(
      "mu", "electrons mobility in depleted zoneat low elcectric fields, in  cm^2*V^-1*S^-1", _mu,
      (double)1400); // value from sinevs code:1340;the paper mentioned in void settanlorentzangle suggested 1417

  registerProcessorParameter("Temperatur", "Temperatur in Kelvin", _T,
                             (double)250); // 220-270 according to Konstantin

  // default parameter value from sinevs code: 34; value in "Vertex Detectors:The State of the Art and Future
  // Prospect",by Damerell,Dec 95: 34.6 cm^2/s at room temperature
  registerProcessorParameter("diffusioncoefficient", "diffcoeff in depleted zone in cm^2*S^-1", _difcoef, (double)34);
  ...
  registerProcessorParameter("reconstructmethod", "reconstruction method", _recmethod,
                             (int)1); //

  // threshold on charge deposited on one pixel (in electons),only needed reconstruction method 0
  registerProcessorParameter("Threshold", "Cell Threshold in electrons", _threshold, 200.);
  ...
  registerProcessorParameter("framesize", "size of cluster in reconstruction", _framesize, (int)2);
}
...
void CCDDigitizer::init() {
  ...
  stepx =
      _pixelSizeX /
      Numstepx; // distance between points at which amplitude of diffusion is calculated within one pixel in x direction
  stepy = _pixelSizeY / Numstepy; // distance between points  at which amplitude of diffusion is calculated within one
  ...
  histdist = pHistogramFactory->createHistogram1D("dist", "distance (mm) between rawhits and reconstructed hits", 100,
                                                  0, 0.05);
  ...
  histdistxy = pHistogramFactory->createHistogram2D(
      "distxy", "distance(mm) between rawhits and reconstructed hit (x,y- direction in local coordinates of ladder)",
      100, -0.05, 0.05, 100, -0.05, 0.05);
  histNionpoint =
      pHistogramFactory->createHistogram1D("numberofionpoints", "number of ionisationpoints per hit", 30, 0, 30);
  ...
  // table
  //  sigmin=0;//minimum sigma for diffusion,is sigma smaller than sigmin, the total charge is stored in the central
  //  pixel sigstep=0.005; settable();
  // table//
...
void CCDDigitizer::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
  ...
  try {
    ...
    for (int i = 0; i < nSTH; ++i) {
      ...
      if (accept) {
        ...
        if (_PoissonSmearing != 0)
          PoissonSmearer(simTrkHitVec);
        if (_electronicEffects != 0)
          GainSmearer(simTrkHitVec);
...
void CCDDigitizer::end() {
  ...
  std::cout << "CCDDigitizer::end()  " << name() << " processed " << _nEvt << " events in " << _nRun << " runs "
            << std::endl;
}
...
void CCDDigitizer::FindLocalPosition(SimTrackerHit* hit, double* localPosition, double* localDirection) {
  ...
  double xLab[3] = {hit->getPosition()[0], hit->getPosition()[1], hit->getPosition()[2]};
  ...
  // geant sometimes (about 1 from 10 times) produces a z coordinate, which is not in the exact centre of the active
  // layer, but which is still in the active layer.
...
void CCDDigitizer::ProduceIonisationPoints(SimTrackerHit* hit) {
  ...
  for (int i = 0; i < _numberOfSegments; ++i) {
    ...
    double de =
        _fluctuate->SampleFluctuations(double(1000. * _currentParticleMomentum), double(1000. * _currentParticleMass),
                                       _cutOnDeltaRays, segmentLength, double(1000. * dEmean)) /
        1000.;
...
void CCDDigitizer::ProduceHits(SimTrackerHitImplVec& vectorOfHits) {
  ...
  for (int i = 0; i < _numberOfSegments; i++) {
    ...
    // in undepleted zone: reflected and direct part contributes to charge in pixel, both parts are weighted by a factor
    // depending on distance to the undepleted zone
    ...
    if (distancetoplane <= depdep) {
      ...
      // double sigma=1.375*sqrt(_difcoef*trt)*10;// convert in mm with a factor 10, according to gear data;Sinevs
      // formular
      double sigma = sqrt(2 * _difcoef * trt) *
                     10; // convert in mm with a factor 10, according to gear data;formular taken from "Vertex
                         // Detectors:The State of the Art and Future Prospects",by Damerell,Dec 95
...
TrackerHitImpl* CCDDigitizer::ReconstructTrackerHit(SimTrackerHitImplVec& simTrkVec) {
  ...
  if (charge > 0) {
    ...
    double dist = sqrt(pow(posraw[0] - pos[0], 2) + pow(posraw[1] - pos[1], 2) + pow(posraw[2] - pos[2], 2));
    //  cout<<"dist:  "<<dist<<endl;
...
void CCDDigitizer::PrintInfo(SimTrackerHit* simHit, TrackerHitImpl* recoHit) {
  ...
  std::cout << "Simulated hit position = "
            << " " << simHit->getPosition()[0] << " " << simHit->getPosition()[1] << " " << simHit->getPosition()[2]
            << std::endl;

  std::cout << "Reconstructed hit position = "
            << " " << recoHit->getPosition()[0] << " " << recoHit->getPosition()[1] << " " << recoHit->getPosition()[2]
...
void CCDDigitizer::diffusion(double xdif, double ydif, double sigma) {
  ...
  // it computes a dampfactor(dependent on the distance between ionpoint and observatiopoint (in the xy plane) and the
  // functionparameter sigma) (sinev) for Numstepx*Numstepy observationpoints in each pixel and summarizes them (the
  // distribution is approximated by discrete points); after that values are normed
...
I started to create a lookuptable to decrease computing time. this part is not debugged. it should reduce computing time
to 70% of the time needed without. void CCDDigitizer::settable(){

...
Compute_dEdxProcessor::Compute_dEdxProcessor() : Processor("Compute_dEdxProcessor") {
  ...
  registerProcessorParameter("NumberofHitsCorrectionParameters", "parameters for number of hits correction", _ncorrpar,
                             float(1.468));

  std::vector<float> apar = {0.635762, -0.0573237};
  registerProcessorParameter("AngularCorrectionParameters", "parameters for angular correction", _acorrpar, apar);
...
std::pair<double, double> Compute_dEdxProcessor::CalculateEnergyLoss(TrackerHitVec& hitVec, Track* trk) {
  ...
  } else // to if (_StratCompHist)
  {
    ...
    normdedx = getNormalization(dedx, (float)nTruncate, trkcos);
    if (_isSmearing)
...
double Compute_dEdxProcessor::getNormalization(double dedx, float nHit, double trkcos) {
  // cal. hit dep.

...
SimpleFCalDigi::SimpleFCalDigi() : Processor("SimpleFCalDigi") {
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "FCALCollections", "Fcal Collection Names", _fcalCollections,
                           fcalCollections);

  registerOutputCollection(LCIO::CALORIMETERHIT, "FCALOutputCollection", "Fcal Collection of real Hits",
                           _outputFcalCollection, std::string("FCAL"));

  registerOutputCollection(LCIO::LCRELATION, "RelationOutputCollection", "CaloHit Relation Collection",
                           _outputRelCollection, std::string("RelationFcalHit"));

  registerProcessorParameter("FcalThreshold", "Threshold for Fcal Hits in GeV", _thresholdFcal, (float)0.0);

  registerProcessorParameter("CalibrFCAL", "Calibration coefficients for FCAL", _calibrCoeffFcal, (float)31.);
  ...
  registerProcessorParameter("FixLCalHits",
                             "Fix the hit positions in LCal using the cellID (for DBD simulated samples)", _fixLCalHits,
                             bool(false));

  registerProcessorParameter("CaloType", "type of calorimeter: em, had, muon", _caloType, std::string("had"));

  registerProcessorParameter("CaloID", "ID of calorimeter: lcal, fcal, bcal", _caloID, std::string("fcal"));

  registerProcessorParameter("CaloLayout", "subdetector layout: barrel, endcap, plug, ring", _caloLayout,
                             std::string("endcap"));
}
      ...
      for (int iHit(0); iHit < numElements; ++iHit) {
        ...
        if (energy > _thresholdFcal) {
          ...
          if (!_caloID.compare("lcal") && // true if it is false ...
            ...
            float z_from_cell =
                (zMin + (k - 1) * dZ + (dZ - WThickness) / 2.0) - 0.11; // 0.11 is a guess on the sensorthikness
            ...
            streamlog_out(DEBUG3) << "i,j,k,s : " << i << " " << j << " " << k << " " << s << " " << oddeven
                                  << std::endl;
            streamlog_out(DEBUG3) << "xyzr  input                         : " << hit->getPosition()[0] << " "
                                  << hit->getPosition()[1] << " " << hit->getPosition()[2] << " "
                                  << sqrt(hit->getPosition()[0] * hit->getPosition()[0] +
                                          hit->getPosition()[1] * hit->getPosition()[1])
                                  << std::endl;
            streamlog_out(DEBUG3) << "xyzr with r and z fr cellID rotated : " << pos[0] << " " << pos[1] << " "
                                  << pos[2] << " " << sqrt(pos[0] * pos[0] + pos[1] * pos[1]) << std::endl;
            streamlog_out(DEBUG2) << " hit and diffs " << hit->getPosition()[0] << " " << hit->getPosition()[1] << " "
                                  << hit->getPosition()[2] << " " << hit->getPosition()[0] - pos[0] << " "
                                  << hit->getPosition()[1] - pos[1] << " " << hit->getPosition()[2] - pos[2]
                                  << std::endl;
          ...
          calhit->setType(CHT(caloTypeFromString(_caloType), caloIDFromString(_caloID),
                              layoutFromString(_caloLayout.c_str()), idDecoder(hit)[_cellIDLayerString]));

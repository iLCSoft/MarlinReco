...
TPCDigiProcessor::TPCDigiProcessor() : Processor("TPCDigiProcessor") {
  ...
  registerProcessorParameter("DontEncodeSide", "Do not encode the side in the cellID of the TrackerHit",
                             _dontEncodeSide, (bool)true);
}
...
void TPCDigiProcessor::init() {
  ...
  _TREE = _TRF->create("TPCDigi.root", "root", false, true);
...
void TPCDigiProcessor::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }

void TPCDigiProcessor::processEvent(LCEvent* evt) {

  gsl_rng_set(_random, Global::EVENTSEEDER->getSeed(this));
  streamlog_out(DEBUG) << "seed set to " << Global::EVENTSEEDER->getSeed(this) << " for event number "
  ...
  if (STHcol != 0) {
    ...
    for (int i = 0; i < n_sim_hits; i++) {
      ...
      if (driftLength < 0) {
        streamlog_out(DEBUG3) << " TPCDigiProcessor : Warning! driftLength < 0 " << driftLength
      ...
      //      double bReso = ( (_diffRPhi * _diffRPhi) / _nEff ) * sin(padTheta) * ( 6.0 / (padheight) )  * ( 4.0 /
      //      bField  ) ;
      double bReso =
          ((_diffRPhi * _diffRPhi) / _nEff) * sin(padTheta) * (6.0 / (padheight)) * ((4.0 * 4.0) / (bField * bField));
      ...
      int iZHit =
          (int)((float)NBinsZ * (gearTPC.getMaxDriftLength() + thisPoint.z()) / (2.0 * gearTPC.getMaxDriftLength()));
...
void TPCDigiProcessor::end() {
  ...
  streamlog_out(MESSAGE) << "TPCDigiProcessor::end()  " << name() << " processed " << _nEvt << " events in " << _nRun
                         << " runs " << endl;
  //
}
...
void TPCDigiProcessor::plotHelixHitResidual(MCParticle* mcp, CLHEP::Hep3Vector* thisPoint) {
  ...
  if (pt > 1.0) {
    ...
    double rHit_diff = thisPoint->perp() - padLayout.getPlaneExtent()[0] - ((row + 0.5) * padLayout.getPadHeight(pad));
...
double TPCDigiProcessor::getPadPhi(CLHEP::Hep3Vector* thisPoint, CLHEP::Hep3Vector* firstPoint,
...
double TPCDigiProcessor::getPadTheta(CLHEP::Hep3Vector* firstPoint, CLHEP::Hep3Vector* middlePoint,

...
G2CD aG2CD;
G2CD::G2CD() : Processor("G2CD"), _output(0) {
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "ECALCollections", "Input ECAL Hits Collection Names",
                           _ecalCollections, ecalCollections);
  ...
  registerProcessorParameter("DigiECALCollection", "Name of Digitized ECAL Hit Collections", _outputEcalCollections,
                             outputEcalCollections);

  std::vector<float> calibrEcal;
  calibrEcal.push_back(40.91);
  calibrEcal.push_back(81.81);
  registerProcessorParameter("CalibrECAL", "Calibration coefficients for ECAL", _calibCoeffEcal, calibrEcal);

  registerProcessorParameter("NumThinEcalLayer", "Num of thiner Ecal layers", _NEcalThinLayer, 20);
  ...
  registerProcessorParameter("DigiHCALCollection", "Name of Digitized HCAL Hit Collections", _outputHcalCollections,
  ...
  ChargeSpatialDistri.push_back(0.2);
  ChargeSpatialDistri.push_back(0.1);
  registerProcessorParameter("ChargeSpatialDistribution", "Spactial Distribution of MIP charge X*Y;",
                             _ChargeSpatialDistri, ChargeSpatialDistri);

  std::vector<int> ShowerPositionShiftID;
  ShowerPositionShiftID.push_back(0);
  ShowerPositionShiftID.push_back(0);
  ShowerPositionShiftID.push_back(0);
  registerProcessorParameter("PositionShiftID", "Global Position Shift For Overlay", _ShowerPositionShiftID,
                             ShowerPositionShiftID);

  registerProcessorParameter("DigiCellSize", "Size of Digitized Cell (in mm)", _DigiCellSize, 10);
  ...
  registerProcessorParameter("UsingDefaultDetector",
                             ...
                             _UsingDefaultDetector, 0);

  registerProcessorParameter("PolyaParaA", "Polya: x^A*exp(-b*x) + c", _PolyaParaA, float(0.7));

  registerProcessorParameter("PolyaParaB", "Polya: x^a*exp(-B*x) + c", _PolyaParaB, float(0.045));

  registerProcessorParameter("PolyaParaC", "Polya: x^a*exp(-b*x) + C", _PolyaParaC, float(0.03));

  registerProcessorParameter("ChanceOfKink", "Chance of one boundary hit to create a multiple hit with boosted charge",
                             _ChanceOfKink, float(0.0));

  registerProcessorParameter("KinkHitChargeBoost", "Scale factor of Charge on boosted multiple hits",
                             _KinkHitChargeBoost, float(1.0));
}
...
void G2CD::init() {
  ...
  cout << "Parameters After Para Review: " << ChanceOfKink << ", " << KinkHitChargeBoost << ", " << _DigiCellSize
       << endl;
  printParameters();
  ...
  if (CoverAreaLength > _DigiCellSize - 1) {
    cout << "WARNING! CoverAreaLength is too large comparing to the Digitized Cell Size: to have maximally 4 multiple "
            "hit, CoverAreaLength must <= _DigiCellSize - 1"
         << endl;
    exit(0);
  }
  int tmpIndex = 0;
  ...
  for (int i2 = 0; i2 < _DigiCellSize; i2++) {
    for (int j2 = 0; j2 < _DigiCellSize; j2++) {
      ...
      for (int i3 = 0; i3 < CoverAreaLength; i3++) {
        if (i3 < IndexA)
          WeightA += NormalWeight[i3];
        if (i3 < IndexB)
          WeightB += NormalWeight[i3];
      }
...
void G2CD::processEvent(LCEvent* evtP) {
  if (evtP) {
    try {
        ...
        try {
          ...
          for (int k1 = 0; k1 < NumEcalhit; k1++) {
            ...
            if (HitEn > _thresholdEcal) {
              ...
              LCRelationImpl* rel =
                  new LCRelationImpl(DigiEcalhit, SimEcalhit, 1.0); // only keep the leading contribution
      ...
      for (unsigned int i(0); i < _hcalCollections.size(); ++i) { // strictly follow barrel, endcap, ring order
        ...
        try {
          ...
          for (int j(0); j < numOrihits; ++j) {
            ...
            for (int i0 = 0; i0 < int(CurrWeightVec.size()); i0++) {
              ...
              if (IDtoDigiHit.find(DHCellID0) == IDtoDigiHit.end() && IDtoPos.find(DHCellID0) == IDtoPos.end()) {
                ...
                if (i == 0) // Barrel
                {
                  IDtoDigiHit[DHCellID0].PosX =
                      RefPosX + (DeltaPosI + int(DHIndexI - CurrI) * _DigiCellSize) * cos(tmpS * pi / 4.0) +
                      _ShowerPositionShiftID[0] * _DigiCellSize; //(mm in unit)
                  IDtoDigiHit[DHCellID0].PosY =
                      RefPosY + (DeltaPosI + int(DHIndexI - CurrI) * _DigiCellSize) * sin(tmpS * pi / 4.0) +
                      _ShowerPositionShiftID[1] * _DigiCellSize;
                  IDtoDigiHit[DHCellID0].PosZ =
                      RefPosZ + DeltaPosJ + int(DHIndexJ - CurrJ) * _DigiCellSize +
                      _ShowerPositionShiftID[2] * _DigiCellSize; // Rotation is needed, based on S;
                } else                                           // endcap or ring;
                {
                  IDtoDigiHit[DHCellID0].PosX = RefPosX + DeltaPosI + (DHIndexI - CurrI) * _DigiCellSize +
                                                _ShowerPositionShiftID[0] * _DigiCellSize; //(mm in unit)
                  IDtoDigiHit[DHCellID0].PosY = RefPosY + DeltaPosJ + (DHIndexJ - CurrJ) * _DigiCellSize +
                                                _ShowerPositionShiftID[1] * _DigiCellSize;
          ...
          for (std::map<int, DigiHit>::iterator ff = IDtoDigiHit.begin(); ff != IDtoDigiHit.end(); ff++) {
            ...
            LCRelationImpl* rel = new LCRelationImpl(calhit, ff->second.LeadSimCaloHit,
                                                     ff->second.ChargeShare); // only keep the leading contribution
            relcol->addElement(rel);
...
void G2CD::end() { std::cout << "General Gas Digitizer FINISHED" << std::endl; }

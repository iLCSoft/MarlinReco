...
QuarkJetPairing::QuarkJetPairing() : Processor("QuarkJetPairing") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputJetCollection", "Input collection of jets",
                          _inputJetCollection, std::string("NewJets"));

  registerInputCollection(LCIO::MCPARTICLE, "InputMCPCollection", "Input collection of MCParticles",
                          _inputMCPCollection, std::string("MCParticlesSkimmed"));
}
...
void QuarkJetPairing::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
  ...
  if (energy > 495) {
    ...
    for (int n = 0; n < iperm; n++) {
      ...
      for (int ijet = 0; ijet < 4; ijet++) {
        alpha[n] += acos(((jet_px[jet[ijet][n]] * quark_px[quark[ijet][n]]) +
                          (jet_py[jet[ijet][n]] * quark_py[quark[ijet][n]]) +
                          (jet_pz[jet[ijet][n]] * quark_pz[quark[ijet][n]])) /
                         (jet_ptot[jet[ijet][n]] * quark_ptot[quark[ijet][n]]));
      }
    ...
    if (alpha_min < 1) {
      ...
      for (int ijet = 0; ijet < 4; ijet++) {
        ...
        quark_theta[ijet] =
            atan((sqrt((pow(quark_px[quark[ijet][iperm_min]], 2)) + (pow(quark_py[quark[ijet][iperm_min]], 2)))) /
                 quark_pz[quark[ijet][iperm_min]]);
        jet_theta[ijet] = atan((sqrt((pow(jet_px[jet[ijet][iperm_min]], 2)) + (pow(jet_py[jet[ijet][iperm_min]], 2)))) /
                               jet_pz[jet[ijet][iperm_min]]);
        ...
        if (((quark_theta[ijet] > 0.14) && (quark_theta[ijet] < 3)) &&
            ((jet_theta[ijet] > 0.14) && (jet_theta[ijet] < 3))) {
          counter_jets++;
...
void QuarkJetPairing::end() { std::cout << "Finish run" << std::endl; }

...
public:
  ...
  LayerResolverBase(const int _detTypeFlag, const std::string _collectionName, const std::string _detectorName,
...
template <class T>
class LayerResolver : public LayerResolverBase {

public:
  LayerResolver() = delete;
  LayerResolver(const LayerResolver<T>& lt) = delete;
  LayerResolver(const int _detTypeFlag, T*, const std::string _collectionName, const std::string _detectorName,
                double _sensThickCheatVal = -1.);

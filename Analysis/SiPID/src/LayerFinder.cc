...
LayerResolverBase::LayerResolverBase(const int _detTypeFlag, const std::string _collectionName,
                                     const std::string _detectorName, double _sensThickCheatVal)
    : ThicknessSensitive(_sensThickCheatVal > 0. ? &LayerResolverBase::SensitiveThicknessCheat : NULL),
      sensThickCheatVal(_sensThickCheatVal), detTypeFlag(_detTypeFlag), collectionName(_collectionName),
      detectorName(_detectorName), collection(NULL), decoder(NULL) {}
...
int LayerResolverBase::GetNumberOfHits() const {
  if (collection)
    return collection->getNumberOfElements();
  else
    return -1;
}
...
TrackerHitPlane* LayerResolverBase::GetHit(int i) const {
  if (collection)
    return dynamic_cast<TrackerHitPlane*>(collection->getElementAt(i));
  else
    return NULL;
}
...
int LayerResolverBase::SetCollection(EVENT::LCEvent* evt) {
  ...
  streamlog_out(DEBUG5) << "LayerResolver::SetCollection: Looking for collection " << collectionName << ".\n";
  try {
    collection = evt->getCollection(collectionName);
  } catch (EVENT::DataNotAvailableException& dataex) {
    streamlog_out(MESSAGE) << "Collection " << collectionName << " not found in event #" << evt->getEventNumber()
                           << ".\n";
    collection = NULL;
    return -1;
  }
  ...
  streamlog_out(DEBUG5) << "Found collection of type \'" << collection->getTypeName() << "\' with encoding \'"
...
template <class T>
LayerResolver<T>::LayerResolver(const int _detTypeFlag, T* _layering, const std::string _collectionName,
                                const std::string _detectorName, double _sensThickCheatVal)
    : LayerResolverBase(_detTypeFlag, _collectionName, _detectorName, _sensThickCheatVal), layering(_layering) {
  if (_sensThickCheatVal < 0)
    ThicknessSensitive = (LayerResolverFn)(&LayerResolver<T>::SensitiveThicknessRead);
}
...
LayerFinder::LayerFinder(EVENT::StringVec _collectionNames, dd4hep::Detector& theDetector, FloatVec sensThickCheatVals)
    : layerResolvers() {
  ...
  if (_collectionNames.size() != detElements.size()) {
    streamlog_out(ERROR) << "There are " << detElements.size()
                         << " tracker detector elements in "
                            "the geometry and "
                         << _collectionNames.size()
  ...
  for (unsigned i = 0; i < detElements.size(); i++) {
    ...
    if (it == layerResolvers.end()) {
      ...
      streamlog_out(ERROR) << "Detector element \'" << detElements.at(i).name()
                           ...
                           << " which is already taken by the detector element \'" << it->second->GetDetectorName()
                           << "\'! Aborting.\n";
      exit(0);
    }
...
void LayerFinder::ReportHandledDetectors() {
  ...
  for (auto dit : layerResolvers) {
    std::string dettype;
    if (dit.second->GetDetTypeFlag() & dd4hep::DetType::BARREL)
      dettype = "BARREL";
    else if (dit.second->GetDetTypeFlag() & dd4hep::DetType::ENDCAP)
      dettype = "ENDCAP";
  ...
  for (auto cit : layerResolvers) {
    ...
    for (int i = 0; i < cit.second->GetNumberOfHits(); i++) {
      ...
      if (thit == cit.second->GetHit(i)) {
        ...
        streamlog_out(DEBUG5) << " ... Found matching hit " << cit.second->GetHit(i)->getCellID0() << " with energy "
                              << cit.second->GetHit(i)->getEDep() << " in collection "
                              << cit.second->GetCollectionName() << ".\n";

...
PrepareRECParticles::PrepareRECParticles() : Processor("PrepareRECParticles") {
  ...
  _description = "PrepareRECParticles converts input to ReconstructedParticles and puts them into a new collection "
                 "making sure all the information which is needed to run the TauFinder is provided. ";

  // register steering parameters: name, description, class-variable, default value
  registerInputCollection(LCIO::MCPARTICLE, "MCCollectionName           ", "Name of the MCParticle collection",
                          _colNameMC, std::string("MCParticlesSkimmed"));

  registerInputCollection(LCIO::TRACK, "TrackCollectionName", "Name of the Track collection", _colNameTrack,
                          std::string("LDCTracks"));

  registerProcessorParameter("outputColMC", "Name of the output Collection of refilled information", _outcolMC,
                             std::string("MCParticles_tau"));

  registerProcessorParameter("outputColTracks", "Name of the output Collection of refilled information", _outcolTracks,
                             std::string("Tracks_tau"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "RecCollection", "Collection of Rec Particles for TauFinder",
                           _outcolMC, std::string("MCParticles_tau"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "RecCollection_Tracks",
                           "Collection of Rec Particles for TauFinder", _outcolTracks, std::string("Tracks_tau"));

  registerOutputCollection(LCIO::LCRELATION, "MCRECLinkCollectionName",
                           "Name of the MC Truth ReconstructedParticle collection", _colNameMCTruth,
                           std::string("MCRecLink"));

  registerOutputCollection(LCIO::LCRELATION, "TrackRECLinkCollectionName",
                           "Name of the Track Truth ReconstructedParticle collection", _colNameTrackTruth,
...
void PrepareRECParticles::processRunHeader(LCRunHeader*) { _nRun++; }
  ...
  if (colMC != 0) {
    ...
    for (int k = 0; k < nMCP; k++) {
      ...
      if (mc->getGeneratorStatus() == 1) // only stable ones
      {
        ...
        if ((mc->getMass() == 0 && fabs(mc->getPDG()) != 22) || mc->getPDG() == 1000022)
          continue;
...
void PrepareRECParticles::end() {
  ...
  streamlog_out(DEBUG) << "PrepareRECParticles::end()  " << name() << " processed " << _nEvt << " events in " << _nRun
                       << " runs " << std::endl;

...
TruthVertexFinder::TruthVertexFinder() : Processor("TruthVertexFinder") {
  ...
  registerInputCollection(LCIO::MCPARTICLE, "CollectionName", "Name of the MCParticle collection", _colName,
  ...
  registerOutputCollection(LCIO::VERTEX, "OutputCollectionName", "Name of the Vertex collection", _outputcolName,
                           std::string("MCVertex"));
  registerOutputCollection(LCIO::MCPARTICLE, "OutputBStarName", "Name of the Vertex collection", _outputBStarName,
                           std::string("BStar"));
  registerOutputCollection(LCIO::MCPARTICLE, "OutputProngsName", "Name of the Prongs collection", _outputProngsName,
                           std::string("EGProngs"));
  ...
  registerOutputCollection(LCIO::MCPARTICLE, "QuarkCollectionName", "Name of the b-quark collection",
                           _outputquarkcolName, std::string("MCbquarks"));
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "RelCollectionName", "Name of the Jet relation collection",
                          _colRelName, std::string("RecoMCTruthLink"));
  registerProcessorParameter("DecayChainPDGs", "PDGs of desired decays", inputPdg, vector<int>());
  _tagParameter = 6;
  registerProcessorParameter("tagPDG", "PDG of desired particle", _tagParameter, _tagParameter);
  _writeROOTparameter = 0;
  registerProcessorParameter("writeROOT", "write ROOT file?", _writeROOTparameter, _writeROOTparameter);
  _initialQuarkPDGparameter = 5;
  registerProcessorParameter("initialQuarkPDG", "PDG of initial particle", _initialQuarkPDGparameter,
  ...
  registerProcessorParameter("writeBonly", "b parameter", _writeBonlyParameter, _writeBonlyParameter);
  _hfilename = "GenVertexTest.root";
  registerProcessorParameter("ROOTFileName", "ROOT File Name", _hfilename, _hfilename);
...
void TruthVertexFinder::init() {
  ...
  _hVertexTree->Branch("interactionOfParticles", _interactionOfParticles,
                       "interactionOfParticles[numberOfVertexes][15]/I");
...
void TruthVertexFinder::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void TruthVertexFinder::PrintParticle(MCParticle* particle) {
  ...
  streamlog_out(MESSAGE) << "|" << particle->getPDG() << "\t\t|" << particle->getMass() << "\t\t|"
                         << particle->getCharge() << "\t\t|" << particle->getEnergy() << "\t\t|"
                         << particle->getVertex()[0] << "\t\t|" << particle->getVertex()[1] << "\t\t|"
...
void TruthVertexFinder::AddProngs(VertexMCOperator& vertexOperator, MCOperator& opera, DecayChain* chain,
                                  vector<Vertex*>* verticies, std::vector<int>& parameters,
...
void TruthVertexFinder::processEvent(LCEvent* evt) {
  try {
    ...
    if (bbarChain) {
      vector<MCParticle*> daughters =
          opera.SelectStableCloseDaughters(bbarChainRaw->Get(0), bbarChain->Get(0)->getPDG());
...
void TruthVertexFinder::Write(MCOperator& opera, DecayChain* chain, vector<Vertex*>* verticies) {
  ...
  if (chain->GetParentPDG() > 0 && chain->GetSize() > 1) {
    ...
    const vector<MCParticle*> daughters =
        static_cast<MyVertex*>(verticies->at(0))
            ->__GetMCParticles(); // opera.SelectStableCloseDaughters(chain->Get(0), chain->Get(1)->getPDG());
                                  // //opera.ScanForVertexParticles(bverticies->at(0)->getPosition(), 1e-3);
    _bcharge = (int)chain->Get(0)->getCharge();
    _bnumber = daughters.size();
    _bnumber_f = opera.SelectStableCloseDaughters(chain->Get(0), chain->Get(1)->getPDG(), true)
                     .size(); // opera.SelectStableCloseDaughters(chain->Get(0),
                              // chain->Get(1)->getPDG(),true).size();//opera.CheckDaughterVisibility(daughters).size();
    ...
    const vector<MCParticle*> cdaughters =
        static_cast<MyVertex*>(verticies->at(1))
            ->__GetMCParticles(); // opera.SelectStableCloseDaughters(chain->Get(1));
                                  // //opera.ScanForVertexParticles(bverticies->at(1)->getPosition(), 1e-3);
    _cnumber = cdaughters.size();
    _cnumber_f = opera.SelectStableCloseDaughters(chain->Get(1), 0, true)
                     .size(); // opera.CheckDaughterVisibility(cdaughters).size();
  ...
  if (chain->GetParentPDG() < 0 && chain->GetSize() > 1) {
    const vector<MCParticle*> daughters =
        static_cast<MyVertex*>(verticies->at(0))
            ->__GetMCParticles(); // opera.SelectStableCloseDaughters(chain->Get(0), chain->Get(1)->getPDG()); //
                                  // opera.ScanForVertexParticles(bbarverticies->at(0)->getPosition(), 1e-3);
    Write(daughters, -1);
    _bbarcharge = (int)chain->Get(0)->getCharge();
    _bbarnumber = daughters.size();
    _bbarIPdistance = verticies->at(0)->getParameters()[0];
    _bbarnumber_f = opera.SelectStableCloseDaughters(chain->Get(0), chain->Get(1)->getPDG(), true)
                        .size(); // opera.CheckDaughterVisibility(daughters).size();
    streamlog_out(MESSAGE) << "Vertex bbar-quark" << 0 << ": " << verticies->at(0)->getParameters()[0]
                           << " n-tracks: " << _bbarnumber << '\n';
    const vector<MCParticle*> cdaughters =
        static_cast<MyVertex*>(verticies->at(1))
            ->__GetMCParticles(); // opera.SelectStableCloseDaughters(chain->Get(1));
                                  // //opera.ScanForVertexParticles(bbarverticies->at(1)->getPosition(), 1e-3);
    _cbarnumber = cdaughters.size();
    opera.CheckDaughterVisibility(daughters);
    streamlog_out(MESSAGE) << "Vertex cbar-quark: " << verticies->at(1)->getParameters()[0]
                           << " n-tracks: " << _cbarnumber << '\n';
    opera.CheckDaughterVisibility(cdaughters);
    _cbarnumber_f = opera.SelectStableCloseDaughters(chain->Get(1), 0, true)
                        .size(); // opera.CheckDaughterVisibility(cdaughters).size();
...
double TruthVertexFinder::getMissingPt(const vector<MCParticle*>& bdaugthers, const vector<MCParticle*>& cdaughters,
                                       Vertex* vertex) {

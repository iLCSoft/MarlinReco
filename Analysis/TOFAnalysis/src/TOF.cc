...
std::vector<EVENT::CalorimeterHit*> selectFrankEcalHits(EVENT::Cluster* cluster, const Vector3D& posAtEcal,
                                                        const Vector3D& momAtEcal, int maxEcalLayer) {
  ...
  for (auto hit : cluster->getCalorimeterHits()) {
    ...
    if ((!isECALHit) || (layer >= maxEcalLayer))
      continue;
  ...
  selectedHits.erase(
      std::remove_if(selectedHits.begin(), selectedHits.end(), [](CalorimeterHit* h) { return h == nullptr; }),
      selectedHits.end());
...
EVENT::MCParticle* getHitEarliestMC(EVENT::CalorimeterHit* hit,
  ...
  if (navToSimCalorimeterHits.getRelatedToObjects(hit).empty())
    return nullptr;
  ...
  EVENT::SimCalorimeterHit* simHit =
      static_cast<EVENT::SimCalorimeterHit*>(navToSimCalorimeterHits.getRelatedToObjects(hit)[max_i]);
...
float getTofFrankAvg(const std::vector<EVENT::CalorimeterHit*>& selectedHits, const Vector3D& posAtEcal,
...
float getTofFrankFit(const std::vector<EVENT::CalorimeterHit*>& selectedHits, const Vector3D& posAtEcal,
                     float timeResolution) {
  float tof = 0.;
  if (selectedHits.empty())
    return tof;
...
std::tuple<float, float> getTofSET(EVENT::Track* track, float timeResolution) {
  ...
  if (stripObjects.empty())
    return std::tuple(0.f, 0.f);
  else if (stripObjects.size() == 1) {
    streamlog_out(WARNING) << "Found only one SET strip hit, how is this possible!? Writing TOF from a single strip."
                           << std::endl;
  ...
  if (stripObjects.size() > 2)
    streamlog_out(WARNING) << "Found more than two SET strip hits, how is this possible!?" << std::endl;
...
float getTofPhotonTrue(EVENT::MCParticle* mc) {
  ...
  if (mc->getPDG() != 22 || mc->isDecayedInTracker())
    return 0.;

...
public:
  ...
  RombIntSolver(double (T::*fce)(double), T* pTemplate)
      : _eps(EPS_R), _nStepsMin(NSTEPSMIN_R), _nStepsMax(NSTEPSMAX_R), _fce(fce), _pTemplate(pTemplate) {
    ;
  }
  ...
  RombIntSolver(double (T::*fce)(double), T* pTemplate, float eps)
      : _eps(eps), _nStepsMin(NSTEPSMIN_R), _nStepsMax(NSTEPSMAX_R), _fce(fce), _pTemplate(pTemplate) {
    ;
  }
...
double RombIntSolver<T>::Integrate(double endPointA, double endPointB) {
  ...
  for (n = 0; n < _nStepsMax; n++) {
    ...
    // Find out precision
    if ((n >= _nStepsMin) && ((fabs(R[n][n] - R[n][n - 1]) / R[n][n]) < _eps))
      return R[n][n];
    //    std::cout << std::endl;
  }
...
double RombIntSolver<T>::TrapezRuleInt(int n) {
  ...
  // n-th iteration
    ...
    x = _a + h;
    for (subSum = 0, i = 0; i < (iN / 2); i++, x += h2)
      subSum += FCE(x);

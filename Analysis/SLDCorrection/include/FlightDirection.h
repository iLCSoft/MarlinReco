...
void getTrueFlightDirection( const MCP &SLDLepton , TVector3 &trueFlightDirection , DoubleVector &trueStartVertex , DoubleVector &trueSLDVertex ); //gives the true flight direction of parent hadron: trueFlightDirection and the position of start and end vertex of the hadron decays semi-leptonically: trueStartVertex & trueSLDVertex
...
// reconstructs the flight direction of parent hadron and returnes the status of the charged lepton: 
...
//	finds the minimum distance between a track (track) and a straight line (passing from pointOnLine with the direction of momentumOfLine). position of intersection point on the track (PCAatTrack) and the line (PCAatLine) are found

double intersectTrackTrack( const Trk &track1 , const Trk &track2 , TVector3 &PCAatTrack1 , TVector3 &PCAatTrack2 );
//	finds the minimum distance between two tracks (track1 & track2). position of intersection point on each track (PCAatTrack1 & PCAatTrack2) are found

double get3DImpactParameter( const Trk &track , const Vtx &primaryVertex , TVector3 &PCAatTrack );
//	gives the 3D distance of a track (track) from the primary vertex. Point of the closest approach on the track (PCAatTrack) is found
...
//	gives the 3D distance of a line passing from a point (pointOnLine) with direction (momentumOfLine) from the primary vertex. Point of the closest approach on the line (PCAatLine) is found

void drawMCParticles( const MCP &motherHadron , const MCP &mcParticle );
//	draws a MCParticle in event display
void drawReconstructedParticle( const RecoParticle &RecoParticle , const Vtx &primaryVertex , int colorCharged , int colorNeutral );
//	draws a Reconstructed Particle in event display

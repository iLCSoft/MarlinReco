...
IsoLepTrainingProcessor::IsoLepTrainingProcessor() : Processor("IsoLepTrainingProcessor") {
  ...
  registerInputCollection(LCIO::MCPARTICLE, "InputMCParticlesCollection", "Name of the MCParticle collection", _colMCP,
                          std::string("MCParticlesSkimmed"));

  registerInputCollection(LCIO::LCRELATION, "InputMCTruthLinkCollection", "Name of the MCTruthLink collection",
                          _colMCTL, std::string("RecoMCTruthLink"));
  ...
  registerProcessorParameter("IsLepTune", "Is lepton tune?", _is_lep_tune, bool(true));

  registerProcessorParameter("MCDebugging", "set true if you want to check generator information", _mcdebug,
                             bool(false));

  registerProcessorParameter("IsForSignal", "Is for signal?", _is_for_sig, bool(true));

  registerProcessorParameter("IsoLepType", "Isolated Lepton Type", _iso_lep_type, int(13));
}
...
void IsoLepTrainingProcessor::processRunHeader(LCRunHeader*) { _nRun++; }
...
void IsoLepTrainingProcessor::processEvent(LCEvent* evt) {
  ...
  for (Int_t i = 0; i < nMCP; i++) {
    ...
    //    if (pdg==pdgLepMC && (orig==origw||orig==pdgLepMC) && status==1 && TMath::Abs(pdg)==_iso_lep_type &&
    //    !(mcPart->isOverlay())) { if (status == 1 && TMath::Abs(pdg)==_iso_lep_type && TMath::Abs(orig) == 13 &&
    //    !(mcPart->isOverlay())) {
    if (i == 0) {
      serialLepMC = i;
...
void IsoLepTrainingProcessor::end() {
  ...
  cerr << "IsoLepTrainingProcessor::end()  " << name() << " processed " << _nEvt << " events in " << _nRun << " runs "
       << endl;
}

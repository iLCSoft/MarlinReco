...
FPCCDClustering::FPCCDClustering() : Processor("FPCCDClustering") {
  ...
  registerProcessorParameter( "EnrgyDigitizatoin",
...
void FPCCDClustering::modifyEvent( LCEvent * evt )
{
  ...
  if( pHitCol != 0 ){    
    ...
    if( nhit > 0 ) {  // Output Trackhit, if there are pixel hits
      LCCollectionVec* trkHit=new LCCollectionVec( LCIO::TRACKERHIT );
      makeTrackerHitVec(theData, *trkHit);
      evt->addCollection( trkHit, _outColNameVTX ) ;
...
// Set the random noise hit
      if( _randomNoise && _energyDigitization){
        ...
        for(unsigned int i=0; i< gsl_ran_poisson( _rng, (sxiwidth/_pixelSize)*(2*hlength/_pixelSize)*3.1671*1e-5) ; i++){

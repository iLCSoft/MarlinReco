...
IsolatedLeptonTaggingProcessor::IsolatedLeptonTaggingProcessor() : Processor("IsolatedLeptonTaggingProcessor") {
  ...
  registerInputCollection(LCIO::VERTEX, "InputPrimaryVertexCollection", "Name of the Primary Vertex collection",
                          _colPVtx, std::string("PrimaryVertex"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputPFOsWithoutIsoLepCollection",
                           "Name of the new PFOs collection without isolated lepton", _colNewPFOs,
                           std::string("PandoraPFOsWithoutIsoLep"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputIsoLeptonsCollection",
                           "Name of collection with the selected isolated lepton", _colLeptons,
  ...
  registerProcessorParameter("DirOfISOElectronWeights",
                             ...
                             _isolated_electron_weights, std::string("isolated_electron_weights"));

  registerProcessorParameter("DirOfISOMuonWeights", "Directory of Weights for the Isolated Muon MVA Classification",
                             _isolated_muon_weights, std::string("isolated_muon_weights"));

  registerProcessorParameter("CutOnTheISOElectronMVA", "Cut on the mva output of isolated electron selection",
                             _mvaCutForElectron, float(0.5));

  registerProcessorParameter("CutOnTheISOMuonMVA", "Cut on the mva output of isolated muon selection", _mvaCutForMuon,
                             float(0.7));

  registerProcessorParameter("IsSelectingOneIsoLep", "flag to select one most like isolated lepton", _is_one_isolep,
                             bool(true));

  registerProcessorParameter("MinEOverPForElectron",
                             "minimum ratio of energy in calorimeters over momentum for electron",
                             _minEOverPForElectron, float(0.5));

  registerProcessorParameter("MaxEOverPForElectron",
                             "Maximum ratio of energy in calorimeters over momentum for electron",
                             _maxEOverPForElectron, float(1.3));

  registerProcessorParameter("MinEecalOverTotEForElectron", "minimum ratio of energy in ecal over energy in ecal+hcal",
                             _minEecalOverTotEForElectron, float(0.9));

  registerProcessorParameter("MaxD0SigForElectron", "Maximum d0 significance for electron", _maxD0SigForElectron,
                             float(50.));

  registerProcessorParameter("MaxZ0SigForElectron", "Maximum Z0 significance for electron", _maxZ0SigForElectron,
                             float(5.));

  registerProcessorParameter("MinPForElectron", "Minimum momentum for electron", _minPForElectron, float(5.));

  registerProcessorParameter("MaxEOverPForMuon", "Maximum ratio of energy in calorimeters over momentum for muon",
                             _maxEOverPForMuon, float(0.3));

  registerProcessorParameter("MinEyokeForMuon", "Minimum energy in yoke for electron", _minEyokeForMuon, float(1.2));

  registerProcessorParameter("MaxD0SigForMuon", "Maximum D0 significance for muon", _maxD0SigForMuon, float(5.));

  registerProcessorParameter("MaxZ0SigForMuon", "Maximum Z0 significance for muon", _maxZ0SigForMuon, float(5.));

  registerProcessorParameter("MinPForMuon", "Minimum momentum for muon", _minPForMuon, float(5.));

  registerProcessorParameter("CosConeSmall", "cosine of the smaller cone", _cosConeSmall, float(0.98));

  registerProcessorParameter("CosConeLarge", "cosine of the larger cone", _cosConeLarge, float(0.95));

  registerProcessorParameter("UseYokeForMuonID", "use yoke for muon ID", _use_yoke_for_muon, bool(true));

  registerProcessorParameter("UseIP", "use impact parameters", _use_ip, bool(true));
}

void IsolatedLeptonTaggingProcessor::init() {

  streamlog_out(DEBUG) << "IsolatedLeptonTagging   init called  " << std::endl;
...
void IsolatedLeptonTaggingProcessor::processEvent(LCEvent* evt) {
  ...
  for (Int_t i = 0; i < nPFOs; i++) {
    ...
    if (ecalEnergy > 0.)
      ratioECal = ecalEnergy / totalCalEnergy;
    ...
    if (charge != 0 && totalCalEnergy / momentumMagnitude > _minEOverPForElectron &&
      ...
      if (TMath::Abs(nsigd0) < _maxD0SigForElectron &&
          TMath::Abs(nsigz0) < _maxZ0SigForElectron) { // contraint to primary vertex
        mva_electron = _readers[0]->EvaluateMVA("MLP method");
      }
    }
    if (charge != 0 && totalCalEnergy / momentumMagnitude < _maxEOverPForMuon &&
        (momentumMagnitude > _minPForMuon)) { // basic muon ID, should be replaced by external general PID
      if (TMath::Abs(nsigd0) < _maxD0SigForMuon &&
          TMath::Abs(nsigz0) < _maxZ0SigForMuon) { // contraint to primary vertex
        if (_use_yoke_for_muon && yokeEnergy > _minEyokeForMuon) {
          mva_muon = _readers[1]->EvaluateMVA("MLP method");
        } else { // temporarily, provide this option for muon ID without using energy in Yoke; default option for now
                 // before problems get fixed
          mva_muon = _readers[1]->EvaluateMVA("MLP method");
        }
      }
    }
...
void IsolatedLeptonTaggingProcessor::addOutputColls(LCEvent* evt, LCCollection* pfosWithoutIsoLepColl,

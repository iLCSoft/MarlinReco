...
Sphere::Sphere() : Processor("Sphere") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "CollectionName", "Name of the ReconstructedParticle collection",
                          _colName, std::string("RecontructedParticle"));

  registerProcessorParameter("r_value", " exponent in sphericity tensor use 2.0 for classical 1.0 for C,D", _r,
                             float(2.0));
  registerProcessorParameter("eigenvalues_parameter_name", "name of parameter to store the results ", _dumpobjectname,
...
void Sphere::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void Sphere::processEvent(LCEvent* evt) {
  ...
  try {
    ...
    streamlog_out(DEBUG) << "Cannot find PFO Collection in event/run  " << evt->getEventNumber() << " / "
  ...
  if (col->getNumberOfElements() == 0) {
    streamlog_out(DEBUG) << "PFO Collection is empty in event/run  " << evt->getEventNumber() << " / "
                         << evt->getRunNumber() << std::endl;
    return;
  }

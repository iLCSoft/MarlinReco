...
std::vector<std::string> InputAlgorithm_LeptonID::init(const std::vector<float>& inparF,
                                                       const std::vector<std::string>& inparS) {
  print_init(inparF, inparS);

  if (inparS.size() > 0)
    _LeptonIDAlgoName = inparS[0];
  else
    _LeptonIDAlgoName = "LeptonID";

  std::vector<std::string> obsNames{"elness", "muness", "piness"};
  return obsNames;
}

std::vector<std::pair<float, float>> InputAlgorithm_LeptonID::extractObservables(ReconstructedParticleImpl* pfo,
  ...
  for (unsigned int i = 0; i < LeptonIDParams.size() && i < 3; ++i)
    obsValues[i] = std::pair<float, float>{LeptonIDParams[i], 0};

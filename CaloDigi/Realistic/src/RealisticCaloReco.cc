...
RealisticCaloReco::RealisticCaloReco() : Processor("RealisticCaloReco") {
  ...
  registerInputCollections(LCIO::CALORIMETERHIT, "inputHitCollections", "input hit collection names",
                           _inputHitCollections, inputHitCollections);

  std::vector<std::string> inputRelCollections;
  registerInputCollections(LCIO::LCRELATION, "inputRelationCollections",
  ...
  registerProcessorParameter("outputHitCollections", "output hit collection names", _outputHitCollections,
                             outputHitCollections);

  std::vector<std::string> outputRelCollections;
  registerProcessorParameter("outputRelationCollections", "output hit collection names", _outputRelCollections,
                             outputRelCollections);

  std::vector<int> calLayers;
  registerProcessorParameter("calibration_layergroups", "grouping of calo layers", _calLayers, calLayers);

  std::vector<float> calibrCoeff;
  registerProcessorParameter("calibration_factorsMipGev", "Calibration coefficients (MIP->shower GeV) of layers groups",
                             _calibrCoeff, calibrCoeff);
      ...
      for (int j(0); j < numElements; ++j) {
        ...
        if (navi.getRelatedToObjects(hit).size() > 0) {
          SimCalorimeterHit* simhit =
              (SimCalorimeterHit*)navi.getRelatedToObjects(hit)[0]; // assume the first one (should be only one)

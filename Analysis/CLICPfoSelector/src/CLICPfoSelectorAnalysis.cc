...
CLICPfoSelectorAnalysis::CLICPfoSelectorAnalysis() : Processor("CLICPfoSelectorAnalysis") {
  ...
  registerProcessorParameter("AnalyzePhotons", "Boolean factor to decide if perform the analysis on photons",
                             analyzePhotons, bool(true));
  ...
  registerProcessorParameter(
      "AnalyzeOverlay", "Boolean factor to decide if perform the analysis on only on PFOs belonging to the overlay",
...
void CLICPfoSelectorAnalysis::init() {
  ...
  for (auto ipc : particleCategories) {
    for (auto igc : generationCategories) {
      ...
      h_energy_central[graphLabel] = new TH1F((graphLabel + "_energy_central").c_str(),
                                              (graphLabel + "_energy_central").c_str(), 1000, en_min, en_max);
      h_energy_forward[graphLabel] = new TH1F((graphLabel + "_energy_forward").c_str(),
                                              (graphLabel + "_energy_forward").c_str(), 1000, en_min, en_max);
    }
  }
}
...
void CLICPfoSelectorAnalysis::processEvent(LCEvent* evt) {
  streamlog_out(DEBUG) << "CLICPfoSelectorAnalysis ---> processing run = " << _nRun << " event = " << _nEvt
                       << std::endl;
...
void CLICPfoSelectorAnalysis::fillTree(LCEvent* evt, string collName) {
  ...
  if (col != NULL) {
    ...
    for (int i = 0; i < nelem; i++) {
      ...
      } else if (charge != 0 && std::find(particleCategories.begin(), particleCategories.end(), "chargedPfos") !=
                                    particleCategories.end()) {
        h_label = "chargedPfos";
      }
      ...
      if (!atLeastOneSignal && std::find(generationCategories.begin(), generationCategories.end(), "overlay") !=
                                   generationCategories.end()) {
  ...
  for (auto ipc : particleCategories) {
    for (auto igc : generationCategories) {
      ...
      streamlog_out(DEBUG) << " Energy (" << graphLabel << " - central): " << energy_tot_central[graphLabel]
                           << std::endl;
      streamlog_out(DEBUG) << " Energy (" << graphLabel << " - forward): " << energy_tot_forward[graphLabel]
                           << std::endl;
    }
  }
}
  ...
  for (int ie = 0; ie < nEntries; ie++) {
    ...
    if (analyzePhotons == true && type == 22) {
      if (std::find(particleCategories.begin(), particleCategories.end(), "photons") != particleCategories.end() &&
        ...
        // streamlog_out( DEBUG ) << "Filling scatter plot for photon with pT and current clusterTime: " << pT << "," <<
        // currentClusterTime << endl;

        g_timeVsPt["photons_all"]->SetPoint(g_timeVsPt["photons_all"]->GetN(), pT, currentClusterTime);
        if (costheta < cutCosTheta) {
          g_timeVsPt_central["photons_all"]->SetPoint(g_timeVsPt_central["photons_all"]->GetN(), pT,
                                                      currentClusterTime);
        } else {
          g_timeVsPt_forward["photons_all"]->SetPoint(g_timeVsPt_forward["photons_all"]->GetN(), pT,
                                                      currentClusterTime);
        }

        if (analyzeSignal && atLeastOneSignal &&
            std::find(generationCategories.begin(), generationCategories.end(), "signal") !=
                generationCategories.end()) {
          g_timeVsPt["photons_signal"]->SetPoint(g_timeVsPt["photons_signal"]->GetN(), pT, currentClusterTime);
          if (costheta < cutCosTheta) {
            g_timeVsPt_central["photons_signal"]->SetPoint(g_timeVsPt_central["photons_signal"]->GetN(), pT,
                                                           currentClusterTime);
          } else {
            g_timeVsPt_forward["photons_signal"]->SetPoint(g_timeVsPt_forward["photons_signal"]->GetN(), pT,
                                                           currentClusterTime);
          }
        }

        if (analyzeOverlay && !atLeastOneSignal &&
            std::find(generationCategories.begin(), generationCategories.end(), "overlay") !=
                generationCategories.end()) {
          g_timeVsPt["photons_overlay"]->SetPoint(g_timeVsPt["photons_overlay"]->GetN(), pT, currentClusterTime);
          if (costheta < cutCosTheta) {
            g_timeVsPt_central["photons_overlay"]->SetPoint(g_timeVsPt_central["photons_overlay"]->GetN(), pT,
                                                            currentClusterTime);
          } else {
            g_timeVsPt_forward["photons_overlay"]->SetPoint(g_timeVsPt_forward["photons_overlay"]->GetN(), pT,
                                                            currentClusterTime);
          }
        }
    ...
    if (analyzeNeutralHadrons == true && type != 22 && charge == 0) {
      if (std::find(particleCategories.begin(), particleCategories.end(), "neutralHadrons") !=
              particleCategories.end() &&
          std::find(generationCategories.begin(), generationCategories.end(), "all") != generationCategories.end()) {
        ...
        // streamlog_out( DEBUG ) << "Filling scatter plot for neutralHadrons with pT and clusterTime: " << pT << "," <<
        // currentClusterTime << endl;

        g_timeVsPt["neutralHadrons_all"]->SetPoint(g_timeVsPt["neutralHadrons_all"]->GetN(), pT, currentClusterTime);
        if (costheta < cutCosTheta)
          g_timeVsPt_central["neutralHadrons_all"]->SetPoint(g_timeVsPt_central["neutralHadrons_all"]->GetN(), pT,
                                                             currentClusterTime);
        else
          g_timeVsPt_forward["neutralHadrons_all"]->SetPoint(g_timeVsPt_forward["neutralHadrons_all"]->GetN(), pT,
                                                             currentClusterTime);

        if (analyzeSignal && atLeastOneSignal &&
            std::find(generationCategories.begin(), generationCategories.end(), "signal") !=
                generationCategories.end()) {
          g_timeVsPt["neutralHadrons_signal"]->SetPoint(g_timeVsPt["neutralHadrons_signal"]->GetN(), pT,
                                                        currentClusterTime);
          if (costheta < cutCosTheta)
            g_timeVsPt_central["neutralHadrons_signal"]->SetPoint(g_timeVsPt_central["neutralHadrons_signal"]->GetN(),
                                                                  pT, currentClusterTime);
          else
            g_timeVsPt_forward["neutralHadrons_signal"]->SetPoint(g_timeVsPt_forward["neutralHadrons_signal"]->GetN(),
                                                                  pT, currentClusterTime);
        }
        ...
        if (analyzeOverlay && !atLeastOneSignal &&
            std::find(generationCategories.begin(), generationCategories.end(), "overlay") !=
                generationCategories.end()) {
          g_timeVsPt["neutralHadrons_overlay"]->SetPoint(g_timeVsPt["neutralHadrons_overlay"]->GetN(), pT,
                                                         currentClusterTime);
          if (costheta < cutCosTheta)
            g_timeVsPt_central["neutralHadrons_overlay"]->SetPoint(g_timeVsPt_central["neutralHadrons_overlay"]->GetN(),
                                                                   pT, currentClusterTime);
          else
            g_timeVsPt_forward["neutralHadrons_overlay"]->SetPoint(g_timeVsPt_forward["neutralHadrons_overlay"]->GetN(),
                                                                   pT, currentClusterTime);
        }
    ...
    if (analyzeChargedPfos == true && charge != 0) {
      if (std::find(particleCategories.begin(), particleCategories.end(), "chargedPfos") != particleCategories.end() &&
        ...
        // streamlog_out( DEBUG ) << "Filling scatter plot for chargedPfos with pT and clusterTime: " << pT << "," <<
        // currentClusterTime << endl;

        g_timeVsPt["chargedPfos_all"]->SetPoint(g_timeVsPt["chargedPfos_all"]->GetN(), pT, currentClusterTime);
        if (costheta < cutCosTheta)
          g_timeVsPt_central["chargedPfos_all"]->SetPoint(g_timeVsPt_central["chargedPfos_all"]->GetN(), pT,
                                                          currentClusterTime);
        else
          g_timeVsPt_forward["chargedPfos_all"]->SetPoint(g_timeVsPt_forward["chargedPfos_all"]->GetN(), pT,
                                                          currentClusterTime);
        ...
        if (analyzeSignal && atLeastOneSignal &&
          ...
          if (costheta < cutCosTheta)
            g_timeVsPt_central["chargedPfos_signal"]->SetPoint(g_timeVsPt_central["chargedPfos_signal"]->GetN(), pT,
                                                               currentClusterTime);
          else
            g_timeVsPt_forward["chargedPfos_signal"]->SetPoint(g_timeVsPt_forward["chargedPfos_signal"]->GetN(), pT,
                                                               currentClusterTime);
        }
        ...
        if (analyzeOverlay && !atLeastOneSignal &&
            std::find(generationCategories.begin(), generationCategories.end(), "overlay") !=
                generationCategories.end()) {
          g_timeVsPt["chargedPfos_overlay"]->SetPoint(g_timeVsPt["chargedPfos_overlay"]->GetN(), pT,
                                                      currentClusterTime);
          if (costheta < cutCosTheta)
            g_timeVsPt_central["chargedPfos_overlay"]->SetPoint(g_timeVsPt_central["chargedPfos_overlay"]->GetN(), pT,
                                                                currentClusterTime);
          else
            g_timeVsPt_forward["chargedPfos_overlay"]->SetPoint(g_timeVsPt_forward["chargedPfos_overlay"]->GetN(), pT,
                                                                currentClusterTime);
        }

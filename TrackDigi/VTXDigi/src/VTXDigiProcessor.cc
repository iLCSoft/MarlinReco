...
VTXDigiProcessor::VTXDigiProcessor() : Processor("VTXDigiProcessor") {
  ...
  registerProcessorParameter("SmearAlongLadders", "Points smeared along the ladders", _smearAlongLadders, int(1));

  registerProcessorParameter("PointResolutionRPhi_VTX", "R-Phi Resolution in VTX", _pointResoRPhi_VTX, float(0.0040));

  registerProcessorParameter("PointResolutionZ_VTX", "Z Resolution in VTX", _pointResoZ_VTX, float(0.0040));

  registerProcessorParameter("PointResolutionRPhi_SIT", "R-Phi Resolution in SIT", _pointResoRPhi_SIT, float(0.010));

  registerProcessorParameter("PointResolutionZ_SIT", "Z Resolution in SIT", _pointResoZ_SIT, float(0.010));

  registerProcessorParameter("PointResolutionRPhi_SET", "R-Phi Resolution in SET", _pointResoRPhi_SET, float(0.010));

  registerProcessorParameter("PointResolutionZ_SET", "Z Resolution in SET", _pointResoZ_SET, float(0.010));

  std::vector<int> activeSETLayers;
  activeSETLayers.push_back(1);

  registerProcessorParameter("ActiveSETLayers", "only SET hits from active layers are digitized (mimic stereo layers)",
                             _activeSETLayers, activeSETLayers);

  registerProcessorParameter("RemoveDrays", "Remove D-rays ?", _removeDRays, int(0));

  registerProcessorParameter("MomentumCutForDRays", "Momentum Cut For D Rays (MeV)", _momCut, float(10.0));
  ...
  registerProcessorParameter("HitEfficiencyPerLayer_VTX", "hit efficiencies per VXD layer (default: 1.0)", _vxdEff,
                             effDefault);
  ...
  registerInputCollection(LCIO::SIMTRACKERHIT, "SITCollectionName", "Name of the SIT SimTrackerHit collection",
                          _colNameSIT, std::string("SITCollection"));

  registerInputCollection(LCIO::SIMTRACKERHIT, "SETCollectionName", "Name of the SET SimTrackerHit collection",
                          _colNameSET, std::string("SETCollection"));

  // Output collections
  registerOutputCollection(LCIO::TRACKERHIT, "VTXHitCollection", "Name of the vxd TrackerHit output collection",
                           _outColNameVTX, std::string("VTXTrackerHits"));

  registerOutputCollection(LCIO::TRACKERHIT, "SITHitCollection", "Name of the sit TrackerHit output collection",
                           _outColNameSIT, std::string("SITTrackerHits"));

  registerOutputCollection(LCIO::TRACKERHIT, "SETHitCollection", "Name of the set TrackerHit output collection",
                           _outColNameSET, std::string("SETTrackerHits"));
}
...
void VTXDigiProcessor::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void VTXDigiProcessor::processEvent(LCEvent* evt) {
  ...
  for (int iColl = 0; iColl < 3; ++iColl) {
    ...
    try {
      ...
      if (iColl == 0) {
        streamlog_out(DEBUG) << "Collection " << _colNameVTX.c_str() << " is unavailable in event " << _nEvt
                             << std::endl;
      } else if (iColl == 1) {
        streamlog_out(DEBUG) << "Collection " << _colNameSIT.c_str() << " is unavailable in event " << _nEvt
                             << std::endl;
      } else {
        streamlog_out(DEBUG) << "Collection " << _colNameSET.c_str() << " is unavailable in event " << _nEvt
                             << std::endl;
      }
    }
      ...
      for (int i = 0; i < nSimHits; i++) {
        ...
        if (_removeDRays) { // check if hit originates from delta-electron
          ...
          if (totMomentum < _momCut) {
            streamlog_out(DEBUG) << " removeDRays enabled: hit originates from delta-electron, hit dropped"
                                 << std::endl;
            continue; // ----------------- ignore hit
          }
        }
        ...
        if (iColl == 0) {
          ...
          streamlog_out(DEBUG) << " processing collection " << _colNameVTX << " with " << nSimHits << " hits ... "
                               << std::endl;
          ...
          streamlog_out(DEBUG) << "Position of hit before smearing = " << pos[0] << " " << pos[1] << " " << pos[2]
                               << endl;
          ...
          if (_smearAlongLadders != 0 && layerVXD.getNLadders(0) != 0) {
            ...
            for (int ic = 0; ic < layerVXD.getNLadders(hitLayer); ++ic) {
              ...
              if (RXY * cos(PhiInLocal) - layerVXD.getSensitiveDistance(hitLayer) >
                      -layerVXD.getSensitiveThickness(hitLayer) &&
                  RXY * cos(PhiInLocal) - layerVXD.getSensitiveDistance(hitLayer) <
                      layerVXD.getSensitiveThickness(hitLayer)) {
                ladderIndex = ic;
                break;
              }
            ...
            streamlog_out(DEBUG) << ":"
                                 << " Event: " << _nEvt << " hit: " << i << " of " << nSimHits
                                 << "  layer: " << hitLayer << "  ladderIndex: " << ladderIndex
                                 << "  half ladder width " << sensitive_width * 0.5 << "  u: " << u
                                 << "  layer sensitive_offset " << sensitive_offset << "  layer phi0 "
                                 << layerVXD.getPhi0(hitLayer) << "  phi: " << hitvec.phi()
                                 << "  PhiInLocal: " << PhiInLocal << "  ladderPhi: " << ladderPhi
                                 << "  ladder_incline: " << ladder_incline << std::endl;

            if (u > sensitive_width * 0.5 || u < -sensitive_width * 0.5) {
              streamlog_out(DEBUG) << "hit not in sensitive: u: " << u
            ...
            while (tries < 100) {
              ...
              if (tries > 0)
                streamlog_out(DEBUG) << "retry smearing for " << hitLayer << " " << ladderIndex << " : retries "
                                     << tries << std::endl;
            ...
            if (accept_hit == false) {
              streamlog_out(DEBUG) << "hit could not be smeared within ladder after 100 tries: hit dropped"
                                   << std::endl;
              continue;
            } //
          }
...
void VTXDigiProcessor::end() {
  ...
  for (unsigned i = 0; i < _vxdCount.size(); ++i) {
    ...
    streamlog_out(DEBUG) << "     layer " << i << " kept " << _vxdCount[i].first << " hits of " << _vxdCount[i].second
                         << " -> eff = " << double(_vxdCount[i].first) / double(_vxdCount[i].second) << std::endl;
  }
}

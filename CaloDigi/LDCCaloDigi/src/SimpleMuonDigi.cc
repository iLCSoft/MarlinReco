...
SimpleMuonDigi::SimpleMuonDigi() : Processor("SimpleMuonDigi") {
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "MUONCollections", "Muon Collection Names", _muonCollections,
                           muonCollections);

  registerOutputCollection(LCIO::CALORIMETERHIT, "MUONOutputCollection", "Muon Collection of real Hits",
                           _outputMuonCollection, std::string("MUON"));

  registerOutputCollection(LCIO::LCRELATION, "RelationOutputCollection", "CaloHit Relation Collection",
                           _outputRelCollection, std::string("RelationMuonHit"));

  registerProcessorParameter("MuonThreshold", "Threshold for Muon Hits in GeV", _thresholdMuon, (float)0.025);

  registerProcessorParameter("CalibrMUON", "Calibration coefficients for MUON", _calibrCoeffMuon, (float)120000.);

  registerProcessorParameter("MaxHitEnergyMUON", "maximum hit energy for a MUON hit", _maxHitEnergyMuon, (float)2.0);

  IntVec keepBarrelLayersVec, keepEndcapLayersVec;

  registerProcessorParameter("KeepBarrelLayersVec", "Vector of Barrel layers to be kept. Layers start at 1!",
                             _layersToKeepBarrelVec, keepBarrelLayersVec);

  registerProcessorParameter("KeepEndcapLayersVec", "Vector of Endcap layers to be kept. Layers start at 1!",
                             _layersToKeepEndcapVec, keepEndcapLayersVec);
...
void SimpleMuonDigi::processEvent(LCEvent* evt) {
  ...
  streamlog_out(DEBUG) << " process event : " << evt->getEventNumber() << " - run  " << evt->getRunNumber()
                       << std::endl;

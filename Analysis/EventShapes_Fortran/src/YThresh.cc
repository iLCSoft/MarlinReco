...
YThresh::YThresh() : Processor("YThresh") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "RecoParticleCollection",
                          "Name of the input ReconstructedParticle collection", _inputCollection,
                          std::string("RecoParticles"));

  registerProcessorParameter("NMin", "min number of jets, ie. NMin=2 will give y23 variable", _nMin, int(2));

  registerProcessorParameter("PrintOutput", "toggle print text output", _output, int(0));

  registerProcessorParameter("NIterations", "number of iterations", _nIter, int(20));

  registerProcessorParameter("NMinParticles", "min num particles for ythresh calculation", _nMinParticles, int(3));

  registerProcessorParameter("YStart", "starting value for yCut", _yStart, float(0.1));
}
...
void YThresh::processRunHeader(LCRunHeader* run) { _nRun++; }
...
void YThresh::processEvent(LCEvent* evt) {
  ...
  try {
    ...
    if (_nRecoParticles >= _nMinParticles) {
      ...
      do {
        callSatoru(evt);
        if (_jetsWorkArray.NumberOfJets <= _nMin)
          break;
        else
          _yCut[0] = _yCut[0] * 2;
      } while (true);
...
void YThresh::callSatoru(LCEvent* evt) {
  ...
  syjkrn_(_globalMode.c_str(), _nJetRequested, _threshold, _primaryJetFindingMode, _yCut, _mergingMode,
          ...
          _partonsWorkArray.PointerParticleToJets, DimensionOfOutputArray, _jetsWorkArray.Momentum, YMinus, YPlus,
...
void YThresh::end() {
  ...
  streamlog_out(DEBUG4) << "YThresh::end()  " << name() << " processed " << _nEvt << " events in " << _nRun << " runs "
                        << std::endl;
}

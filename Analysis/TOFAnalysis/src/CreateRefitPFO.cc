...
CreateRefitPFO::CreateRefitPFO() : marlin::Processor("CreateRefitPFO") {
  _description = "Create a copy of PandoraPFO collection, where for each Kaon/Proton particle the tracks are "
                 "substituted from MarlinTrkTracksKaon and MarlinTrkTracksProton colelctions respectively. This "
                 "simulates scenario as all pi/k/p tracks are fitted with their true mass assuming perfect PID.";
}

void CreateRefitPFO::init() { _bField = MarlinUtil::getBzAtOrigin(); }
...
void CreateRefitPFO::processEvent(EVENT::LCEvent* event) {
  ...
  for (int i = 0; i < pfos->getNumberOfElements(); ++i) {
    ...
    for (int j = 0; j < nTracks; ++j) {
      ...
      if (std::abs(pdg) == 321)
        track = static_cast<Track*>(tracksKaonRefitCol->getElementAt(idx));
      else if (std::abs(pdg) == 2212)
        track = static_cast<Track*>(tracksProtonRefitCol->getElementAt(idx));
      tracksPdg.push_back(pdg);
      refittedTracks.push_back(track);
    }

    // create new PFO substituting tracks and 4mom and cov matrix in simple cases. Everything else just copy from the
    // old pfo
    ReconstructedParticleImpl* outputPFO = new ReconstructedParticleImpl();
    for (size_t j = 0; j < pfo->getTracks().size(); ++j)
      outputPFO->addTrack(refittedTracks[j]);

    if (nTracks == 1 && (std::abs(tracksPdg[0]) == 321 || std::abs(tracksPdg[0]) == 2212)) {
      double mass = 0.13957039;
      if (std::abs(tracksPdg[0]) == 321)
        mass = 0.493677;
      else if (std::abs(tracksPdg[0]) == 2212)
        mass = 0.938272088;
      ...
      if (std::abs(tracksPdg[1]) == 321)
        mass2 = 0.493677;
      else if (std::abs(tracksPdg[1]) == 2212)
        mass2 = 0.938272088;
      ...
      for (size_t j = 0; j < covMatrix1.size(); ++j)
        covMatrix.push_back(covMatrix1[j] + covMatrix2[j]);
    ...
    for (size_t j = 0; j < pfo->getParticles().size(); ++j)
      outputPFO->addParticle(pfo->getParticles()[j]);
    for (size_t j = 0; j < pfo->getClusters().size(); ++j)
      outputPFO->addCluster(pfo->getClusters()[j]);
    for (size_t j = 0; j < pfo->getParticleIDs().size(); ++j) {
      ...
      for (size_t k = 0; k < inPID->getParameters().size(); ++k)
...
std::vector<float> CreateRefitPFO::updateChargedPFOCovMat(EVENT::Track* track, double mass) {
  ...
  double jacobianByRows[rows * columns] = {0, 0, pt,          pz * pt / energy, -py,         px,
                                           0, 0, -px / omega, -py / omega,      -pz / omega, -p * p / (energy * omega)};
  ...
  double trackCovMatByRows[rows * rows] = {flatTrackCovMat[14], flatTrackCovMat[11], flatTrackCovMat[12],

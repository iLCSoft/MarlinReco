...
int getRecoFlightDirection( const RecoParticle &linkedRecoLepton , TVector3 &recoFlightDirection , double &hadronFlightLength , const Vtx &primaryVertex , const Vtx &startVertex , Vtx &SLDVertex , RecoParticle &SLDVertexRP , const RecoParticle &assignedJet , const VertexVector &verticesInJet , const PFOVector &PFOswithAloneTracks , float &helicesDistance , int vertexingScenario , TVector3 &daughterHadronFlightDirection , double &daughterHadronFlightDistance , DoubleVector &sldVertexPosition , TVector3 &PCAatLepton , TVector3 &PCAatOtherParticle , double &lepton3DImpactParameter , double &OtherParticle3DImpactParameter )
{
	int SLDStatus = -999;
	...
	else if ( verticesInJet.size() != 0 )
	{
		...
		for ( unsigned int i_vtx = 0 ; i_vtx < verticesInJet.size() ; ++i_vtx )
		{
			...
			DoubleVector pointOnLine{}; pointOnLine.clear();
...
void drawReconstructedParticle( const RecoParticle &reconstructedParticle , const Vtx &primaryVertex , int colorCharged , int colorNeutral )
{
	double m_Bfield = MarlinUtil::getBzAtOrigin();

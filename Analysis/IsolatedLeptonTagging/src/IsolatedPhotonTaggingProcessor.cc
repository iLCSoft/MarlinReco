...
IsolatedPhotonTaggingProcessor::IsolatedPhotonTaggingProcessor() : Processor("IsolatedPhotonTaggingProcessor") {
  ...
  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputPFOsWithoutIsoPhotonCollection",
                           "Name of the new PFOs collection without isolated photon", _colNewPFOs,
                           std::string("PandoraPFOsWithoutIsoPhoton"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputIsoPhotonsCollection",
                           "Name of collection with the selected isolated photon", _colPhotons,
                           std::string("ISOPhotons"));

  registerProcessorParameter("DirOfISOPhotonWeights", "Directory of Weights for the Isolated Photon MVA Classification",
                             _isolated_photon_weights, std::string("isolated_photon_weights"));

  registerProcessorParameter("CutOnTheISOPhotonMVA", "Cut on the mva output of isolated photon selection", _mvaCut,
                             float(0.5));

  registerProcessorParameter("IsSelectingOneIsoPhoton", "flag to select one most like isolated photon",
                             _is_one_isophoton, bool(false));

  registerProcessorParameter("MinEForPhoton", "Minimum energy for photon", _minE, float(5.));
  ...
  registerProcessorParameter("RatioNeutralConeEnergy", "Cut on the ratio of Neutral Cone Energy over Photon Energy",
                             _coneNeutralRatio, float(0.2));

  registerProcessorParameter("RatioChargedConeEnergy", "Cut on the ratio of Charged Cone Energy over Photon Energy",
                             _coneChargedRatio, float(0.1));
}

void IsolatedPhotonTaggingProcessor::init() {

  streamlog_out(DEBUG) << "IsolatedPhotonTagging   init called  " << std::endl;
...
void IsolatedPhotonTaggingProcessor::processEvent(LCEvent* evt) {
  ...
  for (Int_t i = 0; i < nPFOs; i++) {
    ...
    if (TMath::Abs(charge) < 0.5 && pandoraID == 22) {
      if (energy < _minE)
        continue; // cut on the minimum energy
      // default option to select all isolated photons
      if (coneEC / energy < _coneChargedRatio &&
          coneEN / energy < _coneNeutralRatio) { // simple charged and neutral cone energy ratio cut

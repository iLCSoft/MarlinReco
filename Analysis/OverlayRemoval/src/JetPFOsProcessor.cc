...
JetPFOsProcessor::JetPFOsProcessor() : Processor("JetPFOsProcessor") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputJetCollection", "Name of the input jet collection",
                          _colJet, std::string("JetsFromKt"));

  registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "OutputPFOsCollection", "Name of the output PFOs collection",
                           _colPFOsFromJet, std::string("PFOsFromKtJet"));
}

void JetPFOsProcessor::init() { streamlog_out(DEBUG) << "   JetPFOs Processor init called  " << std::endl; }

...
Histogram::Histogram(std::string HistName, int Dimension, std::string* VarName, double* StartVal, double* BinWidth,
                     int* NoOfBins) {
  dim = Dimension;
  if (dim < 1) {
...
int Histogram::Fill(VObject* VO) {
  ...
  for (int i = 0; i < dim; i++) {
    ...
    str += varNames[i];
    str += "]";
    std::cerr << std::setw(30) << std::left << str << std::setw(3) << " = " << std::setw(10) << std::left << val
              << std::endl;
  ...
  content[mind] += 1.;
  norm += 1.;
  if (content[mind] / vol > density[0])
    density[0] = content[mind] / vol;
  double mini = 1.e100;
  for (int i = 0; i < totNoOfBins; i++)
    if (content[i] < mini)
      mini = content[i];
...
int Histogram::SetBinContent(int BinInd, double Value) {
  ...
  norm += Value;
  if (content[BinInd] / vol > density[0])
    density[0] = content[BinInd] / vol;
  double mini = 1.e100;
  for (int i = 0; i < totNoOfBins; i++)
    if (content[i] < mini)
      mini = content[i];
...
double Histogram::GetNorm() { return norm; }

double Histogram::GetVolume() { return vol; }

std::string Histogram::GetHistName() { return histName; }

double* Histogram::GetDensities() { return density; }
...
int Histogram::GetDimension() { return dim; }

int Histogram::GetTotNoOfBins() { return totNoOfBins; }

std::string* Histogram::GetVarNames() { return varNames; }

double* Histogram::GetVarStart() { return start; }

double* Histogram::GetVarWidth() { return width; }

int* Histogram::GetVarBins() { return bins; }
...
double Histogram::CompareHist(Histogram* hist) {
  ...
  for (int i = 0; i < dim; i++) {
    ...
    if (fabs(width[i] - hist->GetVarWidth()[i]) > 1.e-10) {
      std::cout << " Histogramms aren't compatible - widthes differ (at least in order) " << width[i] << "  "
                << hist->GetVarWidth()[i] << std::endl;
      return -1;
    }

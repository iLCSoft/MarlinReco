...
WWCategorisationProcessor::WWCategorisationProcessor() : Processor("WWCategorisationProcessor") {
  ...
  registerInputCollection(LCIO::MCPARTICLE, "MCParticles", "Name of the MCParticle collection", _MCParColName,
                          std::string("MCParticlesSkimmed"));
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "IsolatedElectrons",
                          "Name of the ReconstructedParticle collection", _ElectronColName,
                          std::string("IsolatedElectrons"));
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "IsolatedMuons", "Name of the ReconstructedParticle collection",
                          _MuonColName, std::string("IsolatedMuons"));
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "IsolatedTaus", "Name of the ReconstructedParticle collection",
                          _TauColName, std::string("IsolatedTaus"));
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "IsolatedPhotons",
                          "Name of the ReconstructedParticle collection", _PhotonColName,
                          std::string("IsolatedPhotons"));
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "PFOsminusphoton",
                          "Name of the ReconstructedParticle collection", _PFOsMinusIsolatedObjetcs,
                          std::string("PFOsminusphoton"));

  registerProcessorParameter(
      "MCElCat_CosThCut",
      "Cut using cos(theta) value of MC electrons, above which sl-electron events are categorised as 'invisible' "
      "(true_cat 1 instead of 2); default: 0.994 (detector acceptance).",
      _MCElCat_CosThCut, float(0.994));
  registerProcessorParameter("TTreeFileName",
                             "Name of the root file in which the TTree with the 4 event observables is stored; if left "
                             "empty no root file is created; default: WWCategorisationTTreeFile.root",
                             _TTreeFileName, std::string("WWCategorisationTTreeFile.root"));
  registerProcessorParameter("ConfusionMatrixFileName",
                             "Name of the png file in which the confusion matrix is stored; if left empty no file is "
                             "created; default: WWCategorisationConfusionMatrix.png",
                             _ConfusionMatrixFileName, std::string("WWCategorisationConfusionMatrix.png"));
}
...
void WWCategorisationProcessor::init() {
  ...
  if (_ConfusionMatrixFileName != "") {
    ...
    _confusion_matrix_ef =
        new TH2F("ConfusionMatrix_efficiency", " ", _n_cat, -0.5, _n_cat - .5, _n_cat, -0.5, _n_cat - .5);
    ...
    std::stringstream bl2;
    bl2 << "#splitline{true e#nu qq}{cos #theta > " << _MCElCat_CosThCut << " }";
    std::stringstream bl3;
    bl3 << "#splitline{true e#nu qq}{cos #theta < " << _MCElCat_CosThCut << " }";
    _confusion_matrix_ef->GetXaxis()->SetBinLabel(
        2, bl2.str().c_str()); // #splitline{first line}{second line} # use latex with #
...
void WWCategorisationProcessor::processRunHeader(LCRunHeader*) { _nRun++; }
...
void WWCategorisationProcessor::processEvent(LCEvent* evt) {
  ...
  LCCollection *col_muon{}, *col_electron{}, *col_tau{}, *col_mcparticles{}, *col_pfominusisolatedthings{},
      *col_photons{};
  ...
  try {
    ...
    streamlog_out(MESSAGE) << "Input collections not found - skipping event and setting categories to -1 " << _nEvt
                           << std::endl;
  ...
  // If the incoming particle is an electron/positron, check size of the daughters array; 4,3 or 1. If anything else:
  // print
  if (incoming_pdg == 11) {
    ...
    if (daughters.size() <= 3) {                            // if daughters are of length 3 or 2
      for (unsigned int i = 0; i < daughters.size(); ++i) { // go through all daughters and get their PDG
        ...
        if (daughters_pdg > 20 && daughters_pdg < 30) { // make sure the incoming particle daughter is a boson
          ...
          for (unsigned int j = 0; j < boson_daughters.size();
               ++j) { // go through the boson daughters and add them to daughters and pdg lists
          ...
          general_mcdaughters.push_back(
              daughters[i]); // including mc information of remaining daughters that are not from W or Z
          general_daughters_pdgs.push_back(
              daughters_pdg); // including PDGs of remaining daughters that are not from W or Z
        } // by the end we get all 4f daughters PDGs and all 4f daughters mc info
      }
    } else if (daughters.size() == 4) { // go through all daughters of e-/e+ and save their PDG, then use the function
                                        // again to count and categorise the events
    ...
    int channel =
        CountingLeptons(general_daughters_pdgs); // which channel -> based of how many final state leptons we found
    if (channel == 0)
      true_cat = 0; // hadronic
    else if (channel == 2)
      true_cat = 5;          // leptonic
      ...
      for (unsigned int k = 0; k < general_daughters_pdgs.size(); ++k) {
        ...
        if (IsLepton(general_daughters_pdgs[k])) {
          ...
          if (general_daughters_pdgs[k] == 11)
            myelectron = general_mcdaughters[k];
          break;
        }
      }
    }
  ...
  // very forward (i.e. invisible) electrons in semileptonic channel from 'single-W' contribution go to separate true
  // category
  if (true_cat == 2) {
    std::vector<MCParticle*> deepest_electron{}; // sl electron after FSR -> getting daughters of the electron until we
                                                 // have a FSR electron (radiated a photon)
    bool last = false;
    while (!last) {
    ...
    if (cosine_mctheta > _MCElCat_CosThCut)
      true_cat = 1;
  }
  ...
  for (int i = 0; i < 4; ++i) {            // loop through isolated species
    for (int j = 0; j < n_parts[i]; ++j) { // loop through all isolated particles
      ...
      if (momtot > momtot_max[i])
        momtot_max[i] = momtot; // assign highest momentum for that species
    }
  }
  ...
  for (int i = 0; i < n_nonIsoPFOs; i++) { // loop over non-isolated PFOs
    EVENT::ReconstructedParticle* remainingpfo =
        dynamic_cast<EVENT::ReconstructedParticle*>(col_pfominusisolatedthings->getElementAt(i));
  ...
  TLorentzVector missing_neutrino4v_trianglecut =
      full_event4v_trianglecut - cm4v_initialconditions; // with triangle overlay cut
  ...
  if (reco_cat > 1 && (_nPFO <= 20 || _mInv <= 40))
    reco_cat = 5; // any non-hadronic to leptonic
  if (_nPFO >= 100 || _mInv >= 140)
    reco_cat = 0; // any to hadronic

  if (reco_cat == 5 && (40 <= _mInv && _mInv <= 140)) { // leptonic to semileptonic
    if (momtot_max[0] > momtot_max[1] && momtot_max[0] > momtot_max[2])
      reco_cat = 2; // sl electron
    if (momtot_max[1] > momtot_max[0] && momtot_max[1] > momtot_max[2])
      reco_cat = 3; // sl muon
    if (momtot_max[2] > momtot_max[0] && momtot_max[2] > momtot_max[1])
      reco_cat = 4; // sl tau
  }
  ...
  if (reco_cat == 0) { // hadronic to ...
    if (_nPFO <= 20)
      reco_cat = 5; // leptonic
    if ((20 < _nPFO && _nPFO <= 40) || _misspT >= 40)
      reco_cat = 1; // semileptonic invisible
  }
  ...
  if (_doCM)
    _confusion_matrix->Fill(true_cat, reco_cat);
...
bool WWCategorisationProcessor::IsLepton(int pdg) { return (pdg == 11 || pdg == 13 || pdg == 15); }

...
FPCCDClustering::FPCCDClustering() : Processor("FPCCDClustering") {
  ...
  registerProcessorParameter("EnergyDigitizatoin", "switch for digitization of energy deposit", _energyDigitization,
                             int(1));

  registerProcessorParameter("RandomNoise", "Random noise option", _randomNoise, int(0));

  registerProcessorParameter("FPCCD_PixelSize", "Pixel size of FPCCD (unit:mm) (default: 0.005)", _pixelSize,
                             float(0.0050));

  // Start of New edit
  //<New variable is 'PiXelSizeVec'(so far this variable wasn't vector). So I've replaced PixelSizeVec with something
  // which there were. And I
  ...
  registerProcessorParameter("PixelHeight", "Pixel Height(mm)", _pixelheight, float(0.015));

  registerProcessorParameter("PointResolutionRPhi", "R-Phi Resolution in VTX", _pointResoRPhi,
                             float(0.001440)); // 5/sqrt(12)

  registerProcessorParameter("PointResolutionZ", "Z Resolution in VTX", _pointResoZ,
                             float(0.001440)); // 5/sqrt(12)
  ...
  registerProcessorParameter(
      ...
      _positionReso_ReadingFile_ON, bool(true));

  registerProcessorParameter(
      "positionReso_ReadingFile", "On: reading file for allocation of position reso.", _positionReso_ReadingFile,
      std::string("MarlinReco/v01-05/TrackDigi/FPCCDDigi/src/FPCCDClustering_ResoMap_MS_EL_ON_ver0.dat"));

  registerProcessorParameter("noiseElectronSigma", "gaussian sigma of number of electron of noise", _electronNoiseRate,
                             double(50));

  registerProcessorParameter("NumberOfBitsForEnergyDeposit", "Number of Bits used for energy depsit", _nbitsForEdep,
                             int(7));

  registerProcessorParameter("NumberOfElectronForStep", "Number of electron for 1 step", _electronsPerStep, int(25));

  registerProcessorParameter("Threshold", "Cell Threshold in electrons", _threshold, double(200));

  registerProcessorParameter("ElectronsPerKeV", "Electrons per keV", _electronsPerKeV,
  ...
  registerProcessorParameter(
      ...
      "Clusters with ZWidth >= this value are discarded. minus value makes cut on the layer inactive", _firstCut.ZWidth,
      ZWidthVec);
  ...
  registerProcessorParameter(
      ...
      _firstCut.RPhiWidth, RPhiWidthVec);
  ...
  registerProcessorParameter(
      ...
      "Clusters with nPix >= this value are discarded. minus value makes cut on the layer inactive", _firstCut.nPix,
      nPixVec);

  registerProcessorParameter("ClusterRejection_Mori2ndCut_isActive", "true: active, false: inactive", _m2Cut.isActive,
                             bool(false));
  ...
  registerProcessorParameter("ClusterRejection_Mori2ndCut_areaZ_parameter",
                             "You can choose float[0,~900]. 0 discards most of bad clusters, although some clusters "
                             "are sacrified. minus value makes cut on the layer inactive",
                             _m2Cut.zpar, ZParVec);

  registerProcessorParameter("ClusterRejection_Kamai2ndCut_isActive", "true: active, false: inactive", _k2Cut.isActive,
                             bool(false));
  ...
  registerProcessorParameter("ClusterRejection_Kamai2ndCut_b_parameter",
                             "You can choose float[0,10]. less than 2 discards most of bad clusters, although some "
                             "clusters are sacrified. minus value makes cut on the layer inactive",
                             _k2Cut.bpar, BParVec);
  ...
  registerProcessorParameter("ClusterRejection_Kamai2ndCut_Save_minimum_Cluster_Z_Width_parameter",
                             ...
                             _k2Cut.minZWidth, MinZWidthVec);

  registerProcessorParameter("MakeRelationOftrkhitsAndsimthits", "If false, Clustering doesn't make LCRelation.",
                             _makeRelation, bool(true));

  registerProcessorParameter("RemovePixelHitsCollection", "Remove VTX Pixel Hits collection after processing",
                             _remove_pixelhits_collection, bool(false));

  // Input collections
  registerInputCollection(LCIO::SIMTRACKERHIT, "VTXCollectionName", "Name of the VTX SimTrackerHit collection",
                          _colNameSTH, std::string("VXDCollection"));
  ...
  // Output collections
  registerOutputCollection(LCIO::TRACKERHIT, "VTXHitCollection", "Name of the vxd TrackerHit output collection",
                           _outColNameVTX,
                           std::string("VXDTrackerHits")); // old: VXDTrackerHits

  registerOutputCollection(LCIO::LCRELATION, "SimTrackerHitRelCollection",
                           "Name of TrackerHit SimTrackerHit relation collection", _outRelColNameVTX,
                           std::string("VXDTrackerHitRelations")); // old: VTXTrackerHitRelations
#if ROOT_DEBUG
  registerProcessorParameter("outputRootFileName", "output root file name & path (default: ./positionReso.root)",
                             _rootFileName, std::string("./positionReso.root"));

  registerProcessorParameter("treeName",
                             "tree name which belongs to the above output root file name & path(default: tree)",
                             _treeName, std::string("t"));
#endif
}
...
void FPCCDClustering::init() {
  ...
  _tree->Branch("simthits_mcp_isCreatedInSimulation", _simthits.mcp_isCreatedInSimulation,
  ...
  _tree->Branch("simthits_mcp_vertexIsNotEndpointOfParent", _simthits.mcp_vertexIsNotEndpointOfParent,
                "simthits_mcp_vertexIsNotEndpointOfParent[nlinks]/I");
  _tree->Branch("simthits_mcp_isDecayedInTracker", _simthits.mcp_isDecayedInTracker,
                "simthits_mcp_isDecayedInTracker[nlinks]/I");
  _tree->Branch("simthits_mcp_isDecayedInCalorimeter", _simthits.mcp_isDecayedInCalorimeter,
                "simthits_mcp_isDecayedInCalorimeter[nlinks]/I");
  _tree->Branch("simthits_mcp_hasLeftDetector", _simthits.mcp_hasLeftDetector,
...
// =====================================================================
void FPCCDClustering::processRunHeader(LCRunHeader* /*run*/) { _nRun++; }
...
void FPCCDClustering::modifyEvent(LCEvent* evt) {
  ...
  try {
    ...
    if (_debug >= 1) {
      std::cout << "Collection " << _colNameSTH.c_str() << " is unavailable in event " << _nEvt << std::endl;
    }
  }
  ...
  if (pHitCol != 0 && STHcol != 0) {
    ...
    int nhit = theData.unpackPixelHits(
        *pHitCol); // This unpacks LCGenericObjectVec made in FPCCDDigitizer (precisely, FPCCDData).
  ...
  if (pHitCol != 0 && STHcol != 0) {
    ...
    try {
      rootRelCol = dynamic_cast<LCCollectionVec*>(evt->getCollection(_outRelColNameVTX));
    } catch (DataNotAvailableException& e) {
      Go = false;
      std::cout << "miss getCollection!! at _nEvt = " << _nEvt << std::endl;
      if (_debug >= 1) {
        std::cout << "Collection " << _outRelColNameVTX.c_str() << " is unavailable in event " << _nEvt << std::endl;
      }
    }
    /*
       _tree->Branch("nlinks",&_link.nlink,"nlinks/I");
    ...
    if (Go == true) {
      ...
      for (unsigned int ri = 0; ri < _link.nlink; ri++) {
        ...
        if (signRPhi > 0) {
          ...
          _diff.RPhi[ri] =
              (-1) * sqrt(pow(_trkhits.x[ri] - _simthits.x[ri], 2) + pow(_trkhits.y[ri] - _simthits.y[ri], 2));
        }
        ...
        _simthits.pAbs[ri] =
            sqrt(pow(static_cast<double>(_simthits.px[ri]), 2) + pow(static_cast<double>(_simthits.py[ri]), 2) +
                 pow(static_cast<double>(_simthits.pz[ri]), 2));
        _simthits.theta[ri] =
            180 / M_PI * acos(static_cast<double>(_simthits.pz[ri]) / _simthits.pAbs[ri]); // from 0 to pi
        _simthits.phi[ri] =
            180 / M_PI *
            atan2(static_cast<double>(_simthits.py[ri]), static_cast<double>(_simthits.px[ri])); // from -pi to pi
        ...
        double TrkXi = tposR * (cos(tposphi) * _geodata[mly].sinphi[mld] - sin(tposphi) * _geodata[mly].cosphi[mld]) -
                       _geodata[mly].sximin;
...
// ====================================================================
void FPCCDClustering::makeTrackerHitVec(FPCCDData* pHitData, LCCollection* STHcol, LCCollectionVec* relCol,
  ...
  for (int layer = 0; layer < _nLayer; layer++) {
    ...
    for (int ladder = 0; ladder < nladder; ladder++) {
      ...
      while (it != pHitData->itEnd(layer, ladder)) {
        ...
        // Set the noise
        aHit->setEdep(aHit->getEdep() + gsl_ran_gaussian(_rng, _electronNoiseRate / _electronsPerKeV) *
                                            1e-6); //_electronNoiseRate = 50 (by default)
      ...
      if (_randomNoise && _energyDigitization) {
        ...
        for (unsigned int i = 0; i < gsl_ran_poisson(_rng, (sxiwidth / _pixelSizeVec[layer]) *
                                                               (2 * hlength / _pixelSizeVec[layer]) * 3.1671 * 1e-5);
             i++) { // Random noise is generated by 4 sigma plobability.
          ...
          if (noiseIt == ladderHit.end()) {
            ...
            FPCCDPixelHit* noiseHit =
                new FPCCDPixelHit(layer, ladder, noiseXi, noiseZeta, noiseEdep, FPCCDPixelHit::kBKG, 0);
...
void FPCCDClustering::makeTrackerHit(LCCollection* STHcol, int layer, int ladder, FPCCDClusterVec_t& clusterVec,
                                     std::multimap<std::pair<int, int>, SimTrackerHit*> /*relMap*/,
                                     LCCollectionVec* relCol, LCCollectionVec* trkHitVec) {
  ...
  for (unsigned int ic = 0; ic < clusterVec.size(); ic++) {
    ...
    for (unsigned int i = 0; i < nPix; i++) {
      /***Mori added 20121203***/
      int overlaidSize =
          (*cluster)[i]->getSizeOfOrderID(); // Sometimes one pixel is generated by more than one simthits.
      ...
      if (i == 0) {
        ...
        if (aHit->getXiID() > maxxi) {
          maxxi = aHit->getXiID();
          maxXiHit = aHit;
        }
        if (aHit->getXiID() < minxi) {
          minxi = aHit->getXiID();
          minXiHit = aHit;
        }
        if (aHit->getZetaID() > maxzeta) {
          maxzeta = aHit->getZetaID();
          maxZetaHit = aHit;
        }
        if (aHit->getZetaID() < minzeta) {
          minzeta = aHit->getZetaID();
          minZetaHit = aHit;
        }
      }
      ...
      } else if (addedQuality == FPCCDPixelHit::kSignalOverlap) {
        trackquality = FPCCDPixelHit::kSignalOverlap;
      }
    }
    ...
    if (_firstCut.isActive != true || _firstCut.ZWidth[layer] < 0 ||
        zetaWidth < static_cast<unsigned int>(_firstCut.ZWidth[layer])) {
      if (_firstCut.isActive != true || _firstCut.RPhiWidth[layer] < 0 ||
          xiWidth < static_cast<unsigned int>(_firstCut.RPhiWidth[layer])) {

        unsigned short int tilt = 0;
        if (xiWidth == 1 || zetaWidth == 1)
          tilt = 0;
        else {
          if (maxXiHit->getZetaID() - minXiHit->getZetaID() > 0 && maxZetaHit->getXiID() - minZetaHit->getXiID() > 0)
            tilt = 1;
          else if (maxXiHit->getZetaID() - minXiHit->getZetaID() <= 0 &&
                   maxZetaHit->getXiID() - minZetaHit->getXiID() <= 0)
            tilt = 2;
          else
            tilt = 0;
        }
        ...
        if (_m2Cut.isActive != true || _m2Cut.zpar[layer] < 0 ||
            ((mxi * mzeta >= -_m2Cut.zpar[layer] || tilt != 1) && (mxi * mzeta <= _m2Cut.zpar[layer] || tilt != 2))) {

          double newpos[3];
          newpos[0] =
              (xiave + _geodata[layer].sximin) * _geodata[layer].sinphi[ladder] + eta0 * _geodata[layer].cosphi[ladder];
          ...
          // Kamai2ndCut
          if (_k2Cut.isActive != true || int(zetaWidth) <= _k2Cut.minZWidth[layer] ||
              float(zetaWidth) > (std::abs(newpos[2]) * _pixelheight / sqrt(pow(newpos[0], 2) + pow(newpos[1], 2)) /
                                      _pixelSizeVec[layer] -
                                  _k2Cut.bpar[layer])) {
            ...
            if (_positionReso_ReadingFile_ON == true) {
              ...
              if (xiWidth <= 49) {
                ...
                if (pointResoRPhi < 0.00001) {
                  pointResoRPhi = (sqrt((float)(xiWidth + 1.5)) - 1) * 1e-3;
                }
              } else {
                pointResoRPhi = (sqrt((float)(xiWidth + 1.5)) - 1) * 1e-3;
              }
              ...
              if (zetaWidth <= 49) {
                ...
                if (pointResoZ < 0.00001) {
                  pointResoZ = (sqrt((float)(zetaWidth + 1.5)) - 1) * 1e-3;
                }
              } else {
                pointResoZ = (sqrt((float)(zetaWidth + 1.5)) - 1) * 1e-3;
              }
            ...
            cellid1 = ((trackquality << 30) & 0xc0000000) | ((tilt << 28) & 0x30000000) | ((nPix << 18) & 0x0ffc0000) |
                      ((zetaWidth << 9) & 0x0003fe00) | ((xiWidth) & 0x000001ff);
...
void FPCCDClustering::makeClustersInALadder(int layer, FPCCDLadderHit_t& ladderHit, FPCCDClusterVec_t& clusterVec) {
  ...
  while (ih != ladderHit.end()) {
    ...
    for (int i = -1; i <= 1; i++) { // Set initial search direction, neighbouring 8 pixels are searched
      for (int j = -1; j <= 1; j++) {
        if (i != 0 || j != 0) {
          direction.push(Direction_t(i, j));
        }
      }
    }
    ...
    if (cluster->size() > 0) {
      // Mori added
      if (_firstCut.isActive != true || _firstCut.nPix[layer] < 0 ||
          cluster->size() < static_cast<unsigned int>(_firstCut.nPix[layer])) {

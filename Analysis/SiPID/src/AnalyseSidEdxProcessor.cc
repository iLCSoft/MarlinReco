...
AnalyseSidEdxProcessor::AnalyseSidEdxProcessor()
    : Processor("AnalyseSidEdxProcessor"), m_rootFileName(""), m_trackColName(""), m_linkColName(""),
      m_trkHitCollNames(), rootfile(NULL), tree(NULL), nTracks(0), lastRunHeaderProcessed(-1) {
  ...
  registerProcessorParameter("RootFilename", "Name of output root file", m_rootFileName,
                             std::string("SiTracker_dEdx.root"));

  registerInputCollection(LCIO::TRACK, "TrackCollectionName", "Name of the input Track collection", m_trackColName,
                          std::string("SiTracks"));

  registerInputCollection(LCIO::LCRELATION, "LinkCollectionName", "Name of the LCRelation collection", m_linkColName,
...
void AnalyseSidEdxProcessor::processRunHeader(LCRunHeader* run) { lastRunHeaderProcessed = run->getRunNumber(); }
...
void AnalyseSidEdxProcessor::processEvent(LCEvent* evt) {
  ...
  for (int i = 0; i < nTracks; i++) {
    ...
    for (unsigned int ihit = 0; ihit < trackhits.size(); ihit++) {
      ...
      zTrackHit.push_back(hitpos.z());
      xTrackHit.push_back(hitpos.x());
      yTrackHit.push_back(hitpos.y());

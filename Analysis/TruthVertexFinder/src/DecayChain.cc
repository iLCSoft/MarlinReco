...
void DecayChain::Add(MCParticle* particle) { myParticles.push_back(particle); }
...
int DecayChain::GetSize() const { return myParticles.size(); }
const std::vector<MCParticle*>& DecayChain::GetAll() const { return myParticles; }
void DecayChain::Merge(DecayChain& other) {
  for (int i = 0; i < other.GetSize(); i++) {
    Add(other.Get(i));
  }
}
std::string DecayChain::GetName() const { return myName; }
int DecayChain::GetParentPDG() const { return myPDG; }
